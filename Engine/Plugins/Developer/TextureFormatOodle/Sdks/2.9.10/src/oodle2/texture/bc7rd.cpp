...
struct BC7Constraints
{
	...
	{
		// NOTE(fg): trivial to optimize later
		U32 mask = 0;
...
static int seed_past_blocks(vector<bc7rd_windowentry>& past_block_window,const BlockSurface * baseline_blocks,BC7Flags flags)
{
	...
	for LOOP(s,num_seeds)
	{
		...
		bc7enc_state_from_bits(&past_block_window[s].st,bc7bits_U8ptr(&counting[s].val), flags & ~BC7ENC_PRESERVE_EXTREMES);
	}
	#endif
...
static bool are_endpoints_permitted(const BC7BlockState & st, const bc7rd_blockinfo& info)
{
...
static bool are_indices_permitted(const BC7BlockState & st, const bc7rd_blockinfo& info, BC7Flags *pFlags)
{
	*pFlags = info.flags;
	...
	else
	{
		...
		// indices work.
		return (flags & BC7ENC_DISABLE_MODE67) == 0;
	}
}
	...
	const bc7bits & bits_mask)
{
	...
	for LOOP(i,set_size)
	{
		...
		//
		// NOTE: definitely can't change pbits on constrained "preserve extremes"
		// blocks, though
	...
	if (cls_counts[cur_max_cls] == 1)
	{
		...
		bc7enc_reindex_block(&reindex_st,input.subsets,flags,false,true);
	...
	const bc7bits & bits_mask)
{
	...
	for LOOP(i,set_size)
	{
		...
		// Track constraints we have to do enforce on the endpoints and do other validity checks.
		//
	...
	bool do_indices)
{
	...
	for LOOP(mode,8)
	{
		...
		#error can't do this right now (does not handle preserve extremes)
	...
	BC7Flags flags)
{
	...
	for LOOP(bi,nb)
	{
		...
		//
		// extra note: can't do with_pbits for preserve extremes mode
		// -> with_pbits = true is pretty much dead now
	...
	for LOOP(mode,BC7_NUM_MODES)
	{
		...
		for (int vbase = 0; vbase < vcount; )
		{
			...
			entry.ind_constraints[0] = BC7Constraints::none();
			entry.ind_constraints[1] = BC7Constraints::none();
			entry.block_link = -1;
			entry.count = 0;
			entry.count_log2_count = 0;
			entry.distortion_sum = 0;
			entry.merged_onto = -1;
		...
		for LOOPVEC(vi,v_indices_pb[mode])
		{
			...
			// Leave blocks with "preserve extremes" constraints untouched in this pass for now
		...
		for LOOP(bi,nblocks)
		{
			...
			*/
			...
			for LOOP(mtfwindowi,mtfwindi_iters)
			{
				...
				bool change_pbits = false; // NOTE: can never turn this on for "preserve extremes" candidate subst blocks
				//bool change_pbits = slow_exact; // do change pbits only on the last refine? -> meh
				int bits_changed = bc7enc_reindex_block(&st,input->subsets,infos[bi].flags,change_pbits,slow_exact);
				...
				#ifdef CHECK_EXTREMES_PRESERVED
				{
					...
					if (!dec_constraints.superset_of(infos[bi].constraints))
					{
						...
						rrprintf("extremes mismatch! matched mode=%d isb=%d rb=%d cmask=0x%08x slow_exact=%d\n", st.mode, st.subsets[0].isbit, st.subsets[0].rbit, cmask, slow_exact);
			...
			{	
				...
				// Don't try substituting a higher-class mode
				if ( mode_class[st.mode] > mode_class[best_st.mode] )
					continue;

				// If it's a mode we're not allowed to use, skip candidate
				BC7Flags flags = infos[bi].flags;
				if ( ! are_indices_permitted(st,infos[bi],&flags) )
					continue;
				...
				int bits_changed = bc7enc_find_endpoints(&st,block,flags,change_pbits,slow_anneal);
				...
				#ifdef CHECK_EXTREMES_PRESERVED
				{
					...
					if (!dec_constraints.superset_of(infos[bi].constraints))
					{
						...
						rrprintf("extremes mismatch! matched mode=%d isb=%d rb=%d constraint mask=0x%08x slow_anneal=%d\n", st.mode, st.subsets[0].isbit, st.subsets[0].rbit, cmask, slow_anneal);
			...
			{	
				...
				for LOOP(endpi,matrix_dim_end)
				{
					...
					{								
						...
						bc7_decode_block4x4a(decoded,bc7bits_U8ptr(&pair1),infos[bi].flags);
							...
							if ( satisfied_constraints.superset_of(infos[bi].constraints) )
							{
								...
								if ( cur_D < best_J )
								{
									...
									if ( cur_J < best_J )
									{
										...
										bc7enc_state_from_bits(&best_st,bc7bits_U8ptr(&pair1),emit_flags);
