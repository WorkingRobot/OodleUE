...
static S32 AnyIndexD_batch_lookup_SSE4(S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count)
{
	...
	for (; i < (count & ~3); i += 4)
	{
		...
		min_so_far = vmin(min_so_far, Vec128_S32(v_ssd0));
...
S32 compute_sad_bound_for_ssd_threshold(S32 delta_ssd_per_pixel_per_sad[16][8], S32 base_ssd, S32 ssd_limit, S32 * out_ssd_limit)
{
	...
	for (int i=0; i < 16; ++i)
	{
		...
		Vec128_U32 a = vmin(Vec128_U32::loadu(delta_ssd_per_pixel_per_sad[i] + 0), limit).shl<4>() + vi;
		Vec128_U32 b = vmin(Vec128_U32::loadu(delta_ssd_per_pixel_per_sad[i] + 4), limit).shl<4>() + vi;
		vi += Vec128_U32(1);
	...
	for (sad=1;;++sad)
	{
		...
		Vec128_U32 ab = vmin(cur_ssd0, cur_ssd1);
		Vec128_U32 cd = vmin(cur_ssd2, cur_ssd3);
		Vec128_U32 abcd = vmin(ab, cd);
