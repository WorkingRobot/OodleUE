...
{
	if ( num_surfaces <= 0 || num_surfaces > OODLETEX_MAX_SURFACE_COUNT )
	{
		return OodleTex_Err_BadSurfaceCount;
	}
...
rrDXTCOptions dxtcoptions_from_bcnflags(OodleTex_BC to_bcn, OodleTex_BCNFlags flags)
{
	U32 dxtcoptions = 0;
	...
	if ( (flags & OodleTex_BCNFlag_PreserveExtremes_BC3457) )
	{
		switch ( to_bcn )
		{
		case OodleTex_BC3:
		...
		case OodleTex_BC5U:
		case OodleTex_BC5S:
		case OodleTex_BC7RGBA: // explicitly not including BC7RGB, since preserve extremes applies to A channel
			dxtcoptions |= rrDXTCOptions_BC3457_PreserveExtremes;
			break;

		default:
			rrprintf("WARNING: preserve extremes unsupported when encoding to format %s, ignored.\n", OodleTex_BC_GetName(to_bcn));
		}
	}
	...
	if ( flags & OodleTex_BCNFlag_LinearToSRGB )
		dxtcoptions |= rrDXTCOptions_LinearToSRGB;
...
OOFUNC1 const char * OOFUNC2 OodleTex_Err_GetName(OodleTex_Err error)
{
	...
	// this is the function you call on failure of OodleTex_Enter to
	...
	switch (error)
	{
		...
		CASE_TO_STRING(OodleTex_Err_BadSRGBFormat)
		CASE_TO_STRING(OodleTex_Err_BadSurfaceCount)

		CASE_TO_STRING(OodleTex_Err_Canceled)
	default:
		break;
	}
...
static bool is_unsigned_bc45(OodleTex_BC bcn)
{
	return (bcn == OodleTex_BC4U || bcn == OodleTex_BC5U);
}

static bool is_bc4(OodleTex_BC bcn)
{
	return bcn == OodleTex_BC4S || bcn == OodleTex_BC4U;
}

static bool is_bc5(OodleTex_BC bcn)
{
	return bcn == OodleTex_BC5S || bcn == OodleTex_BC5U;
}

static bool is_bc6(OodleTex_BC bcn)
{
	return bcn == OodleTex_BC6S || bcn == OodleTex_BC6U;
}

static bool is_bc1237(OodleTex_BC bcn)
{
	return bcn == OodleTex_BC1
		|| bcn == OodleTex_BC1_WithTransparency
		|| bcn == OodleTex_BC2
		|| bcn == OodleTex_BC3
		|| bcn == OodleTex_BC7RGB
		|| bcn == OodleTex_BC7RGBA;
}

static bool bcn_is_float(OodleTex_BC bcn)
{
	return is_bc6(bcn);
}

static OodleTex_Err OodleTex_Entry_ValidateFormats(
	OodleTex_BC to_bcn,OodleTex_PixelFormat from_format,OodleTex_BCNFlags flags)
{
	// Is this a format we know?
	rrPixelFormat pf = OodleTex_PixelFormat_to_rrPixelFormat(from_format);
	if ( pf == rrPixelFormat_Invalid )
		return OodleTex_Err_BadPixelFormat;
	...
	// Linear to sRGB conversion allowed only for BC1237 and 4xF32 input
	if (flags & OodleTex_BCNFlag_LinearToSRGB)
	{
		if (is_bc4(to_bcn) || is_bc5(to_bcn) || (is_bc1237(to_bcn) && from_format != OodleTex_PixelFormat_4_F32_RGBA))
			return OodleTex_Err_BadSRGBFormat;
	}

	if (info->isFloat)
	{
		// Source image is float
		const U32 pftype = RR_PIXELFORMAT_GET_TYPE(pf);
		
		// F16 input is only accepted for float BCn formats
		if ( pftype == RR_PIXELFORMAT_TYPE_F16 && !bcn_is_float(to_bcn) )
			return OodleTex_Err_BadPixelFormat;

		// must have 4 component input for BC1237
		if (is_bc1237(to_bcn) && info->channels != 4)
			return OodleTex_Err_BadPixelFormat;

		// must have 1 component input for BC4
		if (is_bc4(to_bcn) && info->channels != 1)
			return OodleTex_Err_BadPixelFormat;

		// must have 2 component input for BC5
		if (is_bc5(to_bcn) && info->channels != 2)
			return OodleTex_Err_BadPixelFormat;

		// must have at least 3 component input for BC6
		if (is_bc6(to_bcn) && info->channels < 3)
			return OodleTex_Err_BadPixelFormat;
	...
	{
		// Source image is integer
		const U32 pftype = RR_PIXELFORMAT_GET_TYPE(pf);

		// Enforce restrictions on which kinds of BCn formats can take which types of inputs
		switch ( pftype )
		{
		case RR_PIXELFORMAT_TYPE_U8:
			// U8 is not allowed as input for signed BC4/5 or BC6,
			// allowed for everything else.
			if ( is_signed_bc45(to_bcn) || is_bc6(to_bcn) )
				return OodleTex_Err_BadPixelFormat;
			break;

		case RR_PIXELFORMAT_TYPE_U16:
			// U16 is only allowed as input for unsigned BC4/5
			if ( ! is_unsigned_bc45(to_bcn) )
				return OodleTex_Err_BadPixelFormat;
			break;

		case RR_PIXELFORMAT_TYPE_S8:
		case RR_PIXELFORMAT_TYPE_S16:
			// Signed formats are only allowed as input for signed BC4/5
			if ( ! is_signed_bc45(to_bcn) )
				return OodleTex_Err_BadPixelFormat;
			break;

		default:
			// S32 and complex pixel formats are never allowed as compression input.
			// F16 and F32 are in the "isFloat" branch above.
			return OodleTex_Err_BadPixelFormat;
		}
	}
	...
	int num_job_threads,void * jobify_user_ptr)
{
	...
	err = OodleTex_Entry_ValidateFormats(to_bcn,from_format,flags);
	if ( err != OodleTex_Err_OK )
		return err;
	...
	err = rrSurfaceDXTC_CompressBCN_Blocks(&to_surf,&fm_surf,dxtc_level,num_job_threads,dxtcoptions,jobify_user_ptr);
	...
	int num_job_threads,void * jobify_user_ptr)
{
	...
	vector_st<rrSurface, 1> from_rrs;
	from_rrs.resize(num_from_surfaces);
	for (SINTa i = 0; i < num_from_surfaces; ++i)
		init_rrs_from_texsurf(&from_rrs[i], from_surfaces + i, fm_pf);

	if ( layout == NULL )
	{
		SINTa from_num_blocks = TotalBlockCount(from_rrs.data(),(int)num_from_surfaces);

		if ( num_blocks != from_num_blocks )
			return OodleTex_Err_BlockCountMismatch;
	}

	BlockSurface_AllocCopy_from_RRS_Layout(&from_blocks,from_rrs.data(),(int)num_from_surfaces,layout);
	...
	int num_job_threads,void * jobify_user_ptr)
{
	...
	OodleTex_BCNFlags flags = options->bcn_flags;
	...
	BlockSurfaceObj from_blocks;
	vector_st<rrSurfaceObj, 1> from_rrs;
	from_rrs.resize(num_from_surfaces);
	...
	// Only the RDO encoders for native float formats know what to do with float inputs; for the rest,
	// don't try to get fancy, just downconvert right here.
	if ( rrPixelFormat_GetInfo(from_rrs[0].pixelFormat)->isFloat && ! bcn_is_float(to_bcn) )
	{
		// Determine target format.
		rrPixelFormat target_format = rrPixelFormat_Invalid;
		...
		case OodleTex_BC7RGBA:
			target_format = rrPixelFormat_R8G8B8A8;
			break;

		case OodleTex_BC4S:
			target_format = rrPixelFormat_1_S16;
			break;

		case OodleTex_BC4U:
			target_format = rrPixelFormat_1_U16;
			break;

		case OodleTex_BC5S:
			target_format = rrPixelFormat_2_S16;
			break;

		case OodleTex_BC5U:
			target_format = rrPixelFormat_2_U16;
			break;
		...
		if ( flags & OodleTex_BCNFlag_LinearToSRGB )
		{
			// Note that the remap has to happen and clear the flag since we're about to switch to a sRGB pixel format
			remap = rrRangeRemap_LinearToSRGB;
			flags = (OodleTex_BCNFlags) (flags & ~OodleTex_BCNFlag_LinearToSRGB);
		}

		// Do the conversion
		// This will convert the surfaces from views to owned pointers automatically.
		for (SINTa i = 0; i < num_from_surfaces; ++i)
		{
			// Shouldn't fail here, we should have done all validation up front
			if ( ! rrSurface_ChangeFormatNormalized(&from_rrs[i], target_format, remap) )
				return OodleTex_Err_Internal;
		}
	}

	BlockSurface_AllocCopy_from_RRS_Layout(&from_blocks,from_rrs.data(),(int)num_from_surfaces,use_layout);
		...
		num_blocks >= RDO_MIN_BLOCK_COUNT )
	{
		...
		*/
		...
		if ( err == OodleTex_Err_OK )
		{
			err = rrDXTCRD_Encode_RDO(&encode_to_surf,&from_blocks,from_rrs.data(),(int)num_from_surfaces,use_layout,rdo_lagrange_lambda,dxtcoptions,jobify_user_ptr,num_job_threads,dxtc_metric,rdopts);
			
			// Only log internal errors, more specific errors are expected to either have logged their cause already
			// or not require any handling (e.g. cancellation)
			if ( err == OodleTex_Err_Internal )
			{
	...
	else
	{
		...
		err = rrSurfaceDXTC_CompressBCN_Blocks(&encode_to_surf,&from_blocks,rdopts.effort,num_job_threads,dxtcoptions,jobify_user_ptr);

		// Only log internal errors (see note above)
		if ( err == OodleTex_Err_Internal )
		{
	...
	if ( universal_tiling )
	{
		BlockSurface_Copy_Detile(&to_surf,&encode_to_surf,from_rrs.data(),(int)num_from_surfaces,use_layout->m_tile_w,use_layout->m_tile_h);
...
{
	return OodleTex_DecodeBCN_Blocks_Ex(
		to_pixel_blocks, to_format, num_blocks, from_bcn, from_bcn_blocks,
		OodleTex_BCNDecodeFlags_None);
}
	...
	)
{
	...
	// sRGB to linear conversion allowed only for BC1237 and 4xF32 output
	if (flags & OodleTex_BCNDecodeFlag_SRGBToLinear)
	{
		if (is_bc4(from_bcn) || is_bc5(from_bcn) || (is_bc1237(from_bcn) && to_pf != rrPixelFormat_4_F32))
			return OodleTex_Err_BadSRGBFormat;
		remap = rrRangeRemap_SRGBToLinear;
	}
	...
	if ( to_bs_decomp.blocks != to_bs.blocks )
	{
		...
		ok = BlockSurface_BlitNormalized(&to_bs,&to_bs_decomp,remap);
		if ( ! ok )
			return OodleTex_Err_Internal;
	}
...
{
	return OodleTex_DecodeBCN_LinearSurfaces_Ex(
		to_surfaces, num_to_surfaces, to_format, from_bcn, from_bcn_blocks,
		num_blocks, layout, OodleTex_BCNDecodeFlags_None);
}
	...
	)
{
	...
	BlockSurface_AllocCopyOrSetViewIfFormatMatches_Normalized(&decomp_to_format,&decomp,to_pf,remap);

	vector_st<rrSurface, 1> to_rrs;
	to_rrs.resize(num_to_surfaces);

	for (SINTa i = 0; i < num_to_surfaces; ++i)
		init_rrs_from_texsurf(&to_rrs[i], to_surfaces + i, to_pf);

	BlockSurface_Copy_to_RRS_SameFormat_Layout(to_rrs.data(),(int)num_to_surfaces,&decomp_to_format,layout);
	...
	)
{
	...
	BlockSurface_Copy_to_RRS_SameFormat_Layout(to_rrs.data(),(int)num_to_surfaces,&from_bs,layout);
	...
	)
{
	...
	OodleTex_Layout * layout = OodleNewT(OodleTex_Layout)(bytes_per_block);
	if ( !layout )
		return OodleTex_Err_Internal;
	...
	const OodleTex_Surface * surf2,OodleTex_PixelFormat format2)
{
	...
	if ( do_hdr_diff )
	{
		...
		// CB 05-04-2021 :
		//	change to MRSSEN
		F64 mrssen = rrSurface_GetMRSSE_Neighborhood(&rrs1,&rrs2,channels_to_diff);
		mrssen = sqrt(mrssen);
	...
	else
	{
		...
		F64 rmse_normalized = rrSurface_GetRMSE_Normalized(&rrs1,&rrs2,nullptr,channels_to_diff);
...
OodleTex_PixelFormat OodleTex_Surface_from_RRS( OodleTex_Surface * input, const rrSurface * rrs )
{
	...
	input->width = check_value_cast<S32>(rrs->width);
	input->height = check_value_cast<S32>(rrs->height);
...
static OO_BOOL OODLE_CALLBACK default_cancel_requested_plugin(void * user_ptr)
{
	return false;
}

t_fp_OodleTex_Plugin_CancelRequested * g_fp_CancelRequested = default_cancel_requested_plugin;

void OodleTex_Plugins_SetCancelRequested(t_fp_OodleTex_Plugin_CancelRequested * fp_CancelRequested)
{
	// If null is supplied, replace with default plugin (never cancels)
	if ( ! fp_CancelRequested )
	{
		fp_CancelRequested = default_cancel_requested_plugin;
	}

	g_fp_CancelRequested = fp_CancelRequested;
}
