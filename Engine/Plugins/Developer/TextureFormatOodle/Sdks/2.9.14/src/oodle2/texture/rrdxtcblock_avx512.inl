...
static RADFORCEINLINE void UnpackBGRAto16_Weighted(Vec512 & out_br, Vec512 & out_ga, const Vec512 & pixels)
{
	...
	const Vec512 scale_ga = _mm512_set1_epi32(0x02000100);

	out_br = _mm512_and_si512(pixels, mask_br);
	out_ga = _mm512_maddubs_epi16(pixels, scale_ga);
}
...
{
	const Vec512 scale_br = _mm512_set1_epi16(2);
	const Vec512 scale_ga = _mm512_set1_epi32(0x04000200);

	out_br = _mm512_maddubs_epi16(pixels, scale_br);
	out_ga = _mm512_maddubs_epi16(pixels, scale_ga);
}
