...
// Limit max job size so we have reasonable frequency for cancellation points
#define BLOCKSURFACE_JOBSLICER_MAX_BLOCKS 32768
...
void OODLE_CALLBACK rrSurfaceJobSlicer_Job( void * pdata)
{
	...
	OodleTex_Err ret = (*func)(d,d->pdata);
	
	d->func_ret = ret;
}

template<typename Tjob>
static OodleTex_Err resolve_error(const Tjob* jobs, SINTa count)
{
	for LOOP(i, count)
	{
		if (jobs[i].func_ret != OodleTex_Err_OK)
		{
			return jobs[i].func_ret;
		}
	}
...
OodleTex_Err rrSurface_JobSlicer(rrSurface * to, const rrSurface * from,int num_workers,void * jobify_user_ptr,t_JobSlicer_Func * pfunc,void * pdata)
{
	...
	for LOOP(j,num_jobs)
	{
		...
		// start with success
		job_data[j].func_ret = OodleTex_Err_OK;
	...
	return resolve_error(job_data.data(), num_jobs);
}
...
OodleTex_Err BlockSurface_JobSlicer(BlockSurface * to, const BlockSurface * from,int num_workers,void * jobify_user_ptr,t_BlockJobSlicer_Func * pfunc,void * pdata)
{
	...
	// Don't make enormous jobs so we have reasonable cancellation behavior in any case
	num_jobs = RR_MAX(num_jobs, count / BLOCKSURFACE_JOBSLICER_MAX_BLOCKS);
