...
rrbool rrSurfaceDXTC_DecompressBCN(rrSurface * to, const rrSurface * from, rrPixelFormat format_hint )
{
	...
	// Work a (block) row at a time to avoid both big allocs and streaming data in and out
	// of the cache unnecessarily.
	const SINTa row_step = 4;
	BlockSurfaceObj decomp, decomp_to_format;

	for (SINTa y0 = 0; y0 < from->height; y0 += row_step)
	{
		SINTa height = RR_MIN(row_step, from->height - y0);
		
		// rrSurface View of source slice
		rrSurface temp_view;
		rrSurface_SetView(&temp_view, from, 0, y0, from->width, height);

		// BlockSurface view of source slice
		BlockSurface from_bs;
		BlockSurface_SetView_of_RRS_BCN(&from_bs,&temp_view);
		...
		// Copy blocks to linear dest slice with no format change :
		rrSurface_SetView(&temp_view, to, 0, y0, from->width, height);
		BlockSurface_Copy_to_RRS_SameFormat(&temp_view,1,&decomp_to_format);
	}
...
OodleTex_Err rrSurfaceDXTC_CompressBCN_Blocks_SingleThreaded(BlockSurface * to_full, const BlockSurface * from_full, rrDXTCLevel dxtcQualityLevel, rrDXTCOptions options)
{
	...
	for (SINTa slice_start = 0; slice_start < from_full->count; slice_start += blocks_per_slice)
	{
		...
		case rrPixelFormat_BC7:
			if ( ! rrSurfaceDXTC_CompressBC7(&to,&from,dxtcQualityLevel,options) )
				return OodleTex_Err_Internal;
			break;
		default: RR_ASSERT_FAILURE("bad BCN"); return OodleTex_Err_Internal;
		}
	}
...
OodleTex_Err rrSurfaceDXTC_CompressBCN_JobSlicer_Func(BlockSurfaceJobSlicer_Data * slice,void * data)
{
	rrSurfaceDXTC_CompressBCN_JobSlicer_Data * bcn_data = (rrSurfaceDXTC_CompressBCN_JobSlicer_Data *)data;

	if ( g_fp_CancelRequested( bcn_data->jobify_user_ptr ) )
	{
		return OodleTex_Err_Canceled;
	}
	
	OodleTex_Err err = rrSurfaceDXTC_CompressBCN_Blocks_SingleThreaded(&slice->to_sliced,&slice->from_sliced,bcn_data->dxtcQualityLevel,bcn_data->options);
...
OodleTex_Err rrSurfaceDXTC_CompressBCN_Blocks(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxtcQualityLevel, int num_workers, rrDXTCOptions options, void * jobify_user_ptr)
{
	...
	OodleTex_Err err = BlockSurface_JobSlicer(to,from,num_workers,jobify_user_ptr,rrSurfaceDXTC_CompressBCN_JobSlicer_Func,&data);
...
OodleTex_Err rrSurfaceDXTC_CompressBCN_LinearSurface(rrSurface * to, const rrSurface * from, rrDXTCLevel dxtcQualityLevel, int num_workers, rrDXTCOptions options, void * jobify_user_ptr)
{
	...
	OodleTex_Err ret = rrSurfaceDXTC_CompressBCN_Blocks(&to_blocks,&from_blocks,dxtcQualityLevel,num_workers,options,jobify_user_ptr);
