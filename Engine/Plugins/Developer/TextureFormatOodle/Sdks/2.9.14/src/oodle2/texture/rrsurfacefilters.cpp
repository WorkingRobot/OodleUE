...
static inline SINTa MirrorIndex(SINTa i,SINTa w)
{
	if( (UINTa)i < (UINTa)w ) return i;
	SINTa r;
	...
	// if "w" is tiny and you're trying to index way out of bounds (farther than w out),
	//	it will now be out the other side
	//	eg. padding of 8 on a 4x4 image
	// just do an extra clamp in bounds :
	r = RR_CLAMP(r,0,w-1);
	
	return r;
}

// alternative ways to do MirrorIndex :
// 1. just modulo in (2*w) , then reverse index if >= w
// 2. first return i if < w , then modulo in (w), then reverse index
// these have the same behavior in [-w,2w] but different outside

// takes i signed, returns a number in [0,w)
static inline SINTa ModuloSignedToPositive(SINTa i,SINTa w)
{
	RR_ASSERT( w > 0 );

	SINTa mod_signed = i % w;
	return mod_signed + ( (mod_signed < 0) ? w : 0 );
}

static inline SINTa MirrorIndex_Alt1(SINTa i,SINTa w)
{
	i = ModuloSignedToPositive(i,2*w);
	
	SINTa rev = 2*w-1 - i;

	// use "i" in [0,w)
	// use "rev" in [w,2w)
	// that's the same as taking the min :

	return RR_MIN(i,rev);
}

static inline SINTa MirrorIndex_Alt2(SINTa i,SINTa w)
{
	if( (UINTa)i < (UINTa)w ) return i; // i in [0,w)

	i = ModuloSignedToPositive(i,w);
	// i is in [0,w-1)
	// reverse :
	i = w-1 - i;
...
struct StripInfo
{
	...
	SINTa strip_x0;
	SINTa strip_interior_x0;
	SINTa strip_interior_x1;
	SINTa strip_x1;
	SINTa image_w;

	void init(SINTa strip_x, SINTa strip_w, SINTa image_w, int filter_width);
};

void StripInfo::init(SINTa x, SINTa w, SINTa in_image_w, int filter_width)
{
	...
	constexpr SINTa kMinInteriorWidth = 8;
	...
	CpuDispatchFlags dispatch)
{
	...
	const SINTa strip_x0 = info.strip_x0;
	const SINTa strip_x1 = info.strip_x1;
	const SINTa interior_x0 = info.strip_interior_x0;
	const SINTa interior_x1 = info.strip_interior_x1;
	const SINTa w = info.image_w;

	// Left border
	for (SINTa x = strip_x0; x < interior_x0; x++)
	{
		t_color accum;
	...
	// Right border
	for (SINTa x = interior_x1; x < strip_x1; x++)
	{
		t_color accum;
		accum = filter[0] * from_row[x];
		for (int i=1;i<filter_width;i++) // no i=0
...
void rrSurface_MakeGaussianBlurred_Sub_Strip(rrSurface * pTo,const rrSurface & from_F32,
	...
	int nchan,SINTa stripx,SINTa stripw,
	F32 * * window_rows,SINTa window_height,
	const F32 * filter,int filter_width,
	CpuDispatchFlags dispatch)
{
	SINTa w = from_F32.width;
	SINTa h = from_F32.height;
	int filter_extent = filter_width - 1; // filter, after mirroring, goes [-filter_extent,filter_extent]
	SINTa stripc = stripw * nchan;

	StripInfo strip_info;
	strip_info.init(stripx, stripw, w, filter_width);

	SINTa cur_window_y = 0; // cur_window_y tracks where current y maps to in the circular window
...
//	RR_ASSERT( rrPixelFormat_GetInfo(pf)->bytesPerPixel == sizeof(t_color) );
	...
	SINTa w = from_F32.width;
	SINTa stripw = STRIP_WIDTH_FLOATS / nchan;
	
	for(SINTa sx=0;sx < w;sx += stripw)
	{
		SINTa sxend = RR_MIN(w,sx+stripw);
		SINTa sw = sxend - sx;
