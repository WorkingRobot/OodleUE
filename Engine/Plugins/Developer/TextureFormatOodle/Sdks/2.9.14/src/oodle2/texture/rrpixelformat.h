...
typedef enum e_rrPixelFormat
{
	...
	rrPixelFormat_1_U8 = RR_MAKE_PIXELFORMAT_SIMPLE(1,RR_PIXELFORMAT_TYPE_U8) , // NOTE: reads as grayscale, other 1-channel formats read as red only
	...
	rrPixelFormat_R5G6B5   = RR_MAKE_PIXELFORMAT_BITPACK( 0 ),  // R in high bits, B in low bits
	rrPixelFormat_A1R5G5B5 = RR_MAKE_PIXELFORMAT_BITPACK( 1 ),  // ARGB bit order
	...
	rrPixelFormat_R8 = rrPixelFormat_1_U8, // NOTE: 1x U8 reads as grayscale (replicates R to RGB), other 1-channel formats read as red only
	rrPixelFormat_R8G8 = rrPixelFormat_2_U8, // 2_U8 is RG byte order unlike 3_U8 which is BG byte order
...
//SINTa rrPixelFormat_MakeStride_Aligned(rrPixelFormat format, SINTa w);
...
// should this format be interpretted as SRGB by default (eg. all 8 bit formats)
...
void rrPixelFormat_GetColors4I( const U8 * from, rrPixelFormat format, rrColor4I * color, SINTa count );
void rrPixelFormat_GetColors4F( const U8 * from, rrPixelFormat format, rrColor4F * color, SINTa count );

void rrPixelFormat_PutColors4I( U8 * to, rrPixelFormat format, const rrColor4I * color, SINTa count );
void rrPixelFormat_PutColors4F( U8 * to, rrPixelFormat format, const rrColor4F * color, SINTa count );
