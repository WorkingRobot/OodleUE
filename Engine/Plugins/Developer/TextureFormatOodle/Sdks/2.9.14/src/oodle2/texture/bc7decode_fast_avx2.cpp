...
{
	alignas(8) static U16 override_masks[2][4] =
	{
		{ 0, 0, 0, 0 },		// do nothing when !ignore_alpha
		{ 0, 0, 0, 255 },	// OR 255 over alpha (=set it to 255) when ignore_alpha
	};

	Vec256 override_alpha = _mm256_broadcastq_epi64(load64u(&override_masks[ignore_alpha]));
...
// Pack interpolated colors down to 8-bit pixels
//
// The strange ordering used by the "pack" instruction in here forces the rest of
// this module to work in a matching permuted order:
//   interp16[0] contains pixels 0,1,4,5
//   interp16[1] contains pixels 2,3,6,7
//   interp16[2] contains pixels 8,9,c,d
//   interp16[3] contains pixels a,b,e,f
static RADFORCEINLINE void avx2_pack_result(U8 * out_rgba, const Vec256 interp16[4])
{
	// Pack down to 8 bits and store
	Vec256 interp8_0 = _mm256_packus_epi16(interp16[0], interp16[1]);
	Vec256 interp8_1 = _mm256_packus_epi16(interp16[2], interp16[3]);
...
static RADFORCEINLINE void avx2_interpolate_one_subset_twoind(U8 * out_rgba, U32 crot_idxmode, const __m256i & lerpf8_256, const Vec128 &endpoints16_in, bool ignore_alpha)
{
	...
	// Apply to endpoints
	// NOTE: top 128b undefined but unused!
	...
	//
	// NOTE: the indices here advance in an odd way because of the order forced by avx2_pack_result.
...
#undef IDXS
	...
	Vec256 interp16[4];
	interp16[0] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, diff16), lo16);
	interp16[1] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, diff16), lo16);
	interp16[2] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, diff16), lo16);
	interp16[3] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, diff16), lo16);

	// Pack down to 8 bits and store
	avx2_pack_result(out_rgba, interp16);
}
...
#undef FOUR
	...
	const Vec256 lerpf256 = broadcast128_256(lerpf8);
	...
	// Interpolate via rounding_shift_right(neg_factor * (lo - hi), 15) + lo
	Vec256 interp16[4];
	interp16[0] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, diff16_0), lo16_0);
	interp16[1] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, diff16_1), lo16_1);
	interp16[2] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, diff16_2), lo16_2);
	interp16[3] = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, diff16_3), lo16_3);
...
void avx2_decode_mode6(U8 * out_rgba, const U8 * block_bits, bool ignore_alpha)
{
	...
	// Set up for interpolation
	Vec256 diff16 = _mm256_sub_epi16(lo16, hi16); // yes, lo-hi!
	Vec256 lerpf256 = combine(lerpf8, lerpf8);
...
#undef INDS
	...
	// Diff is the same for all pixels in two-index modes, so can just load one
	// (base is different though because the pixels we're diffing against are folded in)
	const Vec256 diff = load256u(b->diff);
	...
	// Interpolate via rounding_shift_right(neg_factor * (lo - hi), 15) + lo
	// also computes diff to original block values which are subtracted from base
	Vec256 interp16_0 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, diff), load256u(b->base +  0));
	Vec256 interp16_1 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, diff), load256u(b->base + 16));
	Vec256 interp16_2 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, diff), load256u(b->base + 32));
	Vec256 interp16_3 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, diff), load256u(b->base + 48));
