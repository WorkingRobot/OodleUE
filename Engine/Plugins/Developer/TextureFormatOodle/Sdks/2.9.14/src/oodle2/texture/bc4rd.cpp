...
static RADFORCEINLINE Vec128_S32 simd_cached_lls_solve_initial(int * pABsum, const BC4BlockInfo & blki, const BC4IndexCache * ic, const VecF32x4 & clamp_lo, const VecF32x4 & clamp_hi)
{
	...
	*pABsum = _mm_cvtsi128_si32(_mm_add_epi32(ABsum2, shuffle32<2,3,0,1>(ABsum2)));
	...
	ep_flt = ep_flt.max(clamp_lo);
	ep_flt = ep_flt.min(clamp_hi);
	...
	Vec128_S16 weights16_0 = Vec128_U16(vmovl_low_u8 (weights8)).s16();
...
static RADFORCEINLINE void cached_lls_solve(U8 * pEp0, U8 * pEp1, const BC4BlockInfo & blki, const BC4IndexCache * ic, const BC4EndpointEncInfo & info, int mode)
{
	...
	int ep0_int = rr_round_to_int(RR_CLAMP(ep0, info.lo, info.hi));
	int ep1_int = rr_round_to_int(RR_CLAMP(ep1, info.lo, info.hi));
#endif
...
static void batch_cached_lls_solve_mode1(BC4EvalMode1Indices *items, int count, const BC4BlockInfo & blki, const BC4BlockEvalMode1Info & bmode1, const BC4EndpointEncInfo & info)
{
	...
	for (int i=0; i < count; ++i)
	{
		...
		degen_ep = degen_ep.min(bmode1.degen_constrain_hi).max(bmode1.degen_constrain_lo);
...
static void bc4rd_single_block_change_distortion_indirect_batch(S32 *distortions, const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr, const int * inds, int count)
{
	...
	for (int i=0; i < count; ++i)
	{
		...
		// Dot products with itself, and sum to 32-bit
		Vec128_S32 dots = vmull_low_s16(diff01_16, diff01_16);
		dots = vmlal_high_s16(dots, diff01_16, diff01_16);
		dots = vmlal_low_s16(dots, diff23_16, diff23_16);
		dots = vmlal_high_s16(dots, diff23_16, diff23_16);
#endif
...
static RADFORCEINLINE void AnyIndex8D64_add(AnyIndex8D64 *aid, const S16 *src_pixels, const S16 *palette)
{
	...
	for LOOP(c, 16)
	{
		...
		Vec128_U32 diff_lo = vabdl_low_s16 (pixel, pal);
		Vec128_U32 diff_hi = vabdl_high_s16(pixel, pal);

		// add squared differences into ssdx as 64-bit values
		uint64x2x4_t ssdx = vld1q_u64_x4(aid->ssdx[c]);
		ssdx.val[0] = vmlal_low_u32 (ssdx.val[0], diff_lo, diff_lo);
		ssdx.val[1] = vmlal_high_u32(ssdx.val[1], diff_lo, diff_lo);
		ssdx.val[2] = vmlal_low_u32 (ssdx.val[2], diff_hi, diff_hi);
		ssdx.val[3] = vmlal_high_u32(ssdx.val[3], diff_hi, diff_hi);
...
static RADNOINLINE void actually_shrink_vq_heap(vector<BC4IndexVqHeapEntry>& heap, SINTa target_limit)
{
	RR_ASSERT(target_limit <= (SINTa)heap.size());

	//rrprintf("@@ HEAP resample %lld -> %lld\n", (long long)heap.size(), (long long)target_limit);
	...
	stdsort(heap.data(), heap.data() + heap.size(), stdgreater_via_less<BC4IndexVqHeapEntry>());
	heap.resize(target_limit);
}

// if heap is at maximum size, shrink it to be half that size
static inline void test_shrink_vq_heap(vector<BC4IndexVqHeapEntry>& heap, SINTa limit)
{
	...
	const BC4Config & config)
{
	...
	int nb = check_value_cast<int>(bc4_blocks->count << group_shift);
	...
	const SINTa vq_heap_limit = RR_MAX(nb, 64) * 8;
	entries.reserve(nb);
	...
	for (int mode = 0; mode < 2; ++mode)
	{
		...
		for LOOPINT(i, num_large_count[mode])
		{
	...
	//rrprintf("@@ HEAP nb=%d nelem=%d ratio=%.2f\n", nb, heap.size32(), 1.0f * heap.size32() / nb);
	...
	const rrDXTCRD_Options & rdopts)
{
	...
	*/
	...
	bool preserve_extremes = (options & rrDXTCOptions_BC3457_PreserveExtremes) != 0;
	...
	int nblocks = check_value_cast<int>(from_blocks->count);
	...
	for LOOPINT(bi,nblocks)
	{
		...
		for LOOPINT(i,DIRECT_CANDIDATES + config.index_heap_size)
		{
			int pb = i;
