...
#if defined(__RADARM__) && defined(__RAD64__) && defined(_MSC_VER)
// As of this writing 2024-09-30, MSVC for ARM64 doesn't have float16_t
// and uses __int16 for its corresponding intrinsics.
typedef __int16 float16_t;
#endif
...
{
#ifdef DO_BUILD_AVX2
	if ( rrCPUx86_all_features_present( RRX86_CPU_AVX2 | RRX86_CPU_F16C ) )
	{
		internal::rrFP16_to_FP32_Array_AVX2(dst, src, count);
		return;
	}
#endif
...
void rrFP32_to_FP16_Array(rrFP16 * dst, const F32 * src, SINTa count)
{
	...
	if ( rrCPUx86_all_features_present( RRX86_CPU_AVX2 | RRX86_CPU_F16C ) )
	{
		internal::rrFP32_to_FP16_Array_AVX2(dst, src, count);
		return;
	}
#endif
