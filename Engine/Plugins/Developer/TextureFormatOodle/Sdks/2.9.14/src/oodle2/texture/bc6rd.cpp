	...
	const rrDXTCRD_Options & rd_options)
{
	...
	{
		U16 * block_ptr = (U16 *)BlockSurface_Seek(&from_blocks,bi);
		bc6enc_canonicalize_pixels(block_ptr, block_ptr, is_signed);
	}
	...
	for LOOPINT(bi,nblocks)
	{
		...
		// take past blocks and try substituting endpoints
		{
			...
			for LOOPINT(mtfwindowi,endpoint_window.count)
			{
		...
		// not sure whether this is any good or not, let's try it
		{
			...
			for LOOPINT(mtfwindowi,indices_window.count)
			{
