...
static RADFORCEINLINE Vec256 Double256(const Vec256 & v)
{
...
void AnyIndexD_batch_lookup_AVX2(S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count32)
{
	...
	//
	// Double the values since AnyIndexD_Lookup actually returns 2*SSD (somewhat annoyingly)
	Vec256 err01 = Double256(Load256_DeintEvenOdd(&aid->ssd[0][0])); // ssd[0][0], ssd[0][2], ssd[0][4], ssd[0][6] | ssd[0][1], ssd[0][3], ssd[0][5], ssd[0][7]
	Vec256 err23 = Double256(Load256_DeintEvenOdd(&aid->ssd[1][0])); // ssd[1][0], ssd[1][2], ssd[1][4], ssd[1][6] | ssd[1][1], ssd[1][3], ssd[1][5], ssd[1][7]
	Vec256 err45 = Double256(Load256_DeintEvenOdd(&aid->ssd[2][0]));
	Vec256 err67 = Double256(Load256_DeintEvenOdd(&aid->ssd[3][0]));
	Vec256 err89 = Double256(Load256_DeintEvenOdd(&aid->ssd[0][8]));
	Vec256 errab = Double256(Load256_DeintEvenOdd(&aid->ssd[1][8]));
	Vec256 errcd = Double256(Load256_DeintEvenOdd(&aid->ssd[2][8]));
	Vec256 erref = Double256(Load256_DeintEvenOdd(&aid->ssd[3][8]));
...
void compute_chunk_sads_indirect_AVX2(int chunk_sads[], int count32, const U16 indices[], const rrColorBlock4x4 * pair_colors, const rrColorBlock4x4 & rColors)
{
	...
	Vec128 m_active = _mm_cmpgt_epi32(_mm_set1_epi32(count32), m_lane_id);
	...
	for (;;) // Condition in the middle of the loop
	{
		...
		_mm_maskstore_epi32(chunk_sads + i, m_active, sum);
		...
		pInds = indices + i;
		m_active = _mm_cmpeq_epi32(m_active, m_active);
	}
}
...
int filter_by_index_sad_with_inds_AVX2(int positions[], U32 out_indices[], int count32, const U8 * index_batch, const U32 * in_indices, const U8 ref_inds_ptr[16], int sad_thresh)
{
	const Vec256 v_sad_thresh = _mm256_set1_epi32(sad_thresh);
	const Vec256 ref_inds = broadcast128_256(load128u(ref_inds_ptr));
	const SINTa count = count32;
	...
	{
		// Run the kernel
		const Vec256 sad32s = filter_by_index_sad_kernel_AVX2(ref_inds, index_batch + i * 16);

		// store IDs of lanes that have values below sad_thresh
		const Vec256 below = _mm256_cmpgt_epi32(v_sad_thresh, sad32s);
		const Vec256 original_inds = load256u(in_indices + i);
		U32 below_mask = _mm256_movemask_ps(_mm256_castsi256_ps(below));
		...
		Vec256 lane_ids = _mm256_and_si256(_mm256_srlv_epi32(table_val, c_lane_shifts), _mm256_set1_epi32(15));
		store256u(positions + out_count, _mm256_add_epi32(lane_ids, base_index));
		store256u(out_indices + out_count, _mm256_permutevar8x32_epi32(original_inds, lane_ids)); 
		out_count += _mm_popcnt_u32(below_mask);
	...
	for (; i < count; ++i)
	{
		...
		positions[out_count] = (int)i;
		out_indices[out_count] = in_indices[i];
		out_count += _mm_cvtsi128_si32(sum) < sad_thresh;
	}
