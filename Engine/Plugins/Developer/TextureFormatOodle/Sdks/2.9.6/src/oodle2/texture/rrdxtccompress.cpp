...
#define COV_PCA_MULT(cov,pca)										\
			{														\
				F32 b = cov[0]*pca.x + cov[1]*pca.y + cov[2]*pca.z;	\
				F32 g = cov[3]*pca.y + cov[4]*pca.z + cov[1]*pca.x;	\
				F32 r = cov[5]*pca.z + cov[2]*pca.x + cov[4]*pca.y;	\
				pca.x = b;											\
				pca.y = g;											\
				pca.z = r;											\
			}
...
#undef COV_PCA_MULT
	...
	rrVec3f delta = meansf[3] - meansf[0];
	meansf[1] = meansf[0] + delta * (1.f/3.f);
	meansf[2] = meansf[0] + delta * (2.f/3.f);
...
{
	Vec128_S32 sum = Vec128_S32::loadu(&data.sum); // Note(djg): This reads first 32-bits of data.loC
	sum.insert<3>(16*255);                         // Force average to be opaque
	...
	if ( flags & DXT1_4MEANS_PCA )
	{
		...
		for LOOP(i,16)
		{
			Vec128_S32 pixint = Vec128_U8::loadu_lo32(&colors.colors[i]).to_sint32_lo();
		...
		for(int iter=0;iter<PCA_POWER_ITERS/2;iter++)
		{
			...
			pca = (pca / VecF32x4r_sqrt(len_sq)) & len_positive;
		}
	}
	else
	{
		// just diagonal
		Vec128_S32 diag_s32 = Vec128_S32::loadu(&data.diagonal) & Vec128_S32(-1,-1,-1,0); // Zero out alpha channel
		VecF32x4 diag = diag_s32.to_f32();

		// Normalize result
		VecF32x4 len_sq = (diag * diag).sum_across();
		VecF32x4 len_positive = len_sq.cmp_gt(VecF32x4::zero());
		pca = (diag / VecF32x4r_sqrt(len_sq)) & len_positive;
	}

	// dot the colors in the PCA linear fit direction & seed 4-means
	VecF32x4 vMinMaxDot = VecF32x4::zero();
	for LOOP(i,16)
	{
		Vec128_S32 pixint = Vec128_U8::loadu_lo32(&colors.colors[i]).to_sint32_lo();
	...
	F32 minDot = vMinMaxDot.scalar_x();
	F32 maxDot = -vMinMaxDot.yyyy().scalar_x();

	// make 4 points staggered along the pca line :
	VecF32x4 meansf0 = avgF + pca * VecF32x4(0.75f * minDot);
	VecF32x4 meansf3 = avgF + pca * VecF32x4(0.75f * maxDot);
	VecF32x4 delta = meansf3 - meansf0;
	VecF32x4 meansf1 = meansf0 + delta * VecF32x4(1.f/3.f);
	VecF32x4 meansf2 = meansf0 + delta * VecF32x4(2.f/3.f);
	...
	Vec128_U8  means0123_u8 = means01_s16.to_uint8_sat(means23_s16);
