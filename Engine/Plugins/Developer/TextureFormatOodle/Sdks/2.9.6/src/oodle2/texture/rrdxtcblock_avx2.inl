...
static RADFORCEINLINE void UnpackBGRAto16_Weighted(Vec256 & out_br, Vec256 & out_ga, const Vec256 & pixels)
{
	...
	const Vec256 scale_ga = _mm256_set1_epi32(0x020001);

	out_br = _mm256_and_si256(pixels, mask_br);
	out_ga = _mm256_mullo_epi16(_mm256_srli_epi16(pixels, 8), scale_ga);
}
...
{
	const Vec256 mask_br = _mm256_set1_epi16(0xff);
	const Vec256 scale_ga = _mm256_set1_epi32(0x040002);

	out_br = _mm256_and_si256(pixels, mask_br);
	out_ga = _mm256_srli_epi16(pixels, 8);

	out_br = _mm256_add_epi16(out_br, out_br);
	out_ga = _mm256_mullo_epi16(out_ga, scale_ga);
}
