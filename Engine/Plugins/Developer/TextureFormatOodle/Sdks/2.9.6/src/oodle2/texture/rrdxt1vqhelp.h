...
struct FinalMatrixL1Index
{
	...
	RAD_ALIGN(U16, sorted_norms[CHUNK_SIZE + PADDING], 16); // norms of blocks, sorted in increasing order
	RAD_ALIGN(U16, sorted_norms_sub[CHUNK_SIZE / 16], 16); // subsampled norms, for some lookup strategies
...
S32 compute_sad_bound_for_ssd_threshold(S32 delta_ssd_per_pixel_per_sad[16][8], int num_steps_per_pixel[16], S32 base_ssd, S32 ssd_limit, S32 * out_ssd_limit=0);
...
void AnyIndexD_batch_lookup_AVX512(S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count);
...
int filter_by_index_sad_with_inds_AVX2(int remaining_positions[], U32 out_indices[], int count32, const U8 * index_batch, const U32 * in_indices, const U8 ref_inds_ptr[16], int threshold);
int filter_by_index_sad_with_inds_AVX512(int remaining_positions[], U32 out_indices[], int count32, const U8 * index_batch, const U32 * in_indices, const U8 ref_inds_ptr[16], int threshold);
