...
// PopCnt is not used currently
// putting this in Tex rather than Data because we can require POPCNT for Tex but not Data
//	needs CPUID check at Tex Enter
// currently only used for debug checks so just do it the C way
	
static RADINLINE U32 rrPopCnt32( U32 v )
{
	#if 0 // defined(_MSC_VER) && defined(__INTRIN_H_) && (_MSC_VER > 1400)
	return __popcnt(v);
	#else
	
	//return _mm_popcnt_u32(v); // nmmintrin.h , SSE4.2 or ABM

	// but not used anywhere speed is important so don't sweat it
...
static RADINLINE U64 rrPopCnt64( U64 v )
{
#ifdef __RAD64REGS__
	...
	//return _mm_popcnt_u64(v); // nmmintrin.h , SSE4.2 or ABM
	
	v = v - ((v >> 1) & 0x5555555555555555ULL);                    // reuse input as temporary
	v = (v & 0x3333333333333333ULL) + ((v >> 2) & 0x3333333333333333ULL);     // temp
	U64 c = (((v + (v >> 4)) & 0x0F0F0F0F0F0F0F0FULL) * 0x0101010101010101ULL) >> 56; // count
	return c;
	
	#endif
#else
	return rrPopCnt32( (U32)(v>>32) ) + rrPopCnt32( (U32)v );
#endif
}
...
static RADINLINE int bc7bits_popcnt(const bc7bits &rhs)
{
	U64 ret = rrPopCnt64( bc7bits_qw0(rhs) ) + rrPopCnt64( bc7bits_qw1(rhs) );
	return (int)ret;  
}
...
static RADINLINE int bc7bits_popcnt(const bc7bits &rhs)
{
	U64 ret = rrPopCnt64(rhs.qw[0]) + rrPopCnt64(rhs.qw[1]);
	return (int)ret;
}
...
{
	RR_ASSERT( bc7bits_popcnt( bc7bits_and(lhs,rhs) ) == 0 );
