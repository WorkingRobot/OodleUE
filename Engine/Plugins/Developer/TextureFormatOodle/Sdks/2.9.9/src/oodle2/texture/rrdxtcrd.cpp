	...
	void * jobify_user_ptr,int num_workers)
{
	...
	for ( int i = 0; i < num_surfaces; ++i )
	{
		...
		// Always run the job for the final surface blocking on this thread, we were about to wait for it anyway, or
		// rather issue a job that has a dependency on it, but it boils down to the same thing.
		//
		// In the (typical) case where num_surfaces == 1, this makes us not spawn a job for the activity surfs at all.
		int effective_num_workers = (i == num_surfaces - 1) ? OODLEJOB_DISABLE : num_workers;

		ctx->activity_handles[i] = OodleJob_Run_MaybeSingleThreaded(activity_job_func,&ctx->activity_contexts[i],NULL,0,jobify_user_ptr,effective_num_workers);
		if ( ctx->activity_handles[i] != 0 )
			any_jobs_spawned = true;
	}

	// If we didn't spawn any actual jobs, the work is already done,
	// can immediately finish up.
	if ( ! any_jobs_spawned )
	{
