...
static RADFORCEINLINE U8 calc_endpoints_and_pbits(BC7Color endpoints_q[2], const BC7SubsetState *st, BC7Flags flags, int pbits)
{
	...
	// Alpha overrides for "preserve extremes" blocks in modes witha lpha
	...
	Vec128_U16 endpoints16 = Vec128_U8::loadu_lo64(endpoints).to_uint16_lo();
	...
	if (Mode::epb || Mode::spb)
	{
		...
		if (pbits == -1)
		{
			...
			endpoints16 = vblend(dequant0, dequant1, merge_mask.u16());
		}
		else
		{
			// Just select the endpoints directly
			result = static_cast<U8>(pbits);
			endpoints16	= vblend(dequant0, dequant1, Vec128_U16::loada(g_pbit_merge_table[pbits]));
		}
	}
	else
	{
...
static BC7Error sse4_neon_calc_vector_inds_exact(BC7Inds *inds, const BC7SubsetInput *in, const BC7Color endpoints_q[2])
{
	...
	Vec128_S16 chan_weight16 = Vec128_S16::loadu_lo64_dup(in->channel_weights);
	...
	for (i = 0; i < in->num_pixels; i += 4)
	{
		...
		for (int j = 0; j < num_lerp; ++j)
		{
			...
			Vec128_S16 col = Vec128_S16::loadu_lo64_dup(&colors[j][0]);
		...
		// Emit best indices
		Vec128_U8 vinds = (best.s32() & Vec128_S32(0xf)).gather_xyzw_lo8();
...
#else // defined(DO_BUILD_NEON)
		int16x4_t errs_lo = vget_low_s16(masked_errs);
		int16x4_t errs_hi = vget_high_s16(masked_errs);
		sum_err = vmlal_s16(sum_err, errs_lo, errs_lo);
		sum_err = vmlal_s16(sum_err, errs_hi, errs_hi);
#endif
	}

	// Final error reduction and weighting
	return reduce_add(sum_err) * in->channel_weights[3];
}
...
static BC7Error sse4_neon_calc_scalar_inds_linear(U8 out_inds[16], const BC7SubsetInput *in, int ib, const BC7Color endpoints_q[2])
{
	...
	for (int i = 0; i < in->num_pixels; i += 8)
	{
		...
		total_err += ssd;
#else // defined(DO_BUILD_NEON64)
		int16x4_t err_lo = vget_low_s16(err16);
		int16x4_t err_hi = vget_high_s16(err16);
		total_err = vmlal_s16(total_err, err_lo, err_lo);
		total_err = vmlal_s16(total_err, err_hi, err_hi);
#endif
	}

	// reduce and apply weight!
	return reduce_add(total_err) * in->channel_weights[3];
}
...
static void log_subset_state(const BC7SubsetState *st, bool has_two_inds)
{
	...
	TRACE("  idxs[0]=");
	for LOOP(i,16)
		TRACE("%c", "0123456789abcdef"[st->idxs[0].ind[i]]);
	TRACE("\n");
	TRACE("  idxs[1]=");
	for LOOP(i,16)
		TRACE("%c", "0123456789abcdef"[st->idxs[1].ind[i]]);
	TRACE("\n");
}
...
static void scalar_calc_subset_pca3(float mean[4], float pca[4], const BC7SubsetInput *in)
{
	...
	cov[1] = cov[1]*sf - m[1]*m[1];
	cov[2] = cov[2]*sf - m[2]*m[2];
	cov[3] = cov[3]*sf - m[0]*m[1];
	cov[4] = cov[4]*sf - m[1]*m[2];
	cov[5] = cov[5]*sf - m[2]*m[0];
	...
	for(int iter = 0; iter < PCA_POWER_ITERS; ++iter)
	{
		...
		float y = cov[1]*local_pca[1] + cov[4]*local_pca[2] + cov[3]*local_pca[0];
		float z = cov[2]*local_pca[2] + cov[5]*local_pca[0] + cov[4]*local_pca[1];
		local_pca[0] = x;
		local_pca[1] = y;
		local_pca[2] = z;
	}
	...
	if_unlikely (len_sq == 0.0f)
	{
		// Our initial seed was in the nullspace of the covariance
		// matrix.  Switch to the longest diagonal between pixels in the
		// block and the mean color; this is guaranteed to at least be
		// something that makes sense for the block (we handled
		// single-color blocks during init) and is symmetric. In
		// particular it's not in the nullspace of the covariance
		// matrix.
		...
		for (int i = 0; i < in->num_pixels; ++i)
		{
			float d[3];
			for(int c = 0; c < 3; ++c)
				d[c] = in->pixels[i*4+c] - m[c];

			float len2 = (radtex_sqr(d[0]) + radtex_sqr(d[2])) + radtex_sqr(d[1]);
			if (len2 > len_sq)
			{
				for(int c = 0; c < 3; ++c)
					local_pca[c] = d[c];
		...
		// We can have zero length if rgb of the color are == mean (i.e. all identical),
		// which is a degenerate case.
		if (len_sq > 0.0f)
		{
			// Re-run the power iteration to find the eigenvector
			for(int iter = 0; iter < PCA_POWER_ITERS; ++iter)
			{
				// NOTE(fg): the vector versions store the matrix by diagonals and thus sum in a slightly
				// unconventional order; match the order of summation here to get the same results
			...
			len_sq = (radtex_sqr(local_pca[0]) + radtex_sqr(local_pca[2])) + radtex_sqr(local_pca[1]);
		}
		else
		{
			// Just pick _something_
			local_pca[0] = 1.0f;
			local_pca[1] = 0.0f;
			local_pca[2] = 0.0f;
			len_sq = 1.0f;
	...
	// Note(djg): len_sq is not 0.0, so there's no need to check for / 0.0.
...
static void scalar_calc_subset_pca4(float mean[4], float pca[4], const BC7SubsetInput *in)
{
	...
	if_unlikely (len_sq == 0.0f)
	{
		...
		for (int i = 0; i < in->num_pixels; ++i)
		{
			...
			float len2 = (radtex_sqr(d[0]) + radtex_sqr(d[2])) + (radtex_sqr(d[1]) + radtex_sqr(d[3]));
			if (len2 > len_sq)
			{
				for(int c = 0; c < 4; ++c)
					local_pca[c] = d[c];
		...
		// We can have zero length if all colors == mean (and thus each other),
		// which is a degenerate case.
		if (len_sq > 0.0f)
		{
			// Re-run the power iteration to find the eigenvector
			...
			len_sq = (radtex_sqr(local_pca[0]) + radtex_sqr(local_pca[2])) + (radtex_sqr(local_pca[1]) + radtex_sqr(local_pca[3]));
		}
		else
		{
			// Just pick _something_
...
static void sse4_neon_calc_subset_pca3(float mean[4], float pca[4], const BC7SubsetInput *in)
{
...
#if defined(DO_BUILD_SSE4)
	if_unlikely (_mm_ucomieq_ss(len_sq, VecF32x4::zero()))
#else // defined(DO_BUILD_NEON64)
	if_unlikely (len_sq.lane<0>() == 0.0f)
#endif
	{
		// Our initial seed was in the nullspace of the covariance
		// matrix.  Switch to the longest diagonal between pixels in the
		...
		{
			Vec128_S32 pixint = Vec128_U8::loadu_lo32(in->pixels + i*4).to_sint32_lo();
			VecF32x4 d = (pixint.to_f32() - m) & VecF32x4::bitmask(~0u, ~0u, ~0u, 0); // clear A diff

			VecF32x4 len2 = (d * d).sum_across();

			// if (len2 > len_sq)
#if defined(DO_BUILD_SSE4)
			if (_mm_ucomigt_ss(len2, len_sq))
#else // defined(DO_BUILD_NEON64)
			if (len2.lane<0>() > len_sq.lane<0>())
#endif
			{
				local_pca = d;
				len_sq = len2;
		...
		// We can have zero length if rgb of the color are == mean (i.e. all identical),
		// which is a degnerate case.
		if (len_sq.scalar_x() > 0.0f)
		{
			// Re-run the power iteration to find the eigenvector
			for(int iter = 0; iter < PCA_POWER_ITERS; ++iter)
				local_pca = local_pca * cov0 + local_pca.yzxw() * cov1 + local_pca.zxyw() * cov2;
			len_sq = (local_pca * local_pca).sum_across();
		}
		else
		{
			// Just pick _something_
			local_pca = VecF32x4(1.0f, 0.0f, 0.0f, 0.0f);
			len_sq = VecF32x4(1.f);
		}
	}

	// Note(djg): len_sq is not 0.0, so there's no need to check for / 0.0.
	VecF32x4 final_pca = local_pca / VecF32x4r_sqrt(len_sq);
	...
	RR_ASSERT(memcmp(ref_pca, &final_pca, 3*sizeof(float)) == 0);
	RR_ASSERT(final_pca.wwww().scalar_x() == 0.0f);
	#endif

	final_pca.storeu(pca);
}

static void sse4_neon_calc_subset_pca4(float mean[4], float pca[4], const BC7SubsetInput *in)
{
...
{
#if defined(DO_BUILD_SSE4) || defined(DO_BUILD_NEON64)
	sse4_neon_calc_subset_pca3(mean, pca, in);
#else
...
{
#if defined(DO_BUILD_SSE4) || defined(DO_BUILD_NEON64)
	sse4_neon_calc_subset_pca4(mean, pca, in);
#else
...
static void bc7enc_refine_subset_partition(BC7SubsetState *st, const BC7SubsetInput *in, const BC7EncOptions *opt, const BC7Prep * prep, int s)
{
	...
	subset_error_paranoia_check<Mode>(st, in, opt);
}
...
static BC7Error bc7enc_try_partition(const U8 *block, const BC7Prep *prep, BC7Results *results, const BC7EncOptions *opt, int p)
{
	...
	if (Mode::ns >= 2)
	{
		for (int s = 0; s < Mode::ns; ++s)
		{
			...
			opt_ep[s] = find_optimal_endpoints_for_constant_color(Mode::index, seed_info.bbox[s][0].v, enable_alpha, need_pbit_value);
			have_opt[s] = true;
		}
	}
...
template<typename Mode>
static void bc7enc_try_mode_change(const U8 *block, BC7BlockState *st, const BC7EncOptions *opt)
{
	// get subsets in linear order :
	BC7BlockInput block_in;
	block_in.init(block, radtex_subset_to_inds[Mode::ns-1][st->p], st->subsets[0].rbit);
	...
	{
		BC7SubsetState *sst = &st->subsets[s];
		int num_pixels = radtex_num_pixels_per_subset[Mode::ns][st->p][s];

		BC7SubsetInput in;
		in.init(block_in.pixels + total_pixels*4, num_pixels, alpha_chan_index(sst->rbit), opt->alpha_weight);
...
static BC7BlockState *bc7_enc_try_refine(const U8 * block, BC7Results &enc_results, const BC7EncOptions &opt, const BC7Prep * prep)
{
	...
	if(enc_results.best_err > 0)
	{
		...
		if(opt.narrow1)
		{
			...
			{
				TRACE("PCA refine to narrow1, candidate %d:\n",i);
...
void bc7_enc_options_set(BC7EncOptions * popt,rrDXTCLevel level, rrDXTCOptions options)
{
	...
	case rrDXTCLevel_Fast:
		...
		opt.flags |= BC7ENC_DEFER_MODE13SEL; // FG TEST
		opt.max_part = 9;
		opt.narrow0 = 12;
		opt.narrow1 = 6;
		opt.narrow2 = 1;
...
bool BC7_CompressBlock_Sub(U8 * output_bc7, const U8 * block, const BC7EncOptions * in_opt, const BC7Prep * in_prep, const BC7_Mode_ErrBias * mpeb)
{
	...
	// First evaluate possible modes to get viable candidates
	{
		...
		{
			if ( opt.flags & BC7ENC_DEFER_MODE13SEL )
			{
				// Logic with deferred mode 1/3 selection is pretty easy: if mode 1 is enabled, try that,
				// else try mode 3 if that's enabled. If both mode 1 and 3 are on, only try mode 1 here,
				// mode 3 we try from the initial candidates.
				if (opt.enable_mode[1])
					bc7enc_try_mode<BC7Mode<1> >(block, &prep, &enc_results, &opt);
				else if (opt.enable_mode[3])
					bc7enc_try_mode<BC7Mode<3> >(block, &prep, &enc_results, &opt);
			}
			else
			{
	...
	// If deferred mode 1/3 selection is chosen and both modes are enabled, select modes now!
	if ( ( opt.flags & BC7ENC_DEFER_MODE13SEL ) && ( opt.enable_mode[1] && opt.enable_mode[3] ) )
	{
		SIMPLEPROFILE_SCOPE(bc7enc_mode13_select);
		int orig_result_count = enc_results.used;

		// grow heap limit to max
		enc_results.heap_size = BC7Results::MAX_RESULTS;
		RR_ASSERT(orig_result_count < enc_results.heap_size);

		// Both modes 1 and 3 are active, we did the initial encode to mode 1 but we need to decide whether
		// we want to switch
		for LOOP(i,orig_result_count)
		{
			BC7BlockState *slot = enc_results.heap[i].slot;
			if (slot->mode == 1)
			{
				// We already scored mode 1, check whether switching to mode 3 helps.
				BC7BlockState mode3 = *slot;

				mode3.mode = 3;
				bc7enc_try_mode_change<BC7Mode<3> >(block, &mode3, &opt);

				// Add new elements
				// This doesn't shuffle elements around because we grew the heap size above;
				// but if we hit the heap size limit (which will also happen if the initial
				// heap was at max size already), stop, because iteration after this is wonky.
				enc_results.add_state(&mode3);
				if (enc_results.used == enc_results.heap_size)
					break;
			}
		}

		// Shrink back down to target candidate set size
		enc_results.shrink(opt.narrow0);
	}
