...
{
	rrColor32BGRA c;
	c.u.b = U8_check( (S32)lrintf(vec.x) );
	c.u.g = U8_check( (S32)lrintf(vec.y) );
	c.u.r = U8_check( (S32)lrintf(vec.z) );
	c.u.a = 0xFF;
	return c;
...
{
	rrColor32BGRA c;
	c.u.b = RR_CLAMP_U8( (S32)lrintf(vec.x) );
	c.u.g = RR_CLAMP_U8( (S32)lrintf(vec.y) );
	c.u.r = RR_CLAMP_U8( (S32)lrintf(vec.z) );
	c.u.a = 0xFF;
	return c;
}
...
static inline rrColor565Bits Vec3fToQuantized565_RN(const rrVec3f & vec)
{
	...
	rrColor565Bits ret = {};
	ret.u.b = RR_CLAMP( lrintf( scaleRB * vec.x ), 0, 31 );
	ret.u.g = RR_CLAMP( lrintf( scaleG  * vec.y ), 0, 63 );
	ret.u.r = RR_CLAMP( lrintf( scaleRB * vec.z ), 0, 31 );
	return ret;
}
