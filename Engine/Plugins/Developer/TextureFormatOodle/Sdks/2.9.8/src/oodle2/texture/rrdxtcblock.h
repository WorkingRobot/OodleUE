...
struct DXT1_FindErrorsContext
{
	...
	// Internal data
	RAD_ALIGN(S16, unpacked_pixels[64], 32); // used for SSE4/AVX2 versions: first 32 are B/R, last 32 are G/A
	RAD_ALIGN(rrColorBlock4x4, packed_pixels, 16); // always set
	...
	//
	// count must be <= COUNT_LIMIT.
	U32 find_best_palette(const rrColor32BGRA palettes[], int count) const;

	// Initializes the error context for the given pixels.
	void init(CpuDispatchFlags dispatch, const rrColorBlock4x4 & pixels);
};
...
static RADINLINE U32 rrColor32BGRA_DeltaSqrRGBA_1BT(const rrColor32BGRA & c1,const rrColor32BGRA & c2)
{
	...
	U32 adelta = (c1.u.a != c2.u.a) ? 4*255*255 : 0;
	U32 ret = rgb + adelta;
	return ret;
}
...
void DXT1_EvalPalettes_ARM_DP(const DXT1_FindErrorsContext * ctx, U32 out_ssds[], const rrColor32BGRA palette[4], int count);
...
void DXT1_EvalPalettes_AVX512(const DXT1_FindErrorsContext * ctx, U32 out_ssds[], const rrColor32BGRA palette[4], int count);
