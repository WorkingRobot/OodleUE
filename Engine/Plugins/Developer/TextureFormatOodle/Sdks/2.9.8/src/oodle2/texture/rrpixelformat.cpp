...
{
	const U8 * from = ptr;
	Vec128_U32 constant_a { 0, 0, 0, 0xff };
	Vec128_U32 constant_a_4x { 0, 0, 0, 0xffffffff };
	Vec128_U32 mask_8b { 0xff };
	int i;
	...
	for (i = 0; i < (count & ~3); i += 4)
	{
		...
		v = v.xxxw(); // broadcast value to RGB (leave A alone)
		v |= constant_a_4x; // fill alpha channel with 0xff
	...
	for (; i < count; ++i)
	{
		Vec128_U32 v = Vec128_U32::loadu_lo32(from + i);
		v = v.xxxw(); // broadcast value to RGB
		v |= constant_a;
		v.storeu(color + i);
	}
}
...
static inline void rrPixelFormat_GetColors4I_2U8(rrColor4I * color, S32 count, const U8 * ptr)
{
	...
	Vec128_U32 constant_a { 0, 0, 0, 0xff };
	Vec128_U32 constant_a_2x { 0, 0xff, 0, 0xff };
	int i;
	...
	for (i = 0; i < (count & ~1); i += 2)
	{
		...
		Vec128_U16 v16 = v8.to_uint16_lo();
		Vec128_U32 v32 = v16.to_uint32_lo();
	...
	if (i < count)
	{
		...
		Vec128_U8 v8 = Vec128_U8::loadu_lo32(from + i);
...
{
	const U16 * from = (const U16 *)ptr;
	Vec128_U32 constant_a { 0, 0, 0, 0xffff };
	Vec128_U32 mask_r { 0xffff, 0, 0, 0 };
	int i;
	...
	for (i = 0; i < (count & ~1); i += 2)
	{
		...
		((v & mask_r) | constant_a).storeu(color + i + 0);
