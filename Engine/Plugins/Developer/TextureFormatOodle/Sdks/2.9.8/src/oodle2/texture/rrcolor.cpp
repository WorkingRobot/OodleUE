...
void rrColor_LinearToSRGB(const rrColor4F* src, rrColor32BGRA* dst, int count)
{
	for (int i = 0; i < count; i++)
	{
		...
		const Vec128_S32 TabValBG = TabValB.unpack_lo(TabValG);
		const Vec128_S32 TabValsBGR = TabValBG.unpack_lo64(TabValR); // This leaves A=0, which suits us
		...
		const Vec128_S32 InterpolatedBGR = TabValsBGR.s16().madd(FinalMultiplier.s16()).srl<16>();
		...
		const Vec128_S32 FinalBGRA = InterpolatedBGR | FinalA;
