...
rrbool rrSurface_BlitNormalized(rrSurface * to, const rrSurface * from, rrRangeRemap remap)
{
	...
	{
		RR_ASSERT( from->pixelFormat == rrPixelFormat_4_F32 );
		RR_ASSERT( to->pixelFormat == rrPixelFormat_B8G8R8A8 || to->pixelFormat == rrPixelFormat_R8G8B8A8 );

		rrSurface_LinearToSRGB(to, from);
		return true;
	}
	else if (remap == rrRangeRemap_SRGBToLinear)
	{
		RR_ASSERT( from->pixelFormat == rrPixelFormat_R8G8B8A8 );
		RR_ASSERT( to->pixelFormat == rrPixelFormat_4_F32 );

		rrSurface_SRGBToLinear(to, from);
		return true;
	}
...
void rrSurface_LinearToSRGB(rrSurface* to, const rrSurface* from)
{
	RR_ASSERT(from->pixelFormat == rrPixelFormat_4_F32);
	RR_ASSERT(to->pixelFormat == rrPixelFormat_B8G8R8A8 || to->pixelFormat == rrPixelFormat_R8G8B8A8);
...
void rrSurface_SRGBToLinear(rrSurface* to, const rrSurface* from)
{
	RR_ASSERT(from->pixelFormat == rrPixelFormat_R8G8B8A8);
	RR_ASSERT(to->pixelFormat == rrPixelFormat_4_F32);
	...
	rrSurfaceRowCache fmRows;
	fmRows.Start(const_cast<rrSurface*>(from), rrPixelFormat_R8G8B8A8, 8, RR_SURFACE_ROW_CACHE_READ);
	...
	for (int y = 0; y < h; y++)
	{
		const rrColor32RGBA* fmRow = (rrColor32RGBA*)fmRows.GetRow(y);
		rrColor4F* toRow = toRows.GetRow_4F(y);
