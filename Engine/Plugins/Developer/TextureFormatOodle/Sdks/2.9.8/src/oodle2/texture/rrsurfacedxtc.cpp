...
bool BlockSurface_SetView_to_RGBA8_or_BGRA8(BlockSurface * to, const BlockSurface * from, bool RGBX_Ok, bool sRGB,
														rrPixelFormat format_to_change_to)
{
	...
	BlockSurface_AllocCopyOrSetViewIfFormatMatches_Normalized(to,from,format_to_change_to, sRGB ? rrRangeRemap_LinearToSRGB : rrRangeRemap_None);
...
rrbool rrSurfaceDXTC_CompressBC1(BlockSurface * to, const BlockSurface * from, rrDXTCLevel qualityLevel, rrDXTCOptions options )
{
	...
	bool sRGB = (options & rrDXTCOptions_LinearToSRGB) != 0;
	bool is_rgba = BlockSurface_SetView_to_RGBA8_or_BGRA8(&from_converted,from,rgbx_ok,sRGB,rrPixelFormat_B8G8R8A8);
...
rrbool rrSurfaceDXTC_CompressBC2(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxt1QualityLevel, rrDXTCOptions options )
{
	...
	BlockSurfaceObj from_converted;
	bool sRGB = (options & rrDXTCOptions_LinearToSRGB) != 0;
	bool is_rgba = BlockSurface_SetView_to_RGBA8_or_BGRA8(&from_converted,from,false,sRGB,rrPixelFormat_B8G8R8A8);
...
rrbool rrSurfaceDXTC_CompressBC4(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxtcQualityLevel, rrDXTCOptions options )
{
	rrbool is_signed;
	rrbool src_16bit;
	if (rrPixelFormat_IsFloat(from->pixelFormat))
	{
		is_signed = to->pixelFormat == rrPixelFormat_BC4S;
		src_16bit = true;
	}
	else
	{
		is_signed = rrPixelFormat_GetInfo(from->pixelFormat)->isSigned;
		src_16bit = rrPixelFormat_IsInt16(from->pixelFormat);
	}
...
rrbool rrSurfaceDXTC_CompressBC5(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxtcQualityLevel, rrDXTCOptions options )
{
	...
	if (rrPixelFormat_IsFloat(from->pixelFormat))
	{
		is_signed = to->pixelFormat == rrPixelFormat_BC5S;
		src_16bit = true;
	}
	else
	{
...
rrbool rrSurfaceDXTC_CompressBC7(BlockSurface * to, const BlockSurface * from, rrDXTCLevel level, rrDXTCOptions options)
{
	...
	bool rgbx_ok = true; // always allow RGBX, we'll KillAlpha on the block read if was_rgbx
	bool sRGB = (options & rrDXTCOptions_LinearToSRGB) != 0;
	bool is_rgba = BlockSurface_SetView_to_RGBA8_or_BGRA8(&from_converted,from,rgbx_ok,sRGB,rrPixelFormat_R8G8B8A8);
