...
static RADINLINE SINTa wh_to_num_blocks(SINTa w,SINTa h)
{
	SINTa nbx = (w+3)/4;
	SINTa nby = (h+3)/4;
	return nbx*nby;
...
static RADINLINE U8 * BlockSurface_Seek(BlockSurface * bs,SINTa bi)
{
...
static RADINLINE const U8 * BlockSurface_SeekC(const BlockSurface * bs,SINTa bi)
{
	RR_ASSERT( bi >= 0 && bi < bs->count );
...
void BlockSurface_Alloc(BlockSurface * bs,SINTa num_blocks, rrPixelFormat format);

void BlockSurface_AllocCopy(BlockSurface * bs,const BlockSurface * from);

// Swap the internals of two BlockSurfaces
...
// Avoids a copy and just uses a view on NOP conversions
...
// Doesn't make sense to view as an image, the pixels are (in general) in the
// wrong order when viewed this way.
//
// origin is the index of the first block in the view;
// count is the number of blocks in the view, or -1 for "until the end"
//
// NOTE that if the block surface has too many blocks to fit within the
// size limits of a rrSurface, this will assert! If in doubt, always chop
// into pieces with possibly-huge BlockSurfaces.
void BlockSurface_Set_RRS_View(rrSurface * surf,const BlockSurface * bs,SINTa origin=0,SINTa count=-1);
...
void BlockSurface_Copy_to_RRS_SameFormat_Tiled(rrSurface* to_surfaces,int num_surfaces,const BlockSurface * from,SINTa tile_width,SINTa tile_height);
...
// For universal tiling.
void BlockSurface_AllocCopy_from_RRS_Tiled(BlockSurface* to,const rrSurface * from,int num_surfaces,SINTa tile_width,SINTa tile_height);
...
// Get a simple hash of the contents of a BlockSurface for debugging.
// Not intended to be particularly strong.
