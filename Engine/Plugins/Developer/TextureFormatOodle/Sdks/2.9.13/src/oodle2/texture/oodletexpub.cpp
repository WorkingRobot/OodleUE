	...
	int num_job_threads,void * jobify_user_ptr)
{
		...
		num_blocks >= RDO_MIN_BLOCK_COUNT )
	{
		...
		*/
		...
		if ( err == OodleTex_Err_OK &&
			! rrDXTCRD_Encode_RDO(&encode_to_surf,&from_blocks,from_rrs.data(),(int)num_from_surfaces,use_layout,rdo_lagrange_lambda,dxtcoptions,jobify_user_ptr,num_job_threads,dxtc_metric,rdopts) )
		{
