...
void BlockSurface_SetView(BlockSurface * bs, const BlockSurface * from, SINTa start, SINTa count )
{
	BlockSurface_Init(bs);

	RR_ASSERT( 0 <= start && start <= from->count );
	RR_ASSERT( 0 <= count && count <= from->count - start );
...
void BlockSurface_SetView(BlockSurface * bs, void * blocks, SINTa count, rrPixelFormat pf )
{	
	BlockSurface_Init(bs);
...
void BlockSurface_Swap(BlockSurface * a, BlockSurface * b)
{
	swap(a->blocks, b->blocks);
	swap(a->freeData, b->freeData);
	swap(a->count, b->count);
	swap(a->blockSizeBytes, b->blockSizeBytes);
	swap(a->pixelFormat, b->pixelFormat);
}
...
void BlockSurface_Alloc(BlockSurface * bs,SINTa num_blocks, rrPixelFormat pixelFormat)
{
...
{
	THREADPROFILESCOPE_FILTERED("BlockSurfaceCopy", from->count > 65536); // don't log small blits

	RR_ASSERT_ALWAYS( to_array[0].pixelFormat == from->pixelFormat );
	
	SINTa to_block_count = TotalBlockCount(to_array,num_surfaces);
...
void BlockSurface_Copy_to_RRS_SameFormat_Layout(rrSurface* to_surfaces,int num_surfaces,const BlockSurface * from,const OodleTex_Layout * layout)
{
			...
			if ( to_x+4 > to->width || to_y+4 > to->height )
			{
				for LOOP(by,4)
				{
					for LOOP(bx,4)
					{
						...
						SINTa x = RR_MIN(to_x+bx,to->width-1);
						SINTa y = RR_MIN(to_y+by,to->height-1);
	...
	else
	{
		...
		SINTa bypb = from->blockSizeBytes;
...
SINTa TotalBlockCount(const rrSurface * from,int num_surfaces)
{
	SINTa block_count = 0;
	for LOOP(i,num_surfaces)
	{
...
SINTa TotalTileCount(const rrSurface * from,int num_surfaces,SINTa tile_w,SINTa tile_h)
{
	SINTa tile_count = 0;
	for LOOP(i,num_surfaces)
	{
		SINTa w_tiles = (from[i].width + tile_w-1) / tile_w;
		SINTa h_tiles = (from[i].height + tile_h-1) / tile_h;
...
{
	SINTa block_count = TotalBlockCount(from_array,num_surfaces);
	THREADPROFILESCOPE_FILTERED("BlockSurfaceCopy", block_count > 65536); // don't log small blits
	...
	if ( rrPixelFormat_IsBlockCompressed(from_array->pixelFormat) )
	{
		...
		for LOOP(i,num_surfaces)
		{
			...
			// assumes from stride is tight packed :
			//	@@ or copy row by row
			SINTa nbx = (from_array[i].width + 3)/4;
			RR_ASSERT_ALWAYS( from_array[i].stride == nbx * to->blockSizeBytes );

			SINTa cur_num_blocks = wh_to_num_blocks(from_array[i].width,from_array[i].height);
	...
	else
	{
		...
		for LOOP(i,num_surfaces)
		{
			...
			// work on rows of blocks :
			for(SINTa y=0;y<from->height;y+=4)
			{
...
void BlockSurface_AllocCopy_from_RRS_Tiled(BlockSurface* to,const rrSurface * from_array,int num_surfaces,SINTa tile_width,SINTa tile_height)
{
	...
	const SINTa tile_width_blocks = tile_width / BLOCK_SIZE;
	const SINTa tile_height_blocks = tile_height / BLOCK_SIZE;
	const SINTa tile_count = TotalTileCount(from_array,num_surfaces,tile_width,tile_height);
	const SINTa blocks_per_tile = tile_width_blocks * tile_height_blocks;
	const SINTa block_count = blocks_per_tile * tile_count;
	...
	if ( rrPixelFormat_IsBlockCompressed(from_array->pixelFormat) )
	{
		...
		for LOOP(i,num_surfaces)
		{
			...
			// Traverse over tiles
			for (SINTa ty = 0; ty < from->height; ty += tile_height)
			{
				SINTa this_tile_height = RR_MIN(tile_height, from->height - ty);

				for (SINTa tx = 0; tx < from->width; tx += tile_width)
				{
					// Copy row by row within tiles
					const SINTa this_tile_width = RR_MIN(tile_width, from->width - tx);
					const SINTa this_tile_width_blocks = (this_tile_width + BLOCK_SIZE-1) / BLOCK_SIZE;
					...
					if (y < tile_height)
					{
						SINTa bottom_padding_height_blocks = (tile_height - y) / BLOCK_SIZE;
	...
	else
	{
		...
		for LOOP(i,num_surfaces)
		{
			...
			const SINTa width_in_blocks = (from->width + BLOCK_SIZE-1) / BLOCK_SIZE;
			const SINTa width_in_tiles = (from->width + tile_width-1) / tile_width;

			// Horizontal padding amount in blocks to make width be a multiple of the tile size
			const SINTa horiz_padding_in_blocks = width_in_tiles * tile_width_blocks - width_in_blocks;
			...
			for (SINTa ty = 0; ty < from->height; ty += tile_height)
			{
				...
				for(; ity < this_tile_height; ity += BLOCK_SIZE)
				{
					...
					SINTa cols_left = tile_width_blocks;
				...
				if (ity < tile_height)
				{
					SINTa bottom_padding_height_blocks = (tile_height - ity) / BLOCK_SIZE;
		...
		U8 * toEnd = to->blocks + (SINTa)to->count * to->blockSizeBytes;
		RR_ASSERT_ALWAYS( toPtr == toEnd );
	}
}
...
void BlockSurface_AllocCopy_from_RRS_Layout(BlockSurface* to,const rrSurface * from_surfaces,int num_surfaces,const OodleTex_Layout * layout)
{
	...
	THREADPROFILESCOPE_FILTERED("BlockSurfaceCopy", layout->m_nblocks > 65536); // don't log small blits
	RR_ASSERT( layout->m_surfaces.size32() == num_surfaces );

	BlockSurface_Alloc(to,layout->m_nblocks,from_surfaces->pixelFormat);
		...
		for (SINTa bi = 0; bi < layout->m_nblocks; ++bi)
		{
			...
			if ( fm_x+4 > from->width || fm_y+4 > from->height )
			{
				for LOOP(by,4)
				{
					...
					{
						// clamp coordinate, repeat boundary:
						SINTa x = RR_MIN(fm_x+bx,from->width-1);
						SINTa y = RR_MIN(fm_y+by,from->height-1);
...
void BlockSurface_Copy_Detile(BlockSurface * to_bs,const BlockSurface * fm_bs,const rrSurface * dim_surfaces,int num_surfaces,SINTa tile_width,SINTa tile_height)
{
	THREADPROFILESCOPE_FILTERED("BlockSurfaceCopy", to_bs->count > 65536); // don't log small blits

	const SINTa BLOCK_SIZE = 4;

	// Tile sizes must be a multiple of 4x4 pixels for this to work correctly
	RR_ASSERT( tile_width > 0 && tile_width % BLOCK_SIZE == 0 );
	RR_ASSERT( tile_height > 0 && tile_height % BLOCK_SIZE == 0 );

	RR_DURING_ASSERT( SINTa block_count = TotalBlockCount(dim_surfaces,num_surfaces) );
	...
	const SINTa tile_width_blocks = tile_width / BLOCK_SIZE;
	RR_DURING_ASSERT( const SINTa tile_height_blocks = tile_height / BLOCK_SIZE );

	const SINTa tile_row_stride = tile_width_blocks * (SINTa)fm_bs->blockSizeBytes;
	RR_DURING_ASSERT( const SINTa tile_size_bytes = tile_row_stride * tile_height_blocks );
	...
	if ( rrPixelFormat_IsBlockCompressed(fm_bs->pixelFormat) )
	{
		...
		for LOOP(i,num_surfaces)
		{
			...
			const SINTa width_in_blocks = (surf->width + BLOCK_SIZE-1) / BLOCK_SIZE;
			const SINTa height_in_blocks = (surf->height + BLOCK_SIZE-1) / BLOCK_SIZE;

			RR_DURING_ASSERT( const SINTa width_in_tiles = (surf->width + tile_width-1) / tile_width );
			RR_DURING_ASSERT( const SINTa height_in_tiles = (surf->height + tile_height-1) / tile_height );

			// Check output buffer sizes
			const SINTa nblocks = width_in_blocks * height_in_blocks;
			RR_DURING_ASSERT( const SINTa ntiles = width_in_tiles * height_in_tiles );
			...
			for (SINTa ty = 0; ty < surf->height; ty += tile_height)
			{
				SINTa this_tile_height = RR_MIN(tile_height, surf->height - ty);

				for (SINTa tx = 0; tx < surf->width; tx += tile_width)
				{
					// Copy row by row within tiles
					const SINTa this_tile_width = RR_MIN(tile_width, surf->width - tx);
					const SINTa this_tile_width_blocks = (this_tile_width + BLOCK_SIZE-1) / BLOCK_SIZE;
...
U64 BlockSurface_DebugHash(const BlockSurface * from)
{
	return simple_debug_hash(from->blocks, BlockSurface_GetDataSizeBytes(from));
}
