...
void rrSurface_UnregisterAllFastBlitters_ForTestCodeOnly()
{
	memset(s_fastBlitters, 0, sizeof(s_fastBlitters));
}
...
rrbool rrSurface_Blit_NonNormalized( rrSurface * to, const rrSurface * from)
{
	...
	{	
		...
		THREADPROFILESCOPE_FILTERED("Blit",to->height > 8 && to->width * to->height > 65536);  // not for RowCache, nor for small blits
...
rrbool rrSurface_BlitRect_NonNormalized( rrSurface * to, const rrSurface * from,
	...
	if ( fmBpp != 0 && toBpp != 0 )
	{
		...
		for LOOP(y,h)
		{
			...
			if ( fmInfo->isFloat || toInfo->isFloat )
			{
				...
				for (SINTa x = 0; x < w; x += CHUNK_SIZE)
				{
					SINTa count = RR_MIN(w - x, CHUNK_SIZE);
...
void rrSurface_Blit_SameFormat( rrSurface * to, const rrSurface * fm )
{
	...
	SINTa blocksPerRow = (w + info->blockSize - 1 ) / info->blockSize;
	SINTa bytesPerBlockRow = blocksPerRow * info->bytesPerBlock;
	
	if ( info->blockSize == 4 )
	{
		for(SINTa y=0;y<h;y += 4)
		{
	...
	else
	{
		...
		for(SINTa y=0;y<h;y += info->blockSize)
		{
			const U8 * fmPtr = rrPixelFormat_Seek(fm->pixelFormat, fm->data, fm->stride, 0, y );
			U8 * toPtr = rrPixelFormat_Seek(to->pixelFormat, to->data, to->stride, 0, y );
...
void rrSurface_GetColor4I( const rrSurface * fm, rrColor4I * color, SINTa x, SINTa y)
{
...
void rrSurface_GetColor4F( const rrSurface * fm, rrColor4F * color, SINTa x, SINTa y)
{
	const U8 * fmPtr = rrPixelFormat_Seek(fm->pixelFormat, fm->data, fm->stride, x, y );
	if ( fmPtr == NULL )
	{
...
void rrSurface_PutColor4I( rrSurface * surf, const rrColor4I * color, SINTa x, SINTa y)
{
...
void rrSurface_PutColor4F( rrSurface * surf, const rrColor4F * color, SINTa x, SINTa y)
{
	U8 * ptr = rrPixelFormat_Seek(surf->pixelFormat, surf->data, surf->stride, x, y );
	if ( ptr == NULL )
	{
