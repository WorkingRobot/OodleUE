...
struct SubsetCovarCalc1Subset
{
	static constexpr int NPIXELS = 16;
	using VecInt = typename T::VecInt;
	using VecUInt16 = typename T::VecUInt16;
	...
	void compute(VecInt sumi[14], const U8 pixels[], const S8 * shuffles)
	{
		...
		for (SINTa i = 0; i < NPIXELS; ++i)
		{
			...
			// These are 8-bit values, and we do unsigned 16-bit muls instead of 32-bit
			// since they give the same results in this case and are often faster.
			VecUInt16 r = (shuffled & c255).u16();
			VecUInt16 g = (shuffled.template srl<8>() & c255).u16();
			VecUInt16 b = (shuffled.template srl<16>() & c255).u16();
			VecUInt16 a = (shuffled.template srl<24>()).u16();
