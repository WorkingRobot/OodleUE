...
F32 VQD_BC1(CpuDispatchFlags dispatch, const rrColorBlock4x4 & colors,const rrColor32BGRA palette[4],const U32 in_indices,const SingleFloatBlock4x4 & activity)
{
	...
	for (int r = 0; r < 4; r++)
	{
		...
		#if 1 // activity 1F
		accum_f32 += VQD_Vec128_Diff_4U8_Activity1F(src_pixels, pal_pixels, activity.values + r*4);
		#endif

		#if 0 // activity 4F
		accum_f32 += VQD_Vec128_Diff_4U8_Activity4F(src_pixels, pal_pixels, activity.values + r*4);
		#endif
	}
...
F32 VQD(const rrColorBlock4x4 & colors1,const rrColorBlock4x4 & colors2,const FourFloatBlock4x4 & activity)
{
	...
	for(int r=0;r<4;r++)
	{
		...
		accum_f32 += VQD_Vec128_Diff_4U8_Activity4F(v1, v2, activity.values + r*4);
	}
...
F32 VQD(const rrColorBlock4x4 & colors1,const rrColorBlock4x4 & colors2,const SingleFloatBlock4x4 & activity)
{
	...
	if (rrCPUx86_feature_present(RRX86_CPU_AVX2))
		return internal::VQD_AVX2(colors1,colors2,activity);
#endif
	...
	for(int r=0;r<4;r++)
	{
		...
		accum_f32 += VQD_Vec128_Diff_4U8_Activity1F(v1, v2, activity.values + r*4);
	}
