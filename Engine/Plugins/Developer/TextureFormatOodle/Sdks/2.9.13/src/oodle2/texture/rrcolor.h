...
{
	U16	w;
	// a is the highest bit , ARGB high to low
	struct
	{
		U16 b : 5;
		U16 g : 5;
...
static RADINLINE S32 rrColor_Single_FtoI(float x)
{
	// NaNs -> 0 (D3D rules)
	if (x != x)
	{
	...
	// Overflow cases
	if (fabsf(x) >= 2147483648.0f) // 2^31
	{
		// Saturate at INT_MAX / INT_MIN, respectively.
		return (x > 0.0f) ? 2147483647 : (-2147483647 - 1);
	}

	// Regular cases
	return rr_round_to_int(x);
}

RADINLINE void rrColor4_F_to_I_Linear(rrColor4I * to, const rrColor4F * from)
{
	to->r =	rrColor_Single_FtoI( from->r );
	to->g =	rrColor_Single_FtoI( from->g );
	to->b =	rrColor_Single_FtoI( from->b );
	to->a =	rrColor_Single_FtoI( from->a );
}
