...
static RADINLINE const SingleFloatBlock4x4 * BlockSurface_SeekC_SingleFloatBlock4x4(const BlockSurface * bs,SINTa bi)
{
	RR_ASSERT( bs->pixelFormat == rrPixelFormat_1_F32 );
	return (const SingleFloatBlock4x4 *)BlockSurface_SeekC(bs,bi);
}

static RADINLINE const FourFloatBlock4x4 * BlockSurface_SeekC_FourFloatBlock4x4(const BlockSurface * bs,SINTa bi)
{
...
// dest_channel_count=4 means dest format cares about all of RGBA
// dest_channel_count=1 or 2 respecitvely means we care about R or RG only
// pTo must already be allocated to the correct size and pixel format
void make_activity_mask_1or4F(rrSurface * pTo,const rrSurface & source, EMakeActivityMode mam, rrDXTCOptions options, int dest_channel_count=4);

// version for HDR formats which needs to deal with huge contrast ratios
// pTo must already be allocated to the correct size and pixel format
void make_activity_mask_hdr_1F(rrSurface * pTo,const rrSurface & source, rrDXTCOptions options);
