...
{
	bc7_decode_block_fast(decomp_block,bc7bits);
	...
	if ( flags & BC7ENC_IGNORE_ALPHA )
	{
		...
		{
			decomp_block[i] = 255; // this is done by bc7enc_decode_state (but not by bc7_decode_block)
		}
	}
}
	...
	const BC7_RD_Config & config)
{
	...
	const int matrix_chunk_size = (16 * 1024) / sizeof(matrix_decoded[0]);
	int chunk_sads[matrix_chunk_size + FIND_NEXT_BELOW_PADDING];
	U32 chunk_sorted_norm_inds[matrix_chunk_size];
	U16 chunk_sorted_norms[matrix_chunk_size];
	U16 chunk_sorted_inds[matrix_chunk_size];
	...
	for (int matrix_chunk_begin = 0; matrix_chunk_begin < matrix_size; matrix_chunk_begin += matrix_chunk_size)
	{
		...
		for LOOP(i,matrix_chunk_len)
			chunk_sorted_norm_inds[i] = (ColorBlock4x4_ComputeSAD_RGBA(zero_colors,matrix_decoded[matrix_chunk_begin+i]) << 16) | i;

		stdsort(chunk_sorted_norm_inds, chunk_sorted_norm_inds + matrix_chunk_len);
		...
		for LOOP(bi,nb)
		{
			...
			int ref_norm = ColorBlock4x4_ComputeSAD_RGBA(zero_colors,infos[bi].colors);
			...
			const SINTa l = branchless_lower_bound_idx(chunk_sorted_norms, matrix_chunk_len, static_cast<U16>(RR_MAX(ref_norm - (S32) st->must_beat_sad + 1, 0)));
			const SINTa r = branchless_lower_bound_idx(chunk_sorted_norms, matrix_chunk_len, static_cast<U16>(RR_MIN(ref_norm + (S32) st->must_beat_sad, 65535))); // NOTE exclusive!
			...
			compute_chunk_sads_indirect(chunk_sads, culled_len, culled_inds, &matrix_decoded[matrix_chunk_begin], infos[bi].colors);
