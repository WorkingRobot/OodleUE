...
static RADFORCEINLINE F32 VQD_BC1(const rrColorBlock4x4 & colors,const rrDXT1Block & dxtb,const SingleFloatBlock4x4 & activity,rrDXT1PaletteMode pal_mode)
{
	rrColor32BGRA palette[4];
	DXT1_ComputePalette(dxtb.c0,dxtb.c1,palette,pal_mode);
	
	return VQD_BC1(colors,palette,dxtb.indices,activity);
}
	...
	const rrDXT1_VQ_Config & config)
{
	...
	*/
	...
	U32 chunk_sorted_norm_inds[pair_chunk_size];
	U16 chunk_sorted_norms[pair_chunk_size];
	U16 chunk_sorted_inds[pair_chunk_size];
	...
	for (int pair_chunk_begin = 0; pair_chunk_begin < num_pairs; pair_chunk_begin += pair_chunk_size)
	{
		...
		// Prepare sorted (norm,index) pairs
		for LOOP(i,pair_chunk_len)
			chunk_sorted_norm_inds[i] = (ColorBlock4x4_ComputeSAD_RGBA(zero_colors,pair_colors[pair_chunk_begin+i]) << 16) | i;

		stdsort(chunk_sorted_norm_inds, chunk_sorted_norm_inds + pair_chunk_len);

		// Post-sorting, split into the two arrays we actually use in the hot loop
		for LOOP(i,pair_chunk_len)
		{
			chunk_sorted_norms[i] = (U16)(chunk_sorted_norm_inds[i] >> 16);
			chunk_sorted_inds[i] = (U16)(chunk_sorted_norm_inds[i] & 0xffff);
		}
		...
		for LOOPVEC(block_i,blocks)
		{
			...
			int ref_norm = ColorBlock4x4_ComputeSAD_RGBA(zero_colors,pblock->colors);
			...
			const SINTa l = branchless_lower_bound_idx(chunk_sorted_norms, pair_chunk_len, static_cast<U16>(RR_MAX(ref_norm - (S32)must_beat_sad + 1, 0)));
			const SINTa r = branchless_lower_bound_idx(chunk_sorted_norms, pair_chunk_len, static_cast<U16>(RR_MIN(ref_norm + (S32)must_beat_sad, 65535))); // NOTE exclusive!
			const int culled_len = (int)(r - l);
			const U16 * culled_inds = chunk_sorted_inds + l;
			...
			// any decisions
			compute_chunk_sads_indirect(chunk_sads, culled_len, culled_inds, &pair_colors[pair_chunk_begin], pblock->colors);
			...
			for (int i = 0; ; i++) // yes, no loop condition!
			{
				...
				F32 vqd = VQD(pblock->colors,pair_colors[pair_i],pblock->activity);
	...
	const rrDXTCRD_Options & rdopts)
{
	...
	// read baseline encoding to seed :
	{
		...
		for LOOP(bi,nblocks)
		{
			...
			pblock->baseline_vqd = VQD_BC1(colors,block,pblock->activity,pal_mode);
	...
	for LOOP(vq_iterations,config.num_vq_iterations)
	{
		...
		const S32 ssd_max_increase_early_out = config.early_out_endpoints_ssd_max_increase_scaled_by_lambda*lambda;
		...
		for LOOPVEC(i,blocks)
		{
			...
			const U32 bbox_min_d = config.early_out_color_endpoint_distance*lambda;
			...
			for (int filter_i = 0; ; filter_i++) // yes, no loop condition!
			{
				...
				// again
				U32 solved_ssd;
				U32 new_indices = DXT1_FindIndices(colors,vqendpoint.palette,&solved_ssd);
				...
				*/
				...
				F32 vqd = VQD_BC1(colors,vqendpoint.palette,new_indices,pblock->activity);
				...
				if ( vq_iterations > 0 ) // second iter only
				{
					...
					if ( cur_indices != new_indices )
					{
						...
						vqd = VQD_BC1(colors,vqendpoint.palette,cur_indices,pblock->activity);
			...
			if ( pblock->cur != pblock->baseline )
			{
				...
				// not up to date :
				pblock->cur_vqd = VQD_BC1(pblock->colors,pblock->cur,pblock->activity,pal_mode);
		...
		for LOOPVEC(block_i,blocks)
		{
			...
			batch_compute_VQDs(index_vqd_results.data(),
				vqindices.data(),c_unit_index_map,index_vq_palette_num_to_search,
				colors,palettes[0],palettes[1],
				pblock->activity);
			batch_compute_VQDs(index_vqd_results.data() + index_vq_palette_num_to_search*2,
			...
			for (int scan_i = 0; ; scan_i++) // no loop condition, handled below!
			{
				...
				F32 J = vq_J(vqd,vqp_indexes_codelen,try_ep_codelen[ep_i],lambda);
			...
			pblock->cur_vqd = VQD_BC1(pblock->colors,pblock->cur,pblock->activity,pal_mode);
			#else
			RR_ASSERT( pblock->cur_vqd == VQD_BC1(pblock->colors,pblock->cur,pblock->activity,pal_mode) );
			#endif
			...
			*/
			...
			{
				F32 J = vq_J(pblock->cur_vqd,cur_indices_codelen,try_ep_codelen[0],lambda);
				...
				if ( try_ep[0] != try_ep[1] ) // equality is very rare
				{
					...
					F32 alt_vqd = VQD_BC1(pblock->colors,alt,pblock->activity,pal_mode);
					
					F32 alt_J = vq_J(alt_vqd,cur_indices_codelen,try_ep_codelen[1],lambda);
	...
	bc1rd_final_matrix(blocks,
		endpoint_counting,index_counting,
		lambda,pal_mode,config);
...
static void AnyIndexD_add(AnyIndexD * aid,const rrColorBlock4x4 & block,rrColor32BGRA palette[4])
{
	...
	Vec128 pal_ga16 = _mm_and_si128(_mm_srli_epi32(pal, 8), mask_lo);
	...
	for (int c = 0; c < 16; c += 4)
	{
		...
		Vec128 row_ga16 = _mm_and_si128(_mm_srli_epi32(row, 8), mask_lo);
...
static U32 AnyIndexD_best_index_pair_D(const AnyIndexD * aid1, const AnyIndexD * aid2)
{
	...
	for (int c=0;c<16;c+=4)
	{
		__m128i v0 = _mm_add_epi32( load128u((&aid1->ssd[0][c])), load128u((&aid2->ssd[0][c])) );
		__m128i v1 = _mm_add_epi32( load128u((&aid1->ssd[1][c])), load128u((&aid2->ssd[1][c])) );
		__m128i v2 = _mm_add_epi32( load128u((&aid1->ssd[2][c])), load128u((&aid2->ssd[2][c])) );
		__m128i v3 = _mm_add_epi32( load128u((&aid1->ssd[3][c])), load128u((&aid2->ssd[3][c])) );

		__m128i mins = _mm_min_epi32(
			_mm_min_epi32(v0,v1),
			_mm_min_epi32(v2,v3));
...
static U32 AnyIndexD_add_then_find_indices(AnyIndexD * to, const AnyIndexD & fm, U32 * pD)
{
	...
	*/
	...
	for (int c=0;c<16;c+=4)
	{
		__m128i d0 = _mm_add_epi32( load128u((&to->ssd[0][c])), load128u((&fm.ssd[0][c])) );
		__m128i d1 = _mm_add_epi32( load128u((&to->ssd[1][c])), load128u((&fm.ssd[1][c])) );
		__m128i d2 = _mm_add_epi32( load128u((&to->ssd[2][c])), load128u((&fm.ssd[2][c])) );
		__m128i d3 = _mm_add_epi32( load128u((&to->ssd[3][c])), load128u((&fm.ssd[3][c])) );
	
		// store the sums :
		store128u(&to->ssd[0][c], d0);
		store128u(&to->ssd[1][c], d1);
		store128u(&to->ssd[2][c], d2);
		store128u(&to->ssd[3][c], d3);
		
		__m128i best = _mm_slli_epi32(d0, 2);
		best = _mm_min_epi32(best, _mm_add_epi32(_mm_slli_epi32(d1, 2), const32_1));
		best = _mm_min_epi32(best, _mm_add_epi32(_mm_slli_epi32(d2, 2), const32_2));
		best = _mm_min_epi32(best, _mm_add_epi32(_mm_slli_epi32(d3, 2), const32_3));
							...
							const rrDXT1_VQ_Config & config)
{
	...
	for LOOP(loop_is3c,2)
	{
		...
		for LOOP(b,nblocks_total)
		{
			...
			RR_ASSERT( DXT1_OneBitTransparent_Mask_Same(blocks[b].block_1bt_mask,blocks[b].cur) );
	...
	{
	SIMPLEPROFILE_SCOPE(bc1rd_indices_reduce_mkheap); // all the time is here
	...
	RAD_ALIGN(U8, chunk_inds[entry_chunk_size * 16], 32);
		...
		for (int to_chunk_begin = mode_begin; to_chunk_begin < mode_end; to_chunk_begin += entry_chunk_size)
		{
			...
			for (int fm = mode_begin; fm < mode_end; ++fm)
			{
				...
				else
				{
					...
					// "index_diff > 2.5".
					adjusted_index_diff_limit = conservative_index_diff_limit / entries[fm].count;
				}
				...
				int filtered_count = filter_by_index_sad(filtered_elems,to_chunk_len,
					chunk_inds, entries[fm].unpacked_16x8,
					adjusted_index_diff_limit + 1);

				// AnyIndexD needs the packed indices in a flat array - can I make filter_by_index_sad
				// output this easily?
				for (int i = 0; i < filtered_count; i++)
					filtered_entry_inds[i] = chunk_packed_inds[filtered_elems[i]];

				// Rank all the distortions
				AnyIndexD_batch_lookup(candidate_distortions,&aid,filtered_entry_inds,filtered_count);
	...
	for LOOPVEC(entry_i,entries)
	{
		...
		while( link >= 0 )
		{
			...
			RR_ASSERT( DXT1_OneBitTransparent_Mask_Same(block->block_1bt_mask,block->cur) );
