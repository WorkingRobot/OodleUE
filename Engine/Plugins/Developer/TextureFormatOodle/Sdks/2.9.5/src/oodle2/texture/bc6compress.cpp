...
void BC6PartitionPrep::calc(const BC6Input &input, int p, int ns, bool is_signed, const BC6EncOptions *opt, S16 out_initial_endpoints[4][3])
{
	...
	for (int s = 0; s < ns; ++s)
	{
		...
		// Shuffling covar into the scalar order is a bit awkward, but whatever.
		VecF32x4 merged0 = _mm_shuffle_ps(cov0, cov1, _MM_SHUFFLE(2,0,1,0)); // [0][0], [1][1], [0][1], [0][2]
		VecF32x4 merged1 = _mm_shuffle_ps(cov1, cov0, _MM_SHUFFLE(2,2,1,1)); // [1][2], [1][2], [2][2], [2][2]
		merged1.storeu(cov + 3);
		merged0.shuf<0,2,3,1>().storeu(cov);
	#else
		...
		cov[1] = cov[1]*sf - sumf[0]*sumf[1];
		cov[2] = cov[2]*sf - sumf[0]*sumf[2];
		cov[3] = cov[3]*sf - sumf[1]*sumf[1];
		cov[4] = cov[4]*sf - sumf[1]*sumf[2];
		cov[5] = cov[5]*sf - sumf[2]*sumf[2];
	#endif
...
void BC6PartitionPrep::calc_subset_pca(int s, const float cov[6], const S16 ep[2][3])
{
	...
	else
	{
		for(int iter=0;iter<PCA_POWER_ITERS;iter++)
		{
			...
			float y = cov[1]*local_pca[0] + cov[3]*local_pca[1] + cov[4]*local_pca[2];
			float z = cov[2]*local_pca[0] + cov[4]*local_pca[1] + cov[5]*local_pca[2];
			len_sq = x*x + y*y + z*z;
...
// we do need to handle subnormals but those turn out to be easy the way
// we do it
		...
		for (int c = 0; c < 3; ++c)
		{
			...
			for (int i = 0; i < num_lerp; i += 4)
			{
				...
				if (Mode::is_signed) // put the sign back by grabbing the sign bit from scaled_a
					colors = _mm_or_ps(colors, _mm_and_ps(_mm_castsi128_ps(scaled_a), _mm_set1_ps(-0.0f)));
...
#ifdef DO_BUILD_SSE4
static Vec128 sse4_abs32(const Vec128 &x)
{
...
{
	// Transpose pass 1
	VecF32x4 t0 = _mm_unpacklo_ps(x0, x2);
	VecF32x4 t1 = _mm_unpacklo_ps(x1, x3);
	VecF32x4 t2 = _mm_unpackhi_ps(x0, x2);
	VecF32x4 t3 = _mm_unpackhi_ps(x1, x3);

	// Transpose pass 2
	x0 = _mm_unpacklo_ps(t0, t1);
	x1 = _mm_unpackhi_ps(t0, t1);
	x2 = _mm_unpacklo_ps(t2, t3);
	x3 = _mm_unpackhi_ps(t2, t3);
}
...
static BC6Error calc_indexes_exact_rel(const BC6Input &in, BC6BlockState *st, const BC6ColorSetExactRel &set)
{
	...
	for (int i = 0; i < 16; ++i)
	{
		VecF32x4 pixr = _mm_set1_ps(in.blockf32[0][i]);
		VecF32x4 pixg = _mm_set1_ps(in.blockf32[1][i]);
		VecF32x4 pixb = _mm_set1_ps(in.blockf32[2][i]);
		...
		VecF32x4 best = _mm_castsi128_ps(_mm_set1_epi32(255 << 23)); // +inf (with index 0)
		VecF32x4 and_mask = _mm_castsi128_ps(_mm_set1_epi32(-16)); // mask off low 4 bits
	...
	for (int i = 0; i < 16; i += 4)
	{
		...
		// Tranpose pass 1
		VecF32x4 a0 = _mm_unpacklo_ps(in0, in2);
		VecF32x4 a1 = _mm_unpacklo_ps(in1, in3);
		VecF32x4 a2 = _mm_unpackhi_ps(in0, in2);
		VecF32x4 a3 = _mm_unpackhi_ps(in1, in3);

		// Transpose pass 2
		VecF32x4 b0 = _mm_unpacklo_ps(a0, a1);
		VecF32x4 b1 = _mm_unpackhi_ps(a0, a1);
		VecF32x4 b2 = _mm_unpacklo_ps(a2, a3);
		VecF32x4 b3 = _mm_unpackhi_ps(a2, a3);

		// Min reduction
		VecF32x4 best01 = vmin(b0, b1);
		VecF32x4 best23 = vmin(b2, b3);
		VecF32x4 best = vmin(best01, best23);

		// Emit best indices
		Vec128 inds = _mm_and_si128(_mm_castps_si128(best), _mm_set1_epi32(0xf));
	...
	for (int i = 0; i < 16; ++i)
	{
		...
		float_err += best * in.weightf32[i];
		st->idxs.ind[i] = static_cast<U8>(bestj);
	}
#endif
