...
void AnyIndexD_batch_lookup_AVX2(S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count32)
{
	...
	for (;;) // Loop condition inside, loop structure is a bit unusual
	{
		...
		v_ssd = _mm256_permutevar8x32_epi32(err01, v_four_inds);
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, err23, _mm256_srli_epi32(v_four_inds, 4));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, err45, _mm256_srli_epi32(v_four_inds, 8));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, err67, _mm256_srli_epi32(v_four_inds, 12));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, err89, _mm256_srli_epi32(v_four_inds, 16));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, errab, _mm256_srli_epi32(v_four_inds, 20));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, errcd, _mm256_srli_epi32(v_four_inds, 24));
		v_ssd = AnyIndexD_AVX2_Accumulate(v_ssd, erref, _mm256_srli_epi32(v_four_inds, 28));
		...
		// (D = 2 * ssd)
		v_merged = _mm_add_epi32(v_merged, v_merged);
...
static RADFORCEINLINE __m256 to256(Vec256 value)
{
	return _mm256_castsi256_ps(value);
}

static RADFORCEINLINE Vec256 mm256_blendv_epi32(Vec256 a, Vec256 b, Vec256 mask)
{
	__m256 result = _mm256_blendv_ps(to256(a), to256(b), to256(mask));
...
{
	// Sum across the four rows, but hold off on the lo/hi half or even/odd row reductions
...
void compute_chunk_sads_indirect_AVX2(int chunk_sads[], int count, const U16 indices[], const rrColorBlock4x4 * pair_colors, const rrColorBlock4x4 & rColors)
{
	...
	for (; i < (count & ~3); i += 4)
	{
		Vec256 s0 = block_SAD_nored(pair_colors + indices[i + 0], colors0, colors1);
		Vec256 s1 = block_SAD_nored(pair_colors + indices[i + 1], colors0, colors1);
		Vec256 s2 = block_SAD_nored(pair_colors + indices[i + 2], colors0, colors1);
		Vec256 s3 = block_SAD_nored(pair_colors + indices[i + 3], colors0, colors1);
	...
	for (; i < count; ++i)
	{
		const Vec256 sad = block_SAD_nored(pair_colors + indices[i], colors0, colors1);

		// Add up the low and high halves of the 256b vector (even and odd rows, respectively)
		Vec128 sum = _mm_add_epi16(lo_half(sad), hi_half(sad));

		// Finish reduction by adding the low and high half results of the original SAD vectors
		sum = _mm_add_epi16(sum, shuffle32<2,3,0,1>(sum));
...
int filter_by_index_sad_AVX2(int positions[], int count32, const U8 * index_batch, const U8 ref_inds_ptr[16], int sad_thresh)
{
	...
	for (; i < (count & ~7); i += 8)
	{
		...
		// same port. That's unfortunate but I don't see a good way around it.

		// Compute 8 blocks worth of index SADs
		s0 = _mm256_sad_epu8(ref_inds, load256u(inds_base + 0*32));
		s1 = _mm256_sad_epu8(ref_inds, load256u(inds_base + 1*32));
		s2 = _mm256_sad_epu8(ref_inds, load256u(inds_base + 2*32));
		s3 = _mm256_sad_epu8(ref_inds, load256u(inds_base + 3*32));
		...
		// add up the two halves of each of the original 128b (now adjacent 16b lanes) and store
		s0 = _mm256_madd_epi16(s0, _mm256_set1_epi16(1));
		...
		// order.
		s0 = _mm256_permutevar8x32_epi32(s0, c_even_odd_merge);

		// store IDs of lanes that have values below sad_thresh
		const Vec256 below = _mm256_cmpgt_epi32(v_sad_thresh, s0);
...
S32 bc4rd_single_block_change_distortion_AVX2(const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr)
{
	...
	Vec256 index8_scaled = _mm256_castsi128_si256(load128u(index8_scaled_ptr));
...
void bc4rd_single_block_change_distortion_batch_AVX2(S32 *dest_ssd, const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr, int count)
{
	int i=0;
	if (count >= 2)
	{
		...
		for (; i < (count&~1); i += 2)
		{
			...
			Vec256 index8_scaled = load256u(index8_scaled_ptr + 16*i);
	...
	for (; i < count; ++i)
		dest_ssd[i] = bc4rd_single_block_change_distortion_AVX2(src_pixels, palette, index8_scaled_ptr+16*i);
}
