...
void compute_index_reuse_candidates_by_sad(BC4SubstCandidate *candidate_array, const BC4SADIndices &baseline_sad_inds, const U16 *window_inds, const BC4WindowEntry *past_block_window, int num_blocks_in_window)
{
	...
	for (int pb = DIRECT_CANDIDATES; pb < num_blocks_in_window; ++pb)
	{
		...
		Vec128 sad0_0    = _mm_sad_epu8(baseline_sad_inds.vec, past_block_window[pbi].sad_inds[0].vec);
		Vec128 sad1_0    = _mm_sad_epu8(baseline_sad_inds.vec, past_block_window[pbi].sad_inds[1].vec);
...
static int bc4rd_index_sad_bound(int mode, const U8 *index8_ordered, const S16 *src_pixels, const S16 *raw_palette, U32 constraint_mask, S32 ssd_limit)
{
	...
	for (int i=0; i < 16; ++i) {
		...
		num_steps[i] = n;
		delta_ssd[i][n] = ssd_limit+1;
	}

	// explicit merge of 16 arrays of 7 items
	// @TODO: rewrite delta_ssd so it uses merge[] arrays directly

	S32 merge1[16*8],merge2[16*8];
	int num_items1[8], num_items2[4];

	int m=0,a,b;
	for (int i=0; i < 16; i += 2) {
		int m_start = m;
		int limit = num_steps[i] + num_steps[i+1];
		a=b=0;
		for (int j=0; j < limit; ++j)
			if (delta_ssd[i][a] < delta_ssd[i+1][b])
				merge1[m++] = delta_ssd[i+0][a++];
			else
				merge1[m++] = delta_ssd[i+1][b++];
		num_items1[i>>1] = m - m_start;
		merge1[m++] = ssd_limit+1;
	}
	b=m=0;
	for (int i=0; i < 8; i += 2) {
		int m_start = m;
		a = b;
		b += num_items1[i]+1;
		int limit = num_items1[i] + num_items1[i+1];
		for (int j=0; j < limit; ++j)
		{
			U32 smaller;
			if (merge1[a] < merge1[b])
				smaller = merge1[a++];
			else
				smaller = merge1[b++];
			merge2[m++] = smaller;
		}
		num_items2[i>>1] = m - m_start;
		merge2[m++] = ssd_limit+1;
		++b;
	}
	b=m=0;
	for (int i=0; i < 4; i += 2) {
		...
		a = b;
		b += num_items2[i]+1;
		int end = b + num_items2[i+1];
		int limit = num_items2[i] + num_items2[i+1];
		S32 total = ssd;
		for (int j=0; j < limit; ++j)
		{
			U32 smaller;
			if (merge2[a] < merge2[b])
				smaller = merge2[a++];
			else
				smaller = merge2[b++];
			// early out from long lists
			total += smaller;
			if (total > ssd_limit)
				break;
			merge1[m++] = smaller;
	...
	// sum the smallest SSD deltas until we reach ssd_limit, combined with final phase of merge
	{
		int i=0;
		a = b;
		b += num_items1[i]+1;
		...
		for (int j=0; j < limit; ++j)
		{
			...
			ssd += smaller;
			if (ssd > ssd_limit)
				return m; // the number of items we added without failing is the maximum SAD
			++m;
		}
		++b;
	}

	return 16*7; // maximum index SAD
}
...
struct BC4BlockEvalMode1Info
{
	...
	Vec128   degen_constrain_lo;
	Vec128   degen_constrain_hi;
#endif
...
static RADFORCEINLINE void cached_lls_solve(U8 * pEp0, U8 * pEp1, const BC4BlockInfo & blki, const BC4IndexCache * ic, const BC4EndpointEncInfo & info, int mode)
{
	...
	int ep1_int = _mm_extract_epi32(ep_int,1);
...
static void scalar_batch_cached_lls_solve_mode1(BC4EvalMode1Indices *items, int count, const BC4BlockInfo & blki, const BC4BlockEvalMode1Info & bmode1, const BC4EndpointEncInfo & info)
{
	for (int i=0; i < count; ++i)
	{
		...
		ps->match_R = bc4rd_matched_index_R(ps->dist);
	}
}

#ifdef DO_BUILD_SSE4
#define _mm_blendv_epi32(a,b,c)  _mm_castps_si128(_mm_blendv_ps(_mm_castsi128_ps(a),_mm_castsi128_ps(b),_mm_castsi128_ps(c)))

static void sse_batch_cached_lls_solve_mode1(BC4EvalMode1Indices *items, int count, const BC4BlockInfo & blki, const BC4BlockEvalMode1Info & bmode1, const BC4EndpointEncInfo & info)
{
	Vec128 vals0 = load128u(blki.pixels.values + 0);
	Vec128 vals1 = load128u(blki.pixels.values + 8);
	const BC4EndpointVariantPara & var = info.variant[bc4_6interp];
	...
	for (int i=0; i < count; ++i)
	{
		...
		// Constraints must be satisfied for you to call this
		//RR_ASSERT( items[i].ic->extremes.superset_of(blki.constraints) );
		Vec128 weights8 = load128u(ic->weights);
		...
		// Solve for endpoints 0 and 1
		VecF32x4 ABsum2_flt = VecF32x4::from_int32(ABsum2);
		VecF32x4 inv_ata_raw = VecF32x4::loadu(ic->inv_ata);
		VecF32x4 inv_ata = inv_ata_raw.shuf<0,1,1,2>();
		VecF32x4 ep_prods = ABsum2_flt * inv_ata; // (AX*inv_ata[0], AX*inv_ata[1], BX*inv_ata[1], BX*inv_ata[2])
		VecF32x4 ep_flt = ep_prods + ep_prods.shuf<2,3,0,1>(); // (AX*inv_ata[0] + BX*inv_ata[1], AX*inv_ata[1] + BX*inv_ata[2]) replicated twice

		// Clamp endpoints to valid range
		ep_flt = vmax(ep_flt, bmode1.ep_constrain_lo_f);
		ep_flt = vmin(ep_flt, bmode1.ep_constrain_hi_f);

		// Convert to int
		Vec128 ep_int = ep_flt.to_int32_round();

		// ABsum computation (for degenerate case)
		Vec128 ABsum = _mm_add_epi32(ABsum2, shuffle32<2,3,0,1>(ABsum2));

		// handle degeneracies
		VecF32x4 inv_ata3 = inv_ata_raw.shuf<3,3,3,3>();
		VecF32x4 degen_bias = VecF32x4(ic->degen_bias);
		VecF32x4 mean = VecF32x4::from_int32(ABsum) * inv_ata3 + degen_bias;

		// original code used rr_froundint, which rounded 0.5 differently
		Vec128 degen_ep = mean.to_int32_round();
	    degen_ep = _mm_sub_epi32(degen_ep, _mm_setr_epi32(0,1,0,0));
		degen_ep = _mm_min_epi32(degen_ep, bmode1.degen_constrain_hi);
		degen_ep = _mm_max_epi32(degen_ep, bmode1.degen_constrain_lo);

		Vec128 ep_not_degen  = _mm_cmplt_epi32(shuffle32<1,0,3,2>(ep_int), ep_int); // false if degenerate
		ep_not_degen = shuffle32<0,0,0,0>(ep_not_degen);
		Vec128 final_ep_int = _mm_blendv_epi32(degen_ep, ep_int, ep_not_degen);

		int ep0_int = _mm_cvtsi128_si32(final_ep_int);
		int ep1_int = _mm_extract_epi32(final_ep_int,1);
	...
	for (int i=0; i < count; ++i)
	{
		// inlining the functions below is a miniscule performance gain overall

		// moving everything or everything but match_R into first loop is a noticeable performance loss
		BC4EvalMode1Indices *ps = &items[i];

		//
		// sse4_compute_palette
		//

		// Dequantize endpoints
		Vec128 e0vec = _mm_set1_epi16(info.enc.dequant[ps->ep_byte[0]]);
		Vec128 e1vec = _mm_set1_epi16(info.enc.dequant[ps->ep_byte[1]]);
		...
		// values in mode 1 are in [-0x7000,0x7000]
		// both get sufficient precision with a .17 fraction
		Vec128 palette = _mm_srai_epi16(_mm_mulhi_epi16(endpoints_scaled, var.recip), 1);

		// Negative values end up with quotients too small by 1, fix this.
		palette = _mm_sub_epi16(palette, _mm_srai_epi16(endpoints_scaled, 15));

		//
		// sse4_bc4_decode_with_inds_and_pal
		//

		Vec128 index8_scaled = load128u(ps->predecoded_indices);
		Vec128 index8_scaled_plus_1 = _mm_add_epi8(index8_scaled, _mm_set1_epi8(1)); // Second index to grab second half of 16-bit word
		...
		Vec128 result_0 = _mm_shuffle_epi8(palette, index16_0);
		Vec128 result_1 = _mm_shuffle_epi8(palette, index16_1);
		...
		Vec128 diff01_16 = _mm_sub_epi16(result_0, load128u(blki.pixels.values + 0));
		Vec128 diff23_16 = _mm_sub_epi16(result_1, load128u(blki.pixels.values + 8));
		...
		const SingleFloatBlock4x4 *activity = bmode1.pActivity;
		...
		ps->D = sum_f32.sum_across().scalar_x();

		// slightly slower to move this to the first loop or to put it in its own loop
		ps->match_R = bc4rd_matched_index_R(ps->dist);
	}
}
#endif
...
{
	#ifdef DO_BUILD_SSE4
	sse_batch_cached_lls_solve_mode1(items, count, blki, bmode1, info);
	#else
	scalar_batch_cached_lls_solve_mode1(items, count, blki, bmode1, info);
	#endif
}
...
static void bc4rd_single_block_change_distortion_batch(S32 *distortions, const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr, int count)
{
	#ifndef DO_BUILD_SSE4
	for (int i=0; i < count; ++i) {
		S16 decoded[16];
		bc4_decode_with_inds_and_pal(decoded, index8_scaled_ptr, palette);
		distortions[i] = bc4rd_SSD(decoded, src_pixels);
		index8_scaled_ptr += 16;
	}
	#else
#ifdef DO_BUILD_AVX2
	if ( rrCPUx86_feature_present(RRX86_CPU_AVX2) )
	{
		internal::bc4rd_single_block_change_distortion_batch_AVX2(distortions, src_pixels, palette, index8_scaled_ptr, count);
		return;
	}
#endif
...
S32 bc4rd_single_block_change_distortion_explicit(const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr)
{
	S16 decoded[16];
	bc4_decode_with_inds_and_pal(decoded, index8_scaled_ptr, palette);
	return bc4rd_SSD(decoded, src_pixels);
}
#endif
...
static void AnyIndex8D64_add(AnyIndex8D64 *aid, const S16 *src_pixels, const S16 *palette)
{
	for LOOP(c,16)
		for LOOP(i, 8)
	...
	const BC4Config & config)
{
	...
	for LOOPVEC(ei,entries)
	{
		...
		if (entries[ei].count > 1)
			AnyIndex8D64_block_link_init(&aids[ei], entries[ei].block_link, blocks, infos, entries[ei].count);
	}
	...
	U8 filtered_index8_scaled[filter_chunk_size][16];
		...
		for (int to_chunk_begin = start; to_chunk_begin < entries_size; to_chunk_begin += entry_chunk_size)
		{
			...
			for (;fm<first_singleton;fm++)
			{
...

				{
					...
					#if 1
					AnyIndex8D64_batch_lookup(candidate_distortions_64, &aids[fm], chunk_index8_scaled, entries[to_chunk_begin].index8_scaled, sizeof(entries[0]), to_chunk_len);
					for (int i=0; i < to_chunk_len; ++i)
						RR_ASSERT(candidate_distortions_64[i] == AnyIndex8D64_lookup(&aids[fm], entries[to_chunk_begin+i].index8_scaled));
					#endif
				}
		...
		// make heap for singletons
		{
			...
			for (int to_chunk_begin = start; to_chunk_begin < entries_size; to_chunk_begin += filter_chunk_size)
			{
				...
				for (int fm = first_singleton; fm < entries_size; fm++)
				{
					...
					if (to_chunk_begin == start)
					{
						filters[fm].conservative_SAD =
							bc4rd_index_sad_bound(	entries[fm].mode, filters[fm].index8_ordered,
													...
													(S32) RR_MAX(best_distortion, best_distortion * config.distortion_ratio_for_refine));
					}
					...
					filtered_count = filter_by_index_sad(filtered_elems,to_chunk_len,
					...
					for (int i=0; i < filtered_count; ++i)
						memcpy(filtered_index8_scaled[i], &chunk_index8_scaled[filtered_elems[i] * 16], 16);

					bc4rd_single_block_change_distortion_batch(candidate_distortions, pixels, palette, filtered_index8_scaled[0], filtered_count);
					for (int i=0; i < filtered_count; ++i)
					...
					for (int i=0;; ++i)
					{
						...
						i = find_next_below(candidate_distortions, i, RR_MAX((S32) best_distortion * config.distortion_ratio_for_refine, (S32) best_distortion));
						if ( i >= filtered_count )
							break;
	...
	const rrDXTCRD_Options & rdopts)
{
	...
	*/
	...
	for LOOP(bi,nblocks)
	{
		...
		{
			blk_mode1.ep_constrain_hi_f = _mm_blend_ps(blk_mode1.ep_constrain_hi_f, info.lo_vec, 2); // cmin[0] = hi_vec[0]
			degen_ep1_hi = info.enc.min_q;
		}
		 
		if ( blk_info.constraints.mask & 0xffff0000u )
		{
			blk_mode1.ep_constrain_lo_f = _mm_blend_ps(blk_mode1.ep_constrain_lo_f, info.hi_vec, 1); // cmax[1] = lo_vec[0] 
			degen_ep0_lo = info.enc.max_q;
		}

		blk_mode1.degen_constrain_lo = _mm_setr_epi32(degen_ep0_lo, degen_ep1_lo, 0,0);
		blk_mode1.degen_constrain_hi = _mm_setr_epi32(degen_ep0_hi, degen_ep1_hi, 0,0);
		#endif
		...
		for LOOP(i,DIRECT_CANDIDATES + config.index_heap_size)
		{
			...
			mode1_item.dist = bi - wnd->most_recent_bi;
		}
		...
		for LOOP(i,num_mode1_items)
		{
			...
			// compute rate for match distance we would have
			F32 match_R = mode1_items[i].match_R; // bc4rd_matched_index_R( bi - wnd->most_recent_bi );
