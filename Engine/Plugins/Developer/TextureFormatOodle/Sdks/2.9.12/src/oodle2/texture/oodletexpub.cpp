	...
	int num_job_threads,void * jobify_user_ptr)
{
	...
	if ( layout == NULL )
	{
		SINTa from_num_blocks = TotalBlockCount(from_rrs,(int)num_from_surfaces);
	...
	int num_job_threads,void * jobify_user_ptr)
{
	...
	BlockSurfaceObj from_blocks;
	rrSurfaceObj from_rrs_local;
	vector<rrSurfaceObj> from_rrs_vec;
	rrSurface * from_rrs = &from_rrs_local;
	
	if ( num_from_surfaces != 1 )
	{
		from_rrs_vec.resize(num_from_surfaces);
		from_rrs = from_rrs_vec.data();
	}
	...
	if ( rrPixelFormat_GetInfo(from_rrs->pixelFormat)->isFloat && ! bcn_is_float(to_bcn) )
	{
