...
static void sse4_neon_force_anchor_bit_zero(BC7SubsetState *st, const BC7SubsetInput *in, const BC7Color endpoints_q[2], int anchor_pos)
{
	...
	if (idx_v[anchor_pos] & anchor_bit_v)
	{
		...
		// Update the error: subtract the old error and add the new one
		Vec128_S32 lo = Vec128_U8::loadu_lo32(&endpoints_q[0]).to_s32_lo();
		Vec128_S32 hi = Vec128_U8::loadu_lo32(&endpoints_q[1]).to_s32_lo();
...
static bool bc7enc_refine_lsq_endpoints_and_try(BC7SubsetState *cur_best, BC7SubsetState *st, const BC7SubsetInput *in, BC7Flags flags, bool no_reindex = false, bool keep_pbits = false)
{
	...
	// (but if st->err == BC7_ERROR_MAX, we're redoing endpoints for
	// bc7enc_find_endpoints, so we have to determine the actual cost)
	if ( st->endpoints[0] == orig_ep[0] && st->endpoints[1] == orig_ep[1] && st->err != BC7_ERROR_MAX )
		return false;
	...
	bool has_ib2, float min_separation, float min_separation_a)
{
	...
	for (int i = 0; i < in->num_pixels; ++i)
	{
		...
		if (has_ib2)
		{
			min_a = RR_MIN(min_a, a);
			max_a = RR_MAX(max_a, a);
		}
	}
...
template <typename Mode>
static BC7Error bc7enc_try_partition(const U8 *block, const BC7Prep *prep, BC7Results *results, const BC7EncOptions *opt, int p, const U8 * pca_subsets, int config_override=-1)
{
...
static BC7Error bc7enc_try_partition_dispatch(const U8 *block, const BC7Prep *prep, BC7Results *results, const BC7EncOptions *opt, int mode, int p, const U8 * pca_subsets)
{
	...
	case 4: return bc7enc_try_partition<BC7Mode<4>>(block, prep, results, opt, 0, pca_subsets, p - 1); // p for mode 4 is config_override; 0=default, 1-8=specify rbit, isb
	case 5: return bc7enc_try_partition<BC7Mode<5>>(block, prep, results, opt, 0, pca_subsets, p - 1); // p for mode 5 is config_override; 0=default, 1-4=specify rbit
	...
	default: RR_ASSERT_FAILURE("bad mode"); return BC7_ERROR_MAX;
	}
}
...
const U8 * BC7Prep_init(BC7Prep * prep, const U8 in_block[64], const BC7EncOptions & opt, bool force_consider_all_modes)
{
	...
	if (!prep->single_color)
	{
		...
		{
			// always do this even if all 2-subset modes (1, 3, 7) are disabled
			// no encoding preset actually disables all of these, and mode 4/5
			// encoding still wants the subset analysis that we compute when
			// max_ns >= 2.
			prep->max_ns = 2;
		}
	}
