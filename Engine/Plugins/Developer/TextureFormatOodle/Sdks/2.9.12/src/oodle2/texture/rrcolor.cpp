...
static float Clamp01NansTo0(float x)
{
	const float clamped_lo = (x > 0.0f) ? x : 0.0f; // Also turns NaNs into 0.
	return (clamped_lo < 1.0f) ? clamped_lo : 1.0f;
}

void rrColor_SRGBToLinear(const rrColor32RGBA* src, rrColor4F* dst, SINTa count)
{
	for (SINTa i = 0; i < count; i++)
	{
		dst[i].r = srgb_byte_to_linear_unit_float(src[i].u.r);
		dst[i].g = srgb_byte_to_linear_unit_float(src[i].u.g);
		dst[i].b = srgb_byte_to_linear_unit_float(src[i].u.b);
		dst[i].a = src[i].u.a / 255.f;
	}
}

void rrColor_SRGBToLinear(const rrColor32BGRA* src, rrColor4F* dst, SINTa count)
{
	for (SINTa i = 0; i < count; i++)
	{
		dst[i].r = srgb_byte_to_linear_unit_float(src[i].u.r);
...
void rrColor_LinearToSRGB_Reference(const rrColor4F* src, rrColor32BGRA* dst, SINTa count)
{
	for (SINTa i = 0; i < count; i++)
	{
		dst[i].u.r = linear_unit_float_to_srgb_byte(src[i].r);
		dst[i].u.g = linear_unit_float_to_srgb_byte(src[i].g);
		dst[i].u.b = linear_unit_float_to_srgb_byte(src[i].b);
		dst[i].u.a = (U8)(0.5f + Clamp01NansTo0(src[i].a) * 255.f);
	}
}

void rrColor_LinearToSRGB(const rrColor4F* src, rrColor32BGRA* dst, SINTa count)
{
#if defined(DO_BUILD_SSE4)
	for (SINTa i = 0; i < count; i++)
	{
		VecF32x4 InRGBA = VecF32x4::loadu(&src[i].r);
		...
		const VecF32x4 AlmostOne = VecF32x4::bitmask(0x3f7fffff); // 1-eps
		const Vec128_S32 MinValInt { (127 - 13) << 23 };
		const VecF32x4 MinValFlt = MinValInt.f32();

		const VecF32x4 InClamped = InRGBA.max(MinValFlt).min(AlmostOne);
		...
		const Vec128_S32 TabIndexVec = (InClamped.s32() - MinValInt).srl<20>();
		...
		const Vec128_S32 TabValR = Vec128_S32::loadu_lo32(&fp32_to_srgb8_tab4[TabIndexVec.extract<0>()]);
		const Vec128_S32 TabValG = Vec128_S32::loadu_lo32(&fp32_to_srgb8_tab4[TabIndexVec.extract<1>()]);
		const Vec128_S32 TabValB = Vec128_S32::loadu_lo32(&fp32_to_srgb8_tab4[TabIndexVec.extract<2>()]);

		// Merge the four values we just loaded back into a 3-vector (gather complete!)
		const Vec128_S32 TabValRG = TabValR.unpack_lo(TabValG);
		const Vec128_S32 TabValsRGB = TabValRG.unpack_lo64(TabValB); // This leaves A=0, which suits us
		...
		const Vec128_S32 MantissaLerpFactor = InClamped.s32().srl<12>() & Vec128_S32 { 0xff };
		const Vec128_S32 FinalMultiplier = MantissaLerpFactor | Vec128_S32(512 << 16);
		...
		const Vec128_S32 InterpolatedRGB = TabValsRGB.s16().madd(FinalMultiplier.s16()).srl<16>();
		...
		const VecF32x4 ScaledBiasedA = InClamped * VecF32x4(0.f, 0.f, 0.f, 255.f) + VecF32x4(0.5f);
		const Vec128_S32 FinalA = ScaledBiasedA.to_int32_trunc();

		// Merge A into the result, then pack down to bytes and store!
		// InterpolatedBGR has lane 3=0, and ComputedA has the first three lanes zero,
		// so we can just OR them together.
		const Vec128_S32 FinalRGBA = InterpolatedRGB | FinalA;
		const Vec128_S32 FinalBGRA = FinalRGBA.shuf<2,1,0,3>();

		const Vec128_S16 Packed16 = FinalBGRA.to_s16_sat(FinalBGRA);
		const Vec128_U8 Packed8 = Packed16.to_u8_sat();

		store32u(dst + i, Packed8);
	}
#else
	rrColor_LinearToSRGB_Reference(src, dst, count);
#endif
}
