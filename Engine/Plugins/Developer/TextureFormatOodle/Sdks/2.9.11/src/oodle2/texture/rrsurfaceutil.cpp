...
rrbool rrSurface_BlitNormalized(rrSurface * to, const rrSurface * from, rrRangeRemap remap)
{
	...
	case rrRangeRemap_LinearToSRGB:
		// these asserts are not actually required:
		RR_ASSERT( RR_PIXELFORMAT_GET_TYPE(from->pixelFormat) == RR_PIXELFORMAT_TYPE_F32 );
		RR_ASSERT( RR_PIXELFORMAT_GET_TYPE(to->pixelFormat) == RR_PIXELFORMAT_TYPE_U8 );

		// Linear->sRGB conversion happens here, we don't fall through to the rest of the code
		rrSurface_Gamma_SRGB_UnitLinearFloat_To_255Int(to, from);
		return true;

	case rrRangeRemap_SRGBToLinear:
		RR_ASSERT( RR_PIXELFORMAT_GET_TYPE(to->pixelFormat) == RR_PIXELFORMAT_TYPE_F32 );
