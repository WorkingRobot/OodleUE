...
struct rrSurface
{
	...
	// w,h,stride are all 64b on 64b targets now, 32b is a bug factory with large images (>=2GB) due to overflows
	SINTa	width,height;
...
static RADINLINE U8 * rrSurface_Seek(rrSurface * surf,SINTa x,SINTa y)
{
	return rrPixelFormat_Seek(surf->pixelFormat,surf->data,surf->stride,x,y);
}

static RADINLINE const U8 * rrSurface_SeekC(const rrSurface * surf,SINTa x,SINTa y)
{
	return rrPixelFormat_Seek(surf->pixelFormat,surf->data,surf->stride,x,y);
}
...
// debug function to compute a hash of rrSurface contents
U64 rrSurface_DebugHash(const rrSurface * surf);

// assert that a surface has the given dimensions and pixel format
...
public :
	...
	rrSurfaceObj(rrSurfaceObj&& x)
	{
		rrSurface_MoveOwnership(this, &x);
	}
