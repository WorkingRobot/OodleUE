	...
	void * jobify_user_ptr, int num_workers)
{
	...
	rrbool ok = BlockSurface_JobSlicer_256K_then_N(to_blocks,&ctx->from_blocks,num_workers,jobify_user_ptr,rrDXTCRD_Slicer_Func,&data,max_slice_blocks);
				...
				EDXTCRD_Metric metric,const rrDXTCRD_Options &rdoptions)
{
	...
	if ( ! rrSurfaceDXTC_CompressBCN_Blocks(&ctx->to_baseline,&ctx->from_blocks,cfg->baseline_effort,num_workers,options,jobify_user_ptr) )
		return false;
	}
