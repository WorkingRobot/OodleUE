	...
	const bc7bits & bits_mask)
{
	...
	BC7Flags combined_flags = flags & ~BC7ENC_PRESERVE_EXTREMES;
	...
	bool do_indices)
{
	...
	for LOOP(mode,8)
	{
		...
		for LOOP(bi,tot_num_blocks)
		{
			...
			// For now, exclude constrained blocks from final optimize
	...
	for LOOP(iters,1)
	{
		...
		for LOOP(bi,nblocks)
		{
			...
			*/
							...
							if ( satisfied_constraints.superset_of(infos[bi].constraints) )
							{
								...
								if ( cur_D < best_J )
								{
									...
									if ( cur_J < best_J )
									{
										...
										bc7enc_state_from_bits(&best_st,bc7bits_U8ptr(&pair1),infos[bi].flags);
			...
			bc7enc_emit(output_bc7,&best_st, infos[bi].flags);
		...
		if (config.do_bottom_up_merge &&
			(opt.flags & BC7ENC_PRESERVE_EXTREMES) == 0) // For 2.9.7: disable bottom-up merge when preserve extremes on. Revisit later.
		{
			#if 1
			// @@!!
