...
template <typename t_to, typename t_fm>
inline t_to check_value_cast( const t_fm & from )
{
	t_to to = static_cast<t_to>(from);

	// cast back to t_fm and check equality :
	RR_ASSERT( static_cast<t_fm>(to) == from );
	
	// negative-ness should match :
	RR_ASSERT( (from < 0) == (to < 0) );
...
{
	*pto = check_value_cast<t_to>(from);
}
