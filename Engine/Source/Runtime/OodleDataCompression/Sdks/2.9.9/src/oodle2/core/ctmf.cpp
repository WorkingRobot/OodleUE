...
int CacheTableMatchFinder::ProcessChunk(int chunkSize, UnpackedMatchPair * outMatches, int maxPairs)
{
	...
	for(; pos < chunkEnd; pos++)
	{
		...
		for(;;) // two hash rows
		{
			...
			{
				RAD_ALIGN(U32, offsets[16], 16);
				uint32x4_t vRefHash = vdupq_n_u32(hash);
				uint32x4_t vRefPos = vdupq_n_u32(posm1);
				...
				uint32x4_t vOffsBound = vdupq_n_u32((U32) RR_MIN((UINTa) (ptr - dictionaryBase), CTMF_POS_MASK+1));

				#define CHECKFOUR(i) \
					uint32x4_t rowData##i = vld1q_u32(&cur_row[i*4]); \
					/* ((posm1 - row[i]) & CTMF_POS_MASK) + 1) */ \
					uint32x4_t offs##i = vaddq_u32(vandq_u32(vsubq_u32(vRefPos, rowData##i), vPosMask), vOne); \
					/* (row[i] ^ hash) & CTMF_CHECK_MASK */ \
					uint32x4_t checkbits##i = vandq_u32(veorq_u32(rowData##i, vRefHash), vCheckMask); \
					vst1q_u32(offsets + i*4, offs##i); \
					uint32x4_t offsOutOfRange##i = vcgtq_u32(offs##i, vOffsBound); \
					uint32x4_t isInvalid##i = vorrq_u32(checkbits##i, offsOutOfRange##i); /*nonzero if invalid*/ \
					uint32x4_t vValid##i = vceqzq_u32(isInvalid##i)
				...
				uint16x8_t vValid01 = vuzp1q_u16(vreinterpretq_u16_u32(vValid0), vreinterpretq_u16_u32(vValid1));
				uint16x8_t vValid23 = vuzp1q_u16(vreinterpretq_u16_u32(vValid2), vreinterpretq_u16_u32(vValid3));
				uint8x16_t vValid0123 = vuzp1q_u8(vreinterpretq_u8_u16(vValid01), vreinterpretq_u8_u16(vValid23));

				// extract "mask valid" bits
				uint8x8_t vValid_nib = vshrn_n_u16(vreinterpretq_u16_u8(vValid0123), 4); // 0x0 or 0xf per nibble
				U64 validMask = vget_lane_u64(vreinterpret_u64_u8(vValid_nib), 0) & 0x1111111111111111ull;
