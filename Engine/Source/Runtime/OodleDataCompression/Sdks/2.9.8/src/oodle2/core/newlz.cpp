	...
	bool lo0_allowed,const OodleLZ_CompressOptions *pOptions)
{
	...
	{ \
		if ( i != 0 || lo0_allowed ) { \
			...
			int ml = getmatchlen_mml2_one32(ptr32,ptr,ptr-offset,ptr_matchend); \
			if ( ml > lo_ml ) { \
				lo_ml = ml; \
				lo_index = i; \
			} \
	...
	for(;;) // two hash rows
	{
		...
		for(int d=0;d<newLZ_CTMF::c_table_depth;d++)
		{
			...
			U32 offset = (cur_absolute_pos - hash_entry)&CTMF_POS_MASK; 
			...
			if ( newLZ_IsAllowedNormalMatch(len,offset,pOptions) &&
				 ( ( newLZ_CTMF::c_table_depth == 1 && !newLZ_CTMF::c_do_second_hash ) || newLZ_IsNormalMatchBetter(len,offset,bestml,bestoff) ) )
			{
				bestml = len;
				bestoff = offset;
			}
...
#if 0 // no longer used, always TLL now, left for reference

static RADINLINE S32 try_lo_arrival(newlz_optimal_arrival * arrivals,
	...
	const newlz_codecosts & codecosts)
{
	...
	if ( lo_cost < arrivals[pos+ml].cost )
	{
		arrivals[pos+ml].cost = lo_cost;
		arrivals[pos+ml].ml = ml;
		arrivals[pos+ml].lrl = lrl;
		arrivals[pos+ml].offset = -loi;
		arrivals[pos+ml].los = los;
		arrivals[pos+ml].los.MTF(loi);
		arrivals[pos+ml].is_twostep = 0;
	}
	return lo_cost;
}
				
static RADINLINE S32 try_match_arrival(newlz_optimal_arrival * arrivals,
...
{			
	...
	if ( cost < arrivals[pos+ml].cost )
	{
		...
		arrivals[pos+ml].los = los;
		arrivals[pos+ml].los.Add(offset);
		arrivals[pos+ml].is_twostep = 0;
	}
	return cost;
}
...
static RADFORCEINLINE S32 try_lo_arrival_tll(
	newlz_optimal_arrival_tll * tll_arrivals,int num_tlls,
	...
	const newlz_codecosts & codecosts)
{
	...
	if ( num_tlls == 1 )
	{
		newlz_optimal_arrival_tll & arrival = tll_arrivals[ (pos+ml) ];
	...
	{		
		...
		{
			newlz_optimal_arrival_tll & arrival = tll_arrivals[ (pos+ml)*num_tlls ];
		...
		do
		{
			...
			newlz_optimal_arrival_tll & arrival = tll_arrivals[ (pos+ml+t)*num_tlls + t ];
		...
		const OodleKrakenChunkDeadlines * deadline)
{
	...
	*/
	...
	scratch->arrivals_space.extend(sizeof(newlz_optimal_arrival_tll)*num_tlls*(chunk_len+1),arena);
	newlz_optimal_arrival_tll * tll_arrivals = (newlz_optimal_arrival_tll *) scratch->arrivals_space.m_ptr;
	...
	// given type.
	int twostep_end_pos = parse_end_pos - (TWO_STEP_MAX_LRL + NEWLZ_LOMML);
	int twostep_lo_end_pos = twostep_end_pos;
	#endif

	// Rate-limits updating of the parse cost vector. This is a noticeable fraction
	// of encode time on some files.
	int cost_update_interval = (vtable->level >= OodleLZ_CompressionLevel_Optimal2) ? 1 : 1024;
	...
	for(;;)
	{
		...
		// codecosts changed, recost it :
		S32 cost_min_offset = cost_offset(NEWLZ_MIN_OFFSET,codecosts);
		...
		if ( num_tlls > 1 )
		{
			...
			for(int t=1;t<num_tlls;t++)
			{
				...
				tll_arrivals[(parse_chunk_start+t)*num_tlls + t].cost += cost_add_literal(chunk_ptr,parse_chunk_start+t-1,lo,codecosts);
			}
			...
			pos++)
		{
			...
			else // num_tlls > 1
			{
				...
				if ( my_arrivals[last_tll].cost != RR_S32_MAX )
				{
					...
					S32 cost = my_arrivals[last_tll].cost + cost_add_literal(chunk_ptr,pos,lo,codecosts);
			...
			UnpackedMatchPair cur_matches[NEWLZ_MATCH_NUM_PAIRS+1];
			int cur_matches_count = 0;
			S32 cur_matches_cost[NEWLZ_MATCH_NUM_PAIRS+1];
			...
			for(int m=0;m<NEWLZ_MATCH_NUM_PAIRS;m++)
			{
				...
				cur_matches_cost[cur_matches_count] = cost_offset(offset,codecosts);
				cur_matches_count++;
			}
			if ( 1 )
			{
				...
				if ( ml >= mml )
				{
					cur_matches[cur_matches_count].length = ml;
					cur_matches[cur_matches_count].offset = offset;
					cur_matches_cost[cur_matches_count] = cost_min_offset;
					cur_matches_count++;
				}
			}
			...
			for(int lrl_iterator=0;;lrl_iterator++)
			{
				...
				for(int loi=0;loi<NEWLZ_NUM_LAST_OFFSETS;loi++)
				{
					...
					int ml = getmatchlen_mml2_one32(ptr32,ptr,ptr-offset,ptr_matchend);
					// test : LO MML 3 : this is a NOP on text
					//int ml = getmatchlen_mml3_one32(ptr32,ptr,ptr-offset,ptr_matchend);
				...
				Costing matches in the LRL loop is a bit of annoying extra work
				
				Matches are always the same (unlike LO's which depend on the LO-Set that comes from LRL)
				And we'll only take the one cheapest arrival
				So why all the LRL looping?
				
				On most files it's not needed - you could just cost matches *once*
				At the pos which has lowest_base_cost
				Find the one LRL that gets you to the start of the match cheapest,
				and cost from there.
				
				But on some files that's not good enough
				This appears to be because of the LRL-ML correlation in packet
							
				LOD0_021030201133223.pb.tile & silesia_mozilla
				are the only files I've found where this makes a noticeable difference

				trying LRL's :
				LOD0_021030201133223.pb.tile :  8,439,578 -> 2,154,112 =  2.042 bpb =  3.918 to 1 

				not :
				LOD0_021030201133223.pb.tile :  8,439,578 -> 2,207,571 =  2.093 bpb =  3.823 to 1 
				...
				**/
				...
				{
				//SIMPLEPROFILE_SCOPE(inner3_match);
				...
				for(int m=0;m<cur_matches_count;m++)
				{
					...
					S32 cost = base_cost + cur_matches_cost[m];
			...
			if ( longestml >= optimal_skip_len )
			{
				...
				else
				{
					...
					for(int t=1;t<num_tlls;t++)
					{
						...
						tll_arrivals[(longestml_arrival_pos+t)*num_tlls + t].cost += cost_add_literal(chunk_ptr,longestml_arrival_pos+t-1,lo,codecosts);
					}
	...
	if ( raw_len > 64*1024 && raw != NULL )
	{
		...
		if ( level < OodleLZ_CompressionLevel_Normal && level >= OodleLZ_CompressionLevel_HyperFast2 )
		{
			...
			if ( newlz_guess_should_hash_length_be_over_four(raw,raw_len) )
				hash_len = 6;
			//rrprintfvar(hash_length_over_four);
		}
	}
	/**/
	// SuperFast on seven ; encode Weissman says yes (barely)
	//	the win on Silesia is easier ; on pd3d & GTS it's pure lose
	// 0.6 c/B penalty
	// the should_hash test is only 0.2 c/B , but it also adds time to the encode parse
	//	 because of 64-bit hash instead of 32
	//SUM:total : 80,000,000 ->30,191,807 =  3.019 bpb =  2.650 to 1 
	//SUM:encode           : 1.197 seconds, 25.87 c/B, rate= 66.84 MB/s
	//SF: [1.350]
	//SUM:total : 80,000,000 ->30,550,554 =  3.055 bpb =  2.619 to 1 
	//SUM:encode           : 1.168 seconds, 25.23 c/B, rate= 68.52 MB/s
	//SF: [1.346]
