...
Push-Pop a label called "Frame" to get frame timing
...
{
public:
	explicit ThreadProfileScope(const char * label,U64 guid,bool enable)
	{
		m_handle = enable ? ThreadProfiler_Push(label,guid) : TP_INVALID_HANDLE;
	}
...
#define THREADPROFILESCOPE(label)		OODLE_NS_PRE ThreadProfileScope RR_NUMBERNAME(tps) (label,0,true)
#define THREADPROFILESCOPE2(label,guid)	OODLE_NS_PRE ThreadProfileScope RR_NUMBERNAME(tps) (label,guid,true)
#define THREADPROFILESCOPE_FILTERED(label,cond)		OODLE_NS_PRE ThreadProfileScope RR_NUMBERNAME(tps) (label,0,(cond))
#endif
...
#define THREADPROFILESCOPE_FILTERED(label)	
