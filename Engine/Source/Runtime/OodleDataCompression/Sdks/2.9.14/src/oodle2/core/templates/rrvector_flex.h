...
public:
	...
	void extend_default(const size_t count)
	{
		...
		m_size += count;
		default_construct(begin() + oldsize,count);
	}
	...
	// extend by adding one element and constructing it in place (for push_back, emplace_back etc.)
	template<typename... Args>
	RADFORCEINLINE void extend_construct_in_place(Args&&... args)
	{
		const size_t oldsize = m_size;
		if ( needmakefit(oldsize + 1) )
		{
			const size_t oldcapacity = capacity();
			// args can refer back to existing vector element, so need to be careful here
			t_entry * pOld = makefit1(oldsize + 1);
			construct(begin() + oldsize,forward<Args>(args)...);
			m_size = oldsize + 1;
			this->makefit2(pOld, oldsize, oldcapacity);
			return;
		}
...
public:
	...
	SINTa		sizea() const		{ return check_value_cast<SINTa>(size()); }
	...
	void push_back(const value_type & e)	{ parent_type::extend_construct_in_place(e); }
	void push_back(value_type&& e)			{ parent_type::extend_construct_in_place(move(e)); }

	//@@ I'm not fond of push_back with no argument; it's unclear and error prone, but here for compatibility
	void push_back()						{ parent_type::extend_construct_in_place(); }

	template<typename... Args>
	void emplace_back(Args&&... args)		{ parent_type::extend_construct_in_place(forward<Args>(args)...); }
...
T_PRE_IT::insert(const iterator position, const value_type & e)
{
	...
	else
	{
		...
		if ( size() > 0 )
		{
			parent_type::extend_construct_in_place( back() );
		}
