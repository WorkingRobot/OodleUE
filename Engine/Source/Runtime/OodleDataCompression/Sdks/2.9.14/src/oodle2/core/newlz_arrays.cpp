	...
	)
{
	...
	*/
	...
	call runtest -p win_x64 comptime -c8 -z3 -e5 r:\testsets\gametestset -a
	...
	call runtest -p win_x64 comptime -c9 -z1 -zs512 -e5 r:\testsets\gametestset -a
...
// Counts the length of the leading run of 0s at values; i.e. returns
// the largest i such that
//   values[0] .. values[i-1]
// are all 0. If values[0] is nonzero, returns 0. If values[0] .. values[count-1]
// are all 0, returns count.
static SINTa count_zero_run_len(const U32 * values, SINTa count)
{
	SINTa i = 0;

	// Scan over pairs of zeros at a time
	while ( i <= count - 2 )
	{
		const U64 two_vals = RR_GET64_NATIVE_UNALIGNED(values + i);
		if ( two_vals != 0 )
		{
			return i + (values[i] == 0 ? 1 : 0);
		}
		i += 2;
	}

	while ( i < count && values[i] == 0 )
		++i;
...
U32 newlz_array_estimate_complen_bits(const U32 * histo,int alphabet, U32 sumCounts)
{
	...
	if ( sumCounts < NEWLZ_HUFF_ARRAY_MIN_SIZE ) // short arrays can't use proper Huff/TANS
	{
		// If it's all one symbol, can use trivial RLE mode (1 byte payload)
		if ( sumCounts > 0 )
		{
			// Find first symbol with nonzero count in the histo (there has to be one)
			SINTa i = count_zero_run_len(histo, alphabet);

			// Is total accounted for by that one symbol?
			if (i < alphabet && histo[i] == sumCounts)
			{
				// Header + 1 byte payload
				return newlz_array_header_size_bits + 8;
			}
		}

		// 3 bytes for header, plus uncompressed payload
		return (3 + sumCounts)*8;
	}
	...
	U32 header_bits_shape = 0;
	U64 cl_sum = 0;
	int num_non_zero = 0;
	for(int i=0;i<alphabet;) // no i increment here, that's handled inline
	{
		// Determine length of zero run (missing symbols)
		U32 zero_runlen = (U32)count_zero_run_len(histo + i, alphabet - i);
		i += zero_runlen;

		// Length of zero run is sent as an EG(1) code
		// Initial zero run can have length 0, subsequent ones cannot
		RR_ASSERT(i == 0 || zero_runlen != 0);
		if (zero_runlen != 0)
			header_bits_shape += rrVarBits_CountBits_EliasGamma((zero_runlen - 1) >> 1) + 1;

		if (i >= alphabet)
			break;

		U32 nonzero_runlen = 0;
		for (; i < alphabet && histo[i] != 0; ++i)
		{
			U32 c = histo[i];
			nonzero_runlen++;
			...
			// Accumulate in 64 bits because this might overflow otherwise
			cl_sum += (U64)c * cl;
		}

		// Length of nonzero run is sent as an EG(0) code
		RR_ASSERT(nonzero_runlen >= 1);
		header_bits_shape += rrVarBits_CountBits_EliasGamma(nonzero_runlen - 1);
		num_non_zero += nonzero_runlen;
	}
	...
	// alternate shape transmission (for low num_non_zero)
	// 8 bits per non-zero symbol :
	U32 alt_shape_size = num_non_zero * 8;
	U32 header_sum = header_bits_counts + RR_MIN(alt_shape_size,header_bits_shape);
