...
#error Set NEWLZHC_PARSE_CAREFUL_OUTPUT to 0 or 1!
	...
	**********/	
		...
		**/
		...
		#else // RAW, SUB, LAMSUB
		{
			...
			
			{
				...
				SINTa lrl  = lrl_part >> NEWLZHC_PACKET_ML_BITS;
			#ifdef NEWLZHC_PARSE_X64_USE_INLINE_ASM // newer Clangs completely butcher the intended codegen for this, so use inline ASM
				SINTa temp;
				__asm__(
					"cmpq	%[lrl_max], %[lrl]\n"
					"leaq	4(%[excesses_ptr]), %[temp]\n"
					"cmovel	(%[excesses_ptr]), %k[lrl]\n"
					"cmoveq	%[temp], %[excesses_ptr]\n"
					: [lrl]"+r"(lrl), [excesses_ptr]"+r"(excesses_ptr), [temp]"=&r"(temp)
					: [lrl_max]"i"(NEWLZHC_PACKET_LRL_MAX)
				);
			#else
	...
	#ifdef NEWLZHC_PARSE_X64_USE_INLINE_ASM
		SINTa temp;
		__asm__(
			"cmpq	%[num_last_offsets], %[packet_offset]\n"
			"sbbq	%[temp], %[temp]\n"
			"leaq	4(%[offsets_ptr], %[temp], 4), %[offsets_ptr]\n"
			: [offsets_ptr]"+r"(offsets_ptr), [temp]"=&r"(temp)
			: [packet_offset]"r"(packet_offset), [num_last_offsets]"i"(NEWLZHC_NUM_LAST_OFFSETS)
		);
	#else
