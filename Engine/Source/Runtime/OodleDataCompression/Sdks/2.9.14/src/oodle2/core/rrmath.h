...
// need the intrinsics for lrintf
#ifdef __RADSSE2__
#include <emmintrin.h>
#endif

#if defined(__RADARM64__) && defined(__RADNEON__)
#include <arm_neon.h>
#endif
...
static RADINLINE U32 rrAlignUp32(const U32 v, const SINTa alignment)
{
    RR_ASSERT(rrIsPow2_A(alignment));
    //RR_ASSERT( v >= 0 );
    return (U32)((v+(alignment-1)) & ~(alignment-1));
}

// Aligns v down, alignment must be a power of two.
static RADINLINE U32 rrAlignDown32(const U32 v, const SINTa alignment)
{
    RR_ASSERT(rrIsPow2_A(alignment));
    //RR_ASSERT( v >= 0 );
    return (U32)(v & ~(alignment-1));
}

// Aligns v up, alignment must be a power of two.
static RADINLINE U64 rrAlignUp64(const U64 v, const SINTa alignment)
{
    RR_ASSERT(rrIsPow2_A(alignment));
    return (U64)((v+(alignment-1)) & ~(alignment-1));
}

// Aligns v down, alignment must be a power of two.
static RADINLINE U64 rrAlignDown64(const U64 v, const SINTa alignment)
{
    RR_ASSERT(rrIsPow2_A(alignment));
    return (U64)(v & ~(alignment-1));
}

#ifdef __cplusplus
// 64 bit overloads for the normal names
// Aligns v up, alignment must be a power of two.
static RADINLINE S64 rrAlignUp(const S64 v, const SINTa alignment)
{
    return rrAlignUp64(v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE S64 rrAlignDown(const S64 v, const SINTa alignment)
{
    return rrAlignDown64(v,alignment);
}

static RADINLINE S32 rrAlignUp(const S32 v, const SINTa alignment)
{
    return rrAlignUp32((U32)v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE S32 rrAlignDown(const S32 v, const SINTa alignment)
{
    return rrAlignDown32((U32)v,alignment);
}

static RADINLINE U32 rrAlignUp(const U32 v, const SINTa alignment)
{
    return rrAlignUp32((U32)v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE U32 rrAlignDown(const U32 v, const SINTa alignment)
{
    return rrAlignDown32((U32)v,alignment);
}

static RADINLINE U64 rrAlignUp(const U64 v, const SINTa alignment)
{
    return rrAlignUp64(v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE U64 rrAlignDown(const U64 v, const SINTa alignment)
{
    return rrAlignDown64(v,alignment);
}

static RADINLINE SINTa rrAlignUpA(const SINTa v, const SINTa alignment)
{
    return RR_STRING_JOIN(rrAlignUp,RAD_PTRBITS)((RR_STRING_JOIN(S,RAD_PTRBITS))v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE SINTa rrAlignDownA(const SINTa v, const SINTa alignment)
{
    return RR_STRING_JOIN(rrAlignDown,RAD_PTRBITS)((RR_STRING_JOIN(S,RAD_PTRBITS))v,alignment);
}

static RADINLINE UINTa rrAlignUpA(const UINTa v, const SINTa alignment)
{
    return RR_STRING_JOIN(rrAlignUp,RAD_PTRBITS)((RR_STRING_JOIN(U,RAD_PTRBITS))v,alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE UINTa rrAlignDownA(const UINTa v, const SINTa alignment)
{
    return RR_STRING_JOIN(rrAlignDown,RAD_PTRBITS)((RR_STRING_JOIN(U,RAD_PTRBITS))v,alignment);
}

template <typename T>
static RADINLINE T * rrAlignUpPointer(T * v, const SINTa alignment)
{
    return (T *) rrAlignUpA((UINTa)v,alignment);
}

template <typename T>
static RADINLINE T * rrAlignDownPointer(T * v, const SINTa alignment)
{
    return (T *) rrAlignDownA((UINTa)v,alignment);
}

template <typename T>
static RADINLINE bool rrIsAligned(T v, const SINTa alignment)
{
    return rrAlignDown(v,alignment) == v;
}

template <>
RADINLINE bool rrIsAligned<SINTa>(SINTa v, const SINTa alignment)
{
    return rrAlignDownA(v,alignment) == v;
}

template <>
RADINLINE bool rrIsAligned<UINTa>(UINTa v, const SINTa alignment)
{
    return rrAlignDownA(v,alignment) == v;
}

template <typename T>
static RADINLINE bool rrIsAlignedPointer(T * v, const SINTa alignment)
{
...
// use intrinsics when available to avoid calling into libc funcs
#if defined(__RADSSE2__)
static inline S32 rr_round_to_int(float const x)
{
    return _mm_cvtss_si32(_mm_set_ss(x));
}
#elif defined(__RADARM64__) && defined(__RADNEON__)
static inline S32 rr_round_to_int(float const x)
{
	float32x2_t temp_flt = vdup_n_f32(x);
	int32x2_t temp_int = vcvtn_s32_f32(temp_flt);
	return vget_lane_s32(temp_int, 0);
}
#elif __STDC_VERSION__ >= 199901L || __cplusplus >= 200103L || _MSC_VER >= 1920
