...
static RADFORCEINLINE SINTa getmatchlen_mml1_forceinline(const U8 * p1,const U8 *p2,const U8 * ptrend)
{
    ...
    while ( (UINTa)ptr_big <= (UINTa)ptr_big_end )
	{
		...
		{
			return GetNumBytesZeroNeverAllR(diff) + rrPtrDiff(ptr_big - p1);
		}
    }
    
	{
		// close to the end; just step bytes to finish 
		SINTa len = rrPtrDiff(ptr_big - p1);
	...
	SINTa len = 0;
	SINTa maxlen = rrPtrDiff(ptrend - p1);
...
{
	return (int)getmatchlen_mml1_forceinline(p1,p2,ptrend);
}
...
{
	// The "first 2 bytes match" check knocks out most candidates,
	// so do that directly before we do anything else.
	...
	if ( diff )
	{
		return ( diff > 0x000000FF ) ? 2 : 3;
	}

#else

	if ((U16)p1_32 != (U16)p2_32) return 0;
	U32 diff = p1_32^p2_32;

	if ( diff )
	{
		return ( diff & 0x00FFFFFF ) ? 2 : 3;
	}
...
static RADFORCEINLINE bool havematch_mml2_one32(U32 p1_32,const U8 * p1,const U8 *p2,const U8 * ptrend,int *poutml)
{
    RR_ASSERT( p1 > p2 );
	U32 p2_32 = RR_GET32_NATIVE_UNALIGNED(p2);
	...
	U32 diff = p1_32^p2_32;
	if ( diff > 0x0000FFFF ) return false; // difference in the first two bytes

	if ( diff )
	{
		*poutml = ( diff > 0x000000FF ) ? 2 : 3;
		return true;
	}

#else

	if ((U16)p1_32 != (U16)p2_32) return false;
	U32 diff = p1_32^p2_32;

	if ( diff )
	{
		*poutml = ( diff & 0x00FFFFFF ) ? 2 : 3;
		return true;
	}

#endif

	*poutml = getmatchlen_after4(p1,p2,ptrend);
	return true;
}
...
// havematch_better : tries to find a match candidate longer than prevml
static RADFORCEINLINE bool havematch_better(const U8 * p1,const U8 *p2,const U8 * ptrend,SINTa prevml,SINTa *poutml)
{
	// Check if it's even possible to beat the old match first: if we're near the end,
	// there might not be a legal match long enough
	if ( prevml >= rrPtrDiff(ptrend - p1) )
		return false;

	// If the byte at prevml doesn't match, we also can't beat existing
	if ( p1[prevml] != p2[prevml] )
		return false;

	// Worth trying to score the match
	SINTa len = getmatchlen_mml1_forceinline(p1,p2,ptrend);
	if ( len <= prevml )
		return false;
