...
class SuffixTrie2MatchFinder : public IncrementalMatchFinder
{
	...
	rrArenaArray<int> vfirstbytes;
	rrArenaArray<rrArenaArray<SuffixNode> > node_chunks; // SuffixNodes are allocated in big chunks
...
void SuffixTrie2MatchFinder::LRMJob(int chunkStart, int chunkEnd)
{
	...
	{
		SINTa lrm_offset;
		S32 len = LRMScannerWindowed_FindMatchAndRoll(&scanner, ubuf + i, ubuf + size, &lrm_offset);
		lrm_matches.emplace_back(len, S32_checkA(lrm_offset));
	}
}
...
{
	int node_chunk_count = (m_size + CHUNK_MASK) >> CHUNK_SHIFT;
	int chunk_id = node >> CHUNK_SHIFT;
	RR_ASSERT(chunk_id < node_chunk_count);
	RR_ASSERT(node_chunks[chunk_id].data() == NULL);
	RR_UNUSED_VARIABLE(node_chunk_count);

	node_chunks[chunk_id].Allocate(scratch_arena, CHUNK_SIZE, RR_CACHE_LINE_SIZE);
}

SuffixTrie2MatchFinder::SuffixTrie2MatchFinder(const U8 * ubuf, int size, int startRecordingPos, LRMSet * lrms, OodleLZ_Jobify jobify, void * jobify_userPtr, int firstbytes, rrArenaAllocator *scratch)
{
	...
	ChildrenSparse1_pool.AddScratch(scratch);
	ChildrenSparse2_pool.AddScratch(scratch);
	ChildrenSparseI_pool.AddScratch(scratch);
	ChildrenFull256_pool.AddScratch(scratch);
	...
	vfirstbytes.Allocate(scratch, firstbytes_count);
	vfirstbytes.Zero(firstbytes_count);

	int node_chunk_count = (size + CHUNK_MASK) >> CHUNK_SHIFT;
	node_chunks.Allocate(scratch, node_chunk_count);
	node_chunks.Zero(node_chunk_count);
...
void SuffixTrie2MatchFinder::Release()
{
	if (scratch_arena)
	{
...
void SuffixTrie2MatchFinder::ReportMaxLenMatch(UnpackedMatchPair * matchPairs, int maxPairs, int count)
{
	...
	for(int i = 0; i < count; i++)
	{
		matchPairs[0].length = size - i;
		matchPairs[0].offset = offs;
...
int SuffixTrie2MatchFinder::ProcessChunk(int chunkSize, UnpackedMatchPair * matches, int maxPairs)
{
	...
	rrArenaArray<SuffixNode>* pnode_chunks = node_chunks.data();
	...
	if (maxml_offs)
	{
		if (matches)
		{
			ReportMaxLenMatch(matches, maxPairs, chunkEnd - chunkStart);
			pos = chunkEnd;
		}
	...
	else
	{
		...
		for(pos = chunkStart; pos < chunkEnd; pos++)
		{
			...
			if ( matches )
			{
				...
				// Position to beat (offsets are shorter than current best when pos > posToBeat)
				int posToBeat = pos - curoff;

				// get shorter matches :
				// also update parent->ptr so we store the most recent location of this substring
				#ifdef LIMIT_PARENT_STEPS
				int parent_steps_left = LIMIT_PARENT_STEPS;
				#endif
				while( parent != 0 )
				{
					SuffixNode * n = &NODE(parent);

					// Only interesting if at a lower offset (higher pos) than current;
					// offset==previous is typical due to ptr bubble up
					if (n->pos > posToBeat && numPairs < maxPairs)
					{
			...
			if ( 0 )
			{
				...
				*/
				...
				if ( matches )
				{
					UnpackedMatchPair * matchPairs = matches + (pos - chunkStart)*maxPairs;
					// keep reporting that match until the end of the chunk
					ReportMaxLenMatch(matchPairs, maxPairs, chunkEnd - pos);
				}
...
SuffixTrie2MatchFinder::~SuffixTrie2MatchFinder()
{
	...
	int node_chunk_count = (m_size + CHUNK_MASK) >> CHUNK_SHIFT;
	for (int i = 0; i < node_chunk_count; i++)
	{
		node_chunks[i].Release(scratch_arena, CHUNK_SIZE, RR_CACHE_LINE_SIZE);
	}
	node_chunks.Release(scratch_arena, node_chunk_count);
	vfirstbytes.Release(scratch_arena, firstbytes_mask + 1);

	if (scratch_arena)
	{
		ChildrenSparse1_pool.FreeAllFromArena();
		ChildrenSparse2_pool.FreeAllFromArena();
		ChildrenSparseI_pool.FreeAllFromArena();
		ChildrenFull256_pool.FreeAllFromArena();
	}
}
...
void CountChildren(SuffixTrieSortData * psuffixsortdata)
{
	...
	for LOOP(fb,firstbytes_count)
	{
		...
		{
			CHECK_LEAF(head);
			RR_DURING_ASSERT(total += head ? 1 : 0);
			continue;
		}
		...
		RR_DURING_ASSERT(total += psuffixindices[head].count);
	}
