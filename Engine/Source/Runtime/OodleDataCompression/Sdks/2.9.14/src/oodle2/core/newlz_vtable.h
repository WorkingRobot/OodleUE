...
public:
	...
	newlz_scratchblock() : m_ptr(NULL), m_size(0), m_capacity(0), m_arena(NULL) { }
	...
	*/
	...
	void release()
	{
		if ( m_ptr )
		{
			if (m_arena)
			{
				// make sure to call Free() in case arena wants allocations be freed
				if (m_arena->m_unorderedStart) m_arena->Free(m_ptr, m_capacity);
			}
			else
			{
				OodleFree(m_ptr);
	...
	*/
	...
	void extend(SINTa size, rrArenaAllocator * arena)
	{
		if ( m_capacity > 0 )
		{
			...
			RR_ASSERT_ALWAYS( size <= m_capacity);
			m_size = size;
			return;
		}
		...
		if ( arena )
		{
			// Try to allocate, if we fail we fall through into the path below
			m_ptr = arena->Alloc(size, true);
			m_arena = arena;
		}

		// Regular allocation path or arena alloc failure
		if ( !m_ptr )
		{
			m_ptr = OodleMalloc(size);
			m_arena = NULL;
	...
	// like initial extend() but leaves size to 0
	void reserve(SINTa size, rrArenaAllocator* arena)
	{
		RR_ASSERT(m_capacity == 0);
...
private:
	OODLE_FORBID_CLASS_STANDARDS(newlz_scratchblock);
};
...
struct newlz_vtable
{
	...
	// if need to free pointer from arena in destructor
	rrArenaAllocator* m_free_arena;
	void* m_free_pointer;
	SINTa m_free_size;
	...
	int hash_len_override = 0)
{
	...
	if ( ctmf )
	{
		pvtable->m_free_arena = arena;
		pvtable->m_free_pointer = ctmf;
		pvtable->m_free_size = sizeof(t_ctmf_type);
