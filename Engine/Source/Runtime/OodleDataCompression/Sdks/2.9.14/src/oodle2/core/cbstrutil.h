...
// Output buffer for sprintf with inline memory that auto-allocs on overflow
class StringBuilder
{
	static constexpr SINTa kInlineSize = 128 - 3 * sizeof(SINTa);
	char m_inline_data[kInlineSize];
	char* m_data; // invariant: m_data[m_len] == 0
	SINTa m_len;
	SINTa m_cap;
...
public:
	...
	void clear(); // reset to empty string
	void set(const char* str);
	void append(const char* str);
	void vappendf(const char* fmt, va_list arg);
	...
	inline void vformat(const char* fmt, va_list arg)
	{
		clear();
		vappendf(fmt, arg);
	}
...
private:
	bool ensure_capacity(SINTa target_capacity);
	static char* rrsprintf_callback(char* buf, void* user, int len);
};
