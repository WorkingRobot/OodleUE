...
newlz_vtable::~newlz_vtable()
{
	...
	if (m_free_arena)
	{
		m_free_arena->Free(m_free_pointer, m_free_size);
	}
...
*/
	...
	//
	// 1. No incremental match finder - match finding is done as part of the LZ parse/encode (mf == NULL).
	//    In this case, we have no extra threading going on.
	//
	// 2. Incremental match finder present. As of this writing, this only occurs for the Optimal1 and
	//    higher levels. In this case, we can get some encoder parallelism by splitting match finding
	//    and actual LZ parsing/encoding into two separate threads. For every byte position, we determine
	//    a limited set of what we think are good match candidates in a memory buffer that is populated
	//    by a match finder job and then handed to an encoder/parse job.
	//
	// At least at Optimal1 and Optimal2 levels, match finding and parsing tend to take fairly similar
	// amounts of time, so doing it this way gives a quite reasonable work distribution between two threads.
	//
	// We work one Oodle block (OODLELZ_BLOCK_LEN bytes, 256k as of this writing) at a time. Our match
	// finders are incremental and do both lookups and insertions at the same time. [A pure functional
	// model might seem more appealing, but any more batched scheme such as using suffix arrays has the
	// problem that it also finds matches in future data that hasn't been encountered yet and is not
	// actually available for matches; filtering such invalid matches is possible but annoying. A more
	// serious problem is that it makes it expensive to favor the most recent occurence of each matched
	// substring at the point of matching, which is what we want for LZ matching. Interleaving lookups
	// with insertion just seems to be better for this task.]
	//
	// Because match finding mutates the match finder and is not just a stateless look-up, we can't
	// run multiple match finder tasks at the same time, but we can create a pipeline of match finder
	// jobs, one per Oodle block, and have each depend on the previous one.
	//
	// We then have a second pipeline of LZ parse/encoding jobs, each of which needs the match finding
	// for its block to be complete. These LZ parse/encoding jobs are what writes the output,
	// so they too have serial dependencies on each other, and they also need their corresponding
	// match find to be completed. The resulting dependency graph looks like this:
	//
	//   +--------------+      +--------------+      +--------------+
	//   | Match find 0 | <--- | Match find 1 | <--- | Match find 2 | <--- ...
	//   +--------------+      +--------------+      +--------------+
	//                   ^____ +---------+     ^____ +---------+     ^____
	//                         | Parse 0 | <-------- | Parse 1 |      <--- ....
	//                         +---------+           +---------+
	//
	// As can be seen above, after the first block we can typically keep two jobs in flight in parallel:
	// for the "interior" of the graph, we do match finding for block i+1 while we parse/encode block i.
	// (The first match find and the last parse don't have a companion task).
	//
	// Conceptually, both of these types of tasks can be treated as jobs. In practice, we already have
	// a perfectly fine thread (namely the current thread) to do work for us. Most of the dependencies
	// in the above graph are on the match finders. So we can simplify things a lot by doing the match
	// finding on this thread, and only spawning parse jobs after we've done the corresponding match
	// find. That means no chain of match finding jobs to set up and manage, and no need to track the
	// parse jobs depending on match finding jobs, because we only ever spawn the respective parse jobs
	// once the match finding is done. We do still need to add dependency edges between the parse
	// jobs to ensure they don't run concurrently, however.
	...
	if ( mf )
	{
		...
		// if we have just over a single block, the extra overhead of spinning up a thread
		// and synchronizing with it eats up the gains in processing time
		if ( raw_len <= OODLELZ_BLOCK_LEN + OODLELZ_BLOCK_LEN/16 )
			parse_job_count = 1;
...
struct CreateMatchFinderJob
{
	...
	CreateMatchFinderJob() { RR_ZERO(*this); }
};
...
*/
	...
	job->mf = job->create_fp(job->dictionaryStartPtr, dicPlusRaw, job->dictionary_inset, lrmset, job->jobify, job->jobifyUserPtr, NULL);
...
// We have an incremental match finder set up, but are a single block
// only (so no threading), no LRM, and dictionary base = wholeRawBuf,
// so everything else is easy.
SINTa newlz_compress_vtable_simple_with_mf(newlz_vtable * vtable,
	newlz_encoder_scratch * scratch,
	const U8 * wholeRawBuf,U8 * compBuf,SINTa wholeRawLen,
	rrArenaAllocator * arena)
{
	...
	// Set up the match finder for the block
	RR_ASSERT( vtable->fp_create_match_finder != NULL);
	IncrementalMatchFinder * mf = vtable->fp_create_match_finder(wholeRawBuf,wholeRawLen,0,NULL,pOptions->jobify,pOptions->jobifyUserPtr, arena);

	// Allocate match finder result memory from encoder scratch
	RR_ASSERT( vtable->find_all_matches_num_pairs > 0);
	...
	// Allocate match pairs area enough for match_job_count blocks or the entire data, whichever is shorter
	RR_ASSERT( wholeRawLen <= OODLELZ_BLOCK_LEN );
	SINTa match_space_needed = sizeof(UnpackedMatchPair) * wholeRawLen * vtable->find_all_matches_num_pairs;

	scratch->matches_space.extend(match_space_needed, arena);
	UnpackedMatchPair * matches = (UnpackedMatchPair *)scratch->matches_space.m_ptr;

	// Find matches
	int block_len = (int)wholeRawLen;
	{
		THREADPROFILESCOPE("mf_SimpleBlock");
		SIMPLEPROFILE_SCOPE_N(optimal_mf_simple, block_len);
		mf->ProcessChunk(block_len, matches, vtable->find_all_matches_num_pairs);
	}

	// Do the single-block encode
	SINTa block_comp_space = OodleLZ_GetCompressedBufferSizeNeeded(vtable->compressor,wholeRawLen);
	U8 * compEnd = compBuf + block_comp_space;

	SINTa ret = newlz_compress_vtable_block_outer(vtable,scratch,compBuf,compEnd,
		wholeRawBuf,0 /*block_pos*/,block_len,true /*reset*/,wholeRawBuf /*dict base*/,arena,matches);

	// Clean up the match finder
	mf->Release();
	...
	rrArenaAllocator * arena)
{
	...
	// fp_create_match_finder != NULL; if we're at most a single block, we're not going to do any
	// encoder threading (all our threading is at the block level), so we can use a
	// simpler encoding path that can potentially use encoder scratch memory for its match finder.
	//
	// Also only do this when lrm_casc == NULL and dictionaryBase == wholeRawBuf to rule out all
	// the tricky cases at once.
	if ( wholeRawLen <= OODLELZ_BLOCK_LEN &&
		 lrm_casc == NULL &&
		 dictionaryBase == wholeRawBuf )
	{
		return newlz_compress_vtable_simple_with_mf(vtable,&scratch,wholeRawBuf,compBuf,wholeRawLen,arena);
	}
	...
	// Big picture for what happens here:
	//
	// The data structure sizes for our more precise match finders, suffix tree in particular,
	// are several bytes of data structure per byte indexed in the match finder; a very
	// rough ballpark estimate is about 30 bytes. Eventually these get both seriously
	// unwieldy to search, a problem in terms of memory consumption, and it's also not
	// particularly profitable to find relatively short matches that are many megabytes
	// away, assuming we're even handed enough data for any of this to be relevant in the
	// first place.
	//
	// For truly long-range matches, we have the long-range matcher (LRM) which has larger
	// minimum match lengths, doesn't index every byte (if we have a minimum match length
	// of over 12 bytes, only looking at every 4th byte or similar is fine) and is a lot
	// more memory-efficient.
	//
	// This takes care of the truly distant, and truly long matches, but for medium distances
	// (say ballpark 8-16MB typically), our regular hash tables and suffix trees are fine
	// in principle.
	//
	// We could do a classic sliding window, but especially for the suffix trees, deletions
	// are tricky and fairly expensive, and we'd rather not deal with it.
	//
	// Instead, we periodically rebuild matchers that don't cover more than maxLocalDictionarySize
	// bytes. If all our data fits into one such chunk, no problem. Otherwise, we build
	// trees for chunks generally starting every maxLocalDictionarySize/2 bytes (the exact
	// details don't matter), where each such chunk except the first covers maxLocalDictionarySize
	// bytes, half of them covering the previous chunk and half of them the current chunk (instead
	// of a true sliding window).
	//
	// The first such chunk doesn't have any earlier history to cover; the second and later chunks
	// get pre-seeded to index the contents of the previous mxaLocalDictionarySize/2 bytes before
	// they ever get used for match finding, which proceeds to insert the contents of the active
	// chunk incrementally inserts one byte at a time and does a match find at every position,
	// then saves the results to memory.
	//
	// This find-and-insert loop is part of the block encode and modifies the match finder;
	// the initial pre-seeding of later chunks however is independent of the compression and can be
	// done in parallel.
	//
	// When the jobify mode is set to "aggressive", we do just that: we keep two match finders
	// in flight, one of which covers the "active" chunk currently used by the compressor, while
	// the second is the match finder for the next chunk and gets pre-seeded at the same time.
	// This increases memory use for the match finder by 2x (because now there are two match finders
	// in flight) but increases parallelism.
	//
	// Because we are concerned about memory consumption, we don't want this match finder creation
	// and pre-seeding to get too far ahead. That's why we (at the time of writing) never use more
	// than 2 active match finders, and also add dependencies between the match finder creation jobs
	// to ensure that only one match finder creation task is ever in flight at the same time.
	//
	// This code handles creation and pre-seeding of new match finders as a background task that
	// runs on a job [if aggressive threading is on and we wouldn't immediately start waiting for
	// the job anyway, i.e. only for the second and later sub-chunks]. Actually compressing the
	// chunk is the "foreground" task that executes on our calling thread, although this uses its
	// own jobs too (see newlz_compress_vtable_sub above).
	...
	for (;;)
	{
		...
		while( rawLenRemaining > 0 && next_submit_job - next_retire_job < cmf_job_count )
		{
			...
			// Run the job
			// Note how we set up the dependencies to make the background match finder creation tasks
			// execute as a serial chain. Also, if aggressive threading is off (cmf_job_count == 1) or
			// we're the very first chunk (next_submit_job == 0), we run the job synchronously on this
			// thread instead of farming out to a worker.
