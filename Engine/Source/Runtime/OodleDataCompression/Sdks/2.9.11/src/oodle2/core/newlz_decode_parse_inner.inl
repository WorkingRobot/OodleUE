...
#error Set NEWLZ_PARSE_CAREFUL_OUTPUT to 0 or 1!
	...
	
	{
		...
		#ifdef NEWLZ_PARSE_X64_USE_INLINE_ASM // newer Clangs completely butcher the intended codegen for this, so use inline ASM
			SINTa temp;
			__asm__(
				"cmpq	%[lrl_max], %[lrl]\n"
				...
				: [lrl_max]"i"(NEWLZ_PACKET_LRL_MAX)
			);
		#else
		...
		__asm__(
			"incq %0\n"
			"andq $4, %0\n"
			"addq %0, %1" : "+r"(packet_offset), "+r"(offsets_ptr)
		);
	#else
