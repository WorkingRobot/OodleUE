...
extern void (OODLE_CALLBACK *fp_rrSimpleProf_Pop)(U32 index, U64 push_time, S64 count);
...
static RADINLINE void rrSimpleProf_Pop(U32 index, U64 push_time, S64 count)
{
...
{
public:
	explicit rrScopeProfiler(U32 * pindex, const char * label,S64 count) : m_count(count)
	{
...
#define SIMPLEPROFILE_SCOPE_N(label,count)	\
	...
	::OODLE_NS::rrScopeProfiler RR_STRING_JOIN(label,_scoper) ( &(RR_STRING_JOIN(label,_prof_index)), RR_STRINGIZE(label) , (S64)( count ) );
...
#define SIMPLEPROFILE_SCOPE_SETCOUNT(label,count)	RR_STRING_JOIN(label,_scoper).m_count = (S64)( count )
...
#define SIMPLEPROFILE_SCOPE_STRING(string,count)	\
	...
	::OODLE_NS::rrScopeProfiler prof_scoper( &prof_index, string, (S64)( count ) );
