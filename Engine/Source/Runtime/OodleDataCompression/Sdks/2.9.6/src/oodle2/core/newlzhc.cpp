...
struct newlzhc_histo
{
	...
	// @@ don't really need to be doing sub_and3 if I'm doing sub_andF
	//	can just sum those
	U32 literals_sub_and3[4][256]; // 4k bytes
	U32 literals_sub_andF[16][256];
};
...
static void newLZHC_histo_literals( newlzhc_histo * histo, const U8 * chunk, SINTa pos, SINTa lrl, S32 lastoffset , S32 increment = 1)
{
	...
	// i == 0
	{
		...
		histo->literals_sub_and3[pos&3][sub] += increment;
		#endif
		...
		#ifdef NEWLZ_O2_CONTEXT_COUNT
		int o2cntx = newlz_o2_context( chunk[pos-1], chunk[pos-2], lam );
		histo->literals_o2[ o2cntx ][lit] += increment;
		#endif
...
static void newLZHC_put_literals_multi(int lrl,U8 ** literals_ptr,SINTa lo,SINTa pos,const U8 * chunk)
{
	...
	if ( t_literal_type == NEWLZ_LITERALS_TYPE_O2 )
	{
		while(lrl--)
		{
			int c = newlz_o2_context( chunk[pos-1], chunk[pos-2], chunk[pos - lo] );
			*literals_ptr[c] = chunk[pos];
			literals_ptr[c] ++;
			pos++;
		}
	...
	const OodleLZ_CompressOptions *pOptions)
{
	...
	for(;;) // two hash rows
	{
		...
		for(int d=0;d<newLZHC_CTMF::c_table_depth;d++)
		{
			...
			if ( newLZHC_IsAllowedNormalMatch(len,offset,pOptions) &&
				 ( ( newLZHC_CTMF::c_table_depth == 1 && !newLZHC_CTMF::c_do_second_hash ) || newLZHC_IsNormalMatchBetter(len,offset,bestml,bestoff) ) )
			{
				bestml = len;
				bestoff = offset;
			}
	...
	ENewLZ_MinOffset min_offset)
{
	...
	*/
	...
	else
	{
		...
		for(int i=0;i<4;i++)
		{
			bytes_suband3 += newlzhc_estimate_array_complen(histo->literals_sub_and3[i],256);
		}
		...
		else if ( J_best_complex == J_suband3 )
		{
			...
			for(int i=0;i<4;i++)
			{
				num_literals_and3[i] = rrSumOfHistogram(histo->literals_sub_and3[i],256);
			}
...
#define newLZHC_decode_parse	newLZHC_decode_parse_o2
#include "newlzhc_decode_parse_outer.inl"
#undef newLZHC_decode_parse	
#undef NEWLZHC_DECODE_LITERALS_TYPE			
...
#ifdef NEWLZ_LITERALS_TYPE_O2
#define NEWLZHC_DECODE_LITERALS_TYPE	NEWLZ_LITERALS_TYPE_O2
#define newLZHC_decode_parse	newLZHC_decode_parse_multipacket_o2
#include "newlzhc_decode_parse_outer.inl"
#undef newLZHC_decode_parse	
#undef NEWLZHC_DECODE_LITERALS_TYPE			
#endif // NEWLZ_LITERALS_TYPE_O2
...
#endif
#ifdef NEWLZ_LITERALS_TYPE_O2
	newLZHC_decode_parse_multipacket_o2,
#else
	NULL,
#endif
#ifdef NEWLZ_LITERALS_TYPE_SUBANDF
...
#endif
#ifdef NEWLZ_LITERALS_TYPE_O2
	else if ( chunk_type == NEWLZ_LITERALS_TYPE_O2 )
	{
		return NEWLZHC_O2_ARRAY_COUNT;
	}
#endif
	else
	{
...
struct newlzhc_codecosts
{
	int chunktype;
	#ifdef NEWLZ_O2_CONTEXT_COUNT
	S32 codecost_literal[NEWLZ_O2_CONTEXT_COUNT][256];
	#else
...
static RADINLINE S32 cost_excess_lrl(int excess,
	const newlzhc_codecosts & codecosts)
{
	if ( excess < 255 )
	{
	...
	{
		S32 ret = codecosts.codecost_excess_lrl[255];
		excess -= 255;
...
static RADINLINE S32 cost_excess_ml(int excess,
	const newlzhc_codecosts & codecosts)
{
	if ( excess < 255 )
	{
	...
	{
		S32 ret = codecosts.codecost_excess_ml[255];
		excess -= 255;
		int bits = rrVarBits_CountBits_ExpGolomb(excess,EXCESS_EXPGOLOMB_SHIFT);
		ret += COST_ONE_BIT * bits;
		//ret += COST_PENALTY_EXCESS*2; // extra penalty for excess of excess ?
...
static RADINLINE S32 cost_lo_match_rep0len1(int lrl_for_packet,
		...
		newlzhc_codecosts & codecosts)
{
	...
	case NEWLZ_LITERALS_TYPE_SUBAND3:
		for LOOP(i,4)
		{
			histo_to_codecost(passinfo.literals.literals_sub_and3[i],codecosts.codecost_literal[i],256,0,cost_entropy_threshold);
		}
		break;
	...
	#ifdef NEWLZ_LITERALS_TYPE_O2
	case NEWLZ_LITERALS_TYPE_O2:
		for LOOP(i,NEWLZ_O2_CONTEXT_COUNT)
		{
			histo_to_codecost(passinfo.literals.literals_o2[i],codecosts.codecost_literal[i],256,0,cost_entropy_threshold);
		}
		break;
	#endif
		...
		newlzhc_passinfo & passinfo) // <- read/write argument
{
	...
	#ifdef NEWLZ_LITERALS_TYPE_O2	
	for LOOP(i,NEWLZ_O2_CONTEXT_COUNT)
		optimal_rescale_histo_incremental(passinfo.literals.literals_o2[i],2);
	#endif
	for LOOP(i,4)
		optimal_rescale_histo_incremental(passinfo.literals.literals_sub_and3[i],2);
	for LOOP(i,16)
		...
		newlzhc_passinfo & passinfo) // <- read/write argument
{
	...
	#ifdef NEWLZ_LITERALS_TYPE_O2
	for LOOP(i,NEWLZ_O2_CONTEXT_COUNT)
		optimal_rescale_histo(passinfo.literals.literals_o2[i],2);
	#endif
	for LOOP(i,4)
		optimal_rescale_histo(passinfo.literals.literals_sub_and3[i],2);
	for LOOP(i,16)
		...
		bool literals_same)
{
	...
	#ifdef NEWLZ_LITERALS_TYPE_O2
	for LOOP(i,NEWLZ_O2_CONTEXT_COUNT)
		optimal_rescale_histo2(passinfo1.literals.literals_o2[i],passinfo2.literals.literals_o2[i]);
	#endif
	for LOOP(i,4)
		optimal_rescale_histo2(passinfo1.literals.literals_sub_and3[i],passinfo2.literals.literals_sub_and3[i]);
	for LOOP(i,16)
		...
		const OodleKrakenChunkDeadlines *)
{
	...
	*/
	...
	for(;;)
	{
		...
		S32 cost_offset_8 = cost_offset(8,codecosts);
			...
			pos++)
		{
			...
			S32 cur_matches_cost[NEWLZHC_MATCH_NUM_PAIRS*2+1];
			...
			if ( min_offset == NEWLZ_MIN_OFFSET )
			{
				...
				if ( ml >= mml )
				{
					cur_matches[cur_matches_count].length = ml;
					cur_matches[cur_matches_count].offset = offset;
					cur_matches_cost[cur_matches_count] = cost_offset_8;
					cur_matches_count++;
				}
			}
			...
			for(int lrl_iterator=0;;lrl_iterator++)
			{
				...
				**/
				...
				{
				//SIMPLEPROFILE_SCOPE(inner3_match); // NOTE(fg): this is really heavyweight, uncomment if you need it
