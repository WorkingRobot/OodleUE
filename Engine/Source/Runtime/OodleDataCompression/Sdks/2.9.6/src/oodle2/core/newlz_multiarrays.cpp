...
{	
	...
	**/
	...
	for(int trellis_iter=0;;trellis_iter++)
	{
		...
		RAD_ALIGN(U16,entropyset_cost,16) [64];
		...
		for LOOP(array_i,num_arrays)
		{
			...
			while(ptr<end)
			{
				...
				U32 sym = *ptr++;
				U16 * codelens_for_sym = histo_codelens_transposed + (U32)num_entropysets_padded * sym;
	...
	*/
	...
	for LOOP(i,num_entropysets) histo_use_counts[i].index = i;
