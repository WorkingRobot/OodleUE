...
StringBuilder::StringBuilder()
	: m_data(m_inline_data), m_len(0), m_cap(kInlineSize)
{
	m_data[0] = 0;
}
...
StringBuilder::~StringBuilder()
{
	if (m_data != m_inline_data)
	{
		OodleDeleteArray(m_data, m_cap);
	}
}
...
{
	SINTa len = strlen(str);
	if (ensure_capacity(m_len + len + 1))
	{
		// copy including terminating 0
		memcpy(m_data + m_len, str, len + 1);
		m_len += len;
	}
}

void StringBuilder::vappendf(const char* fmt, va_list arg)
{
	char buf[RR_SPRINTF_MIN];
	rrvsprintfcb(rrsprintf_callback, this, buf, fmt, arg);
	// insert terminating 0
	m_data[m_len] = 0;
}

bool StringBuilder::ensure_capacity(SINTa target_capacity)
{
	if (target_capacity <= m_cap)
	{
		return true;
	}

	// grow
	SINTa new_cap = RR_MAX(target_capacity, m_cap * 2);
	char* new_data = OodleNewArray(char, new_cap);
	if (!new_data)
	{
		return false;
	}

	memcpy(new_data, m_data, m_len + 1);
	if (m_data != m_inline_data)
	{
		OodleDeleteArray(m_data, m_cap);
	}
...
char* StringBuilder::rrsprintf_callback(char* buf, void* user, int len)
{
	StringBuilder* self = (StringBuilder*)user;
	
	// Reserve space for new chars including a trailing 0
	if (self->ensure_capacity(self->m_len + len + 1))
	{
		// ...but don't put in a trailing 0 just yet
		memcpy(self->m_data + self->m_len, buf, len);
		self->m_len += len;
	}
