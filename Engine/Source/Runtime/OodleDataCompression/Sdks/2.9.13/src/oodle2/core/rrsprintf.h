...
#if __SANITIZE_ADDRESS__ || HAS_FEATURE(address_sanitizer)
#define ADDRESS_SANITIZER 1
#elif defined(__RADFUZZ__)
#define ADDRESS_SANITIZER 1
#endif

#ifdef ADDRESS_SANITIZER

// no rrsprintf with ADDRESS_SANITIZER

#include <stdio.h>

#define rrvsprintf vsprintf
#define rrvsnprintf vsnprintf
#define rrsprintf sprintf
#define rrsnprintf snprintf

#else // not ADDRESS_SANITIZER
...
integers in binary: "%b" for 256 would print 100000000.

This function always outputs 8-bit output, but if you would like to
pass in %S wide char (16-bit) unicode strings, you can define the option
RR_SPRINTF_NEED_WCHAR, and we will convert strings passed to %S into
UTF-8.
...
NOTES:
======
This code casts back and forth between char* and U32* - it's really
more of a memory writing function like memcpy or strcpy.  I've tested
with tons of GCCs and Clangs, and all of the MSVCs, but it still may
behave oddly with strict aliasing turned on.

Another technique it uses for speed is reading up to 3 chars 
beyond the end of a terminating null.  It only does this with 
aligned U32s, so it's impossible for this to crash (all CRT 
strcpys and strlens do the same thing), but you might get 
memory sanitizer false positives.

Finally, there are a few places we do read U32s unaligned, but 
this can be turned off with the RR_SPRINTF_NOUNALIGNED define.

This code is written primarily for speed, so it's a bit gross
in places, but if there are simple fixes for compiler warning
and such, send them over.
*/
...
#if defined(_MSC_VER)
#include <sal.h>
#define RRSPRINTF_FORMAT_STRING_ARG _Printf_format_string_
#define RRSPRINTF_FORMAT_STRING_DECL(fmt_arg, check_after)
#elif defined(__GNUC__)
#define RRSPRINTF_FORMAT_STRING_ARG
#define RRSPRINTF_FORMAT_STRING_DECL(fmt_arg, check_after) __attribute__((format(printf, fmt_arg, check_after)))
#else
#define RRSPRINTF_FORMAT_STRING_ARG
#define RRSPRINTF_FORMAT_STRING_DECL(fmt_arg, check_after)
#endif
...
RRPUBLIC_DEF int RR_SPRINTF_DECORATE( sprintf ) ( char * buf, RRSPRINTF_FORMAT_STRING_ARG char const * fmt, ... ) RRSPRINTF_FORMAT_STRING_DECL(2,3);
RRPUBLIC_DEF int RR_SPRINTF_DECORATE( snprintf )( char * buf, int count, RRSPRINTF_FORMAT_STRING_ARG char const * fmt, ... ) RRSPRINTF_FORMAT_STRING_DECL(3,4);
...
#ifndef rUINTa 
#if defined(__ppc64__) || defined(__aarch64__) || defined(_M_ARM64) || defined(_M_X64) || defined(__x86_64__) || defined(__x86_64) || defined(__wasm64__)
#define rUINTa rU64
#else
#define rUINTa rU32
#endif
    ...
    switch( f[0] )
    {
      ...
      case 'S':
        {
          ...
          s = 0 ; sn = 0;
          // get the string
          sh = va_arg(va,unsigned short*); if (sh==0) { s = (char*)"null"; goto asciistr; }
          // get the length
          l = 0;
          snh = sh;
          for(;;)
          {
            rU32 v = snh[0];
            if ((v==0)||((pr>=0)&&(l>=(rU32)pr))) break;
            if ((v>=0xd800) && (v<0xe0000)) ++snh;
            ++l; ++snh;
          }
          lead[0]=0; tail[0]=0; pr = 0; dp = 0; cs = 0;
          fl|=WC;
      ...
      case 'd': // integer
        ...
        for(;;)
        {
          ...
          if (n64==0) { if ((s!=(num+NUMSZ)) && (s[0]=='0')) ++s; break; }
       ...
       scopy: 
        ...
        if ( fl & WC )
        {
          n = l; while (n) { rU32 v; chk_cb_buf(4); v=sh[0]; if ((v>=0xd800) && (v<0xe0000)) { v=((v-0xd800)<<10)+(sh[1]-0xdc00)+0x100000; ++sh; } if (v<0x80) {*bf++=(char)v;} else if (v<(1<<11)) {bf[0]=0xc0|(char)(v>>6);bf[1]=0x80|(char)(v&0x3f);bf+=2;} else if (v<(1<<16)) {bf[0]=0xe0|(char)(v>>12);bf[1]=0x80|(char)((v>>6)&0x3f);bf[2]=0x80|(char)(v&0x3f);bf+=3;} else if (v<(1<<21)) {bf[0]=0xf0|(char)(v>>18);bf[1]=0x80|(char)((v>>12)&0x3f);bf[2]=0x80|(char)((v>>6)&0x3f);bf[3]=0x80|(char)(v&0x3f);bf+=4;} else { *bf++='!'; } --n; ++sh; } chk_cb_buf(1);
        }
        else  // skip the next clause, if we copied unicode 16s
#endif        
...
static char * rrclampcallback( char * buf, void * user, int len )
{
  ...
  return ( c->count >= RR_SPRINTF_MIN ) ? c->buf : c->tmp; // go direct into buffer if you can, otherwise use tmp
}
...
RRPUBLIC_DEF int RR_SPRINTF_DECORATE( vsnprintf )( char * buf, int count, char const * fmt, va_list va )
{
  ...
  wrote = RR_SPRINTF_DECORATE( vsprintfcb )( rrclampcallback, &c, rrclampcallback(0,&c,0), fmt, va );
  
  // zero-terminate, if they sent us a buffer
  if ( ( buf ) && ( count ) )
  {
    int l = wrote;
