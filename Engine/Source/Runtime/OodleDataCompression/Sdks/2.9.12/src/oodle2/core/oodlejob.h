...
void OodleJob_WaitAll( U64 * handles, SINTa num, void * user_ptr );
...
// Helper class to hold a job functor with associated context data
template<typename JobFunctor>
class OodleJobFunctorContext
{
	JobFunctor func;

public:
	explicit OodleJobFunctorContext(JobFunctor&& target) : func(move(target)) {}

	// t_fp_Oodle_Job compatible callback that invokes the function then destroys the context
	// pass an instance as ctx
	static void OODLE_CALLBACK invoke_and_destroy(void * ctx)
	{
		OodleJobFunctorContext * me = (OodleJobFunctorContext *)ctx;
		me->func();
		OodleDelete(me);
	}
};

template<typename JobFunctor,typename t_int_type>
class OodleJobFunctorContextI
{
	const JobFunctor & m_func; // note: const ref, does not move lambda, for use in ParallelFor
	t_int_type m_index;

public:
	explicit OodleJobFunctorContextI(const JobFunctor& target,t_int_type index) : m_func(target), m_index(index) {}

	// t_fp_Oodle_Job compatible callback that invokes the function then destroys the context
	// pass an instance as ctx
	static void OODLE_CALLBACK invoke_and_destroy(void * ctx)
	{
		OodleJobFunctorContextI * me = (OodleJobFunctorContextI *)ctx;
		me->m_func(me->m_index);
		OodleDelete(me);
	}
};

// run a functor async on the plugin job system
template<typename JobFunctor>
static RADINLINE U64 OodleJob_Run(JobFunctor&& functor, U64 * dependencies, int num_dependencies, void * user_ptr)
{
	using Context = OodleJobFunctorContext<JobFunctor>;
	Context * context = OodleNewT(Context)((move(functor)));
	return OodleJob_Run(Context::invoke_and_destroy, (void*)context, dependencies, num_dependencies, user_ptr);
}

// as above, but can run in-thread if requested by num_job_threads == OODLEJOB_DISABLE
template<typename JobFunctor>
static RADINLINE U64 OodleJob_Run_MaybeSingleThreaded(JobFunctor&& functor, U64 * dependencies, int num_dependencies, void * user_ptr, int num_job_threads)
{
	using Context = OodleJobFunctorContext<JobFunctor>;
	Context * context = OodleNewT(Context)((move(functor)));
	return OodleJob_Run_MaybeSingleThreaded(Context::invoke_and_destroy, (void*)context, dependencies, num_dependencies, user_ptr, num_job_threads);
}
...
OodleJobWaitSet * OodleJobWaitSet_Create(U64 * handles, SINTa nhandles, void * user_ptr);
...
OodleJobWaitSet * OodleJobWaitSet_CreateChained(OodleJobWaitSet * parent, U64 * handles, SINTa nhandles, void * user_ptr);
