...
{
	OodleJobWaitSet * m_parent = nullptr;
	void * m_user_ptr = nullptr;
	SINTa m_nhandles = 0; // number of original handles to wait on
	vector_st<U64,32> m_all_handles; // all handles including our temps

	void init(OodleJobWaitSet * parent, U64 * handles, SINTa nhandles, void * user_ptr);
	U64 root_handle() const;
	void wait();
};

void OodleJobWaitSet::init(OodleJobWaitSet * parent_wait_set, U64 * handles, SINTa num_raw_handles, void * user_ptr)
{
	SINTa nhandles = 0;

	for (SINTa i=0; i < num_raw_handles; ++i)
	...
	for (SINTa i=0,j=0; i < num_raw_handles; ++i)
	{
		if (handles[i] != 0)
	...
	for ( ;; )
	{
		...
		SINTa parent_count = t - level_start;
		if ( parent_count <= 1 )
			break;
		...
		{
			SINTa n = RR_MIN(parent_count,OODLE_JOB_MAX_DEPENDENCIES);
...
void OodleJobWaitSet::wait()
{
	...
	for (SINTa i = m_all_handles.sizea() - 1; i >= 0; i--)
	{
...
**/
void OodleJob_WaitAll( U64 * handles, SINTa nhandles, void * user_ptr )
{
