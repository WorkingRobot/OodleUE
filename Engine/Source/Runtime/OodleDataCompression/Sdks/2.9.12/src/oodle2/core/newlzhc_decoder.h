...
// NOTE(fg): On x64, newer Clangs really butcher some of the code in the parse to advance pointers
// (to the extent of 9%+ slowdowns in the decode on some platforms just from extra instructions executed).
// Work around this by using inline ASM for the affected targets on x64/Clang, at least for the time being.
// (written 2023-09-25)
#if defined(__RADX64__) && defined(__clang__)
#define NEWLZHC_PARSE_X64_USE_INLINE_ASM
#endif
