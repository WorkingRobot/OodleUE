...
RR_PACKED_STRUCT is a tight-packed sruct on all platforms
...
#elif defined(__clang__) && defined(__RADX64__)

// Clang doesn't reliably turn the shift form into rotates if the amount
// isn't constant (bugs filed, WIP...).

static RADFORCEINLINE U32 rrRotlVar32(U32 x, U32 k) { __asm__("roll %%cl, %k0" : "+r"(x) : "c"(k)); return x; }
static RADFORCEINLINE U32 rrRotrVar32(U32 x, U32 k) { __asm__("rorl %%cl, %k0" : "+r"(x) : "c"(k)); return x; }

#define RR_ROTL32(u32,num) (__builtin_constant_p((num)) ? ( ( (U32)(u32) << (num) ) | ( (U32)(u32) >> (32 - (num))) ) : rrRotlVar32((u32),(num)))
#define RR_ROTR32(u32,num) (__builtin_constant_p((num)) ? ( ( (U32)(u32) >> (num) ) | ( (U32)(u32) << (32 - (num))) ) : rrRotrVar32((u32),(num)))

#elif defined(__GNUC__)

  // Compiler turns this into rotate correctly :
  #define RR_ROTL32(u32,num)  ( ( (u32) << (num) ) | ( (u32) >> (32 - (num))) )
  #define RR_ROTR32(u32,num)  ( ( (u32) >> (num) ) | ( (u32) << (32 - (num))) )
...
// Clang doesn't reliably turn the shift form into rotates if the amount
// isn't constant with some older version (bugs have been filed and since
// fixed, need to check when we can remove the workaround)

static RADFORCEINLINE U64 rrRotlVar64(U64 x, U32 k) { __asm__("rolq %%cl, %0" : "+r"(x) : "c"(k)); return x; }
static RADFORCEINLINE U64 rrRotrVar64(U64 x, U32 k) { __asm__("rorq %%cl, %0" : "+r"(x) : "c"(k)); return x; }

#define RR_ROTL64(u64,num) (__builtin_constant_p((num)) ? ( ( (u64) << (num) ) | ( (u64) >> (64 - (num))) ) : rrRotlVar64((u64),(U32)(num)))
#define RR_ROTR64(u64,num) (__builtin_constant_p((num)) ? ( ( (u64) >> (num) ) | ( (u64) << (64 - (num))) ) : rrRotrVar64((u64),(U32)(num)))

#elif defined(__GNUC__)

  // Compiler turns this into rotate correctly :
#define RR_ROTL64(u64,num)  ( ( (u64) << (num) ) | ( (u64) >> (64 - (num))) )
#define RR_ROTR64(u64,num)  ( ( (u64) >> (num) ) | ( (u64) << (64 - (num))) )
...
// RAD_HINT_ALIGNED_PTR tells the compiler how a given pointer is aligned
//  it returns a pointer with the alignment hint attached
...
//  RAD_HINT_ALIGNED_PTR(U8 *,aligned_ptr,ptr,16,0);
//	now use aligned_ptr

#if __RAD_GCC_VERSION__ >= 40700
#define RAD_HINT_ALIGNED_PTR(type,name,from,alignment,offset)		type name = (type)__builtin_assume_aligned(from,alignment,offset); RR_ASSERT( ((UINTa)(name) & ((alignment)-1)) == (UINTa)(offset) )
#else
#define RAD_HINT_ALIGNED_PTR(type,name,from,alignment,offset)       type name = (type)(from); RADASSUME( ((UINTa)(name) & ((alignment)-1)) == (UINTa)(offset) )
#endif
