...
// Initializes table of minimum match lengths given Clz32 of the
// offset.
// off24mml is a constant for Mermaid/Selkie
// normal_mml can be 3 or 4 for Optimal/Normal parses, or overridden by user
static void newLZF_InitMinLenTab(int minLenTab[32], int normal_mml, int off24mml, const OodleLZ_CompressOptions * pOptions)
{
	...
	for(int i=0;i<10;i++) minLenTab[i] = off24mml_4M;

	// offset < 4M :
	minLenTab[10] = 
	minLenTab[11] = off24mml_1M;
	
	// offset < 1M
	minLenTab[12] = 
	minLenTab[13] =
	minLenTab[14] =
	minLenTab[15] = off24mml;
	
	// offset < 64k
	for(int i=16;i<32;i++) minLenTab[i] = normal_mml;
	...
	minLenTab[29] =
	minLenTab[30] =
	minLenTab[31] = 256*1024; // impossible
	*/
	
	/*	
	static const int minLenTab_size = 32;
	static const S32 minLenTab[] = 
	{
			32,      32,      32,      32,      32,      32,      32,      32,
			32,      32,      14,      14,      10,      10,      10,      10,
			 4,       4,       4,       4,       4,       4,       4,       4,
			 4,       4,       4,       4,       4,       4,       4,       4
	};
	...
	*/
	...
	//rrPrintfS32Array(minLenTab,32,"minLenTab",8,8);
	
	if ( pOptions->farMatchOffsetLog2 > 0 )
	{
		// NOTE: farMatch options can only make the constraint *stronger* (longer ml) than normal

		// clz = 31 - offLeadingBit
		int last_clz = 31 - pOptions->farMatchOffsetLog2;
		for(int i=0;i<=last_clz;i++)
		{
			minLenTab[i] = RR_MAX(minLenTab[i],pOptions->farMatchMinLen);
		}
	}
	
}

static RADINLINE bool newLZF_IsAllowedNormalMatch(int ml,int off, const int minLenTab[32])
{
	RR_ASSERT( off >= NEWLZF_MIN_OFFSET );
	return ml >= minLenTab[rrClz32(off)];
}
...
static RADFORCEINLINE bool newLZF_get_match_heuristic(match *pmatch,
	CTMF<t_hashtype,t_table_depth_bits,t_second_hash_len,t_mml> * ctmf,
	SINTa step, S32 lastoffset, const U8 * ptr, const U8 * ptr_matchend, int mml, const int minLenTab[32],
	S32 dictionarySize, const U8 * literals_start )
{
	...
	if ( sizeof(t_hashtype) == 2 )
	{
		...
		for(int d=0;d<newLZF_CTMF::c_table_depth;d++)
		{
			...
			// force all offsets >= 8
			offset = RR_MAX(offset,NEWLZF_MIN_OFFSET);
			
			const U8 * vs_ptr = ptr - offset;
			
			//S32 len = getmatchlen_mml4_one32_better(ptr32,ptr,vs_ptr,ptr_matchend,prevml);
			...
			RR_ASSERT( newLZF_IsAllowedNormalMatch(len,(int)offset,minLenTab) );
			
			// @@!! could remove newLZF_IsNormalMatchBetter here
	...
	else
	{
		...
		for(;;) // two hash rows
		{
			...
			for(int d=0;d<newLZF_CTMF::c_table_depth;d++)
			{
				...
				if ( offset >= dictionarySize )	continue;
				...
				if ( newLZF_IsAllowedNormalMatch(len,(int)offset,minLenTab)
				&& ( newLZF_CTMF::c_table_depth == 1 || newLZF_IsNormalMatchBetter(len,(int)offset,bestml,bestoff) ) 
				)
				{
					bestml = len;
					bestoff = (S32)offset;
	...
	RR_ASSERT( bestml < mml || newLZF_IsAllowedNormalMatch(bestml,bestoff,minLenTab) );
	...
	S32 dictionarySize, const U8 * literals_start )
{
	...
	// -> Jaguar says yes
	{
		...
		RR_PREFETCHRW_CL(ctmf->m_hash1 + h1n);
		if ( newLZF_CTMF::c_do_hash2 )
		{
			U32 h2checkn;
			UINTa h2n = ctmf->hash2(ptr + step, h2checkn);
			RR_PREFETCHR_CL(ctmf->m_hash2 + h2n);
		}
	}
		...
		(U32)(cur_absolute_pos - hpos) > 0xFFFF ) )
	{
		...
		{		
			...
			//S32 len = getmatchlen_mml8(ptr,vs_ptr,ptr_matchend);
			S32 len = getmatchlen_mml4_one32(ptr32,ptr,vs_ptr,ptr_matchend);
			...
			if ( len >= mml )
			{
				...
				if ( c_is_64k )
				{
					...
					RR_ASSERT( newLZF_IsAllowedNormalMatch(len,offset,minLenTab) );
				...
				{			
					if ( newLZF_IsAllowedNormalMatch(len,offset,minLenTab) )
					{
						bestml = len;
						bestoff = offset;
					}
	...
	if ( newLZF_CTMF::c_do_hash2 )
	{
		...
		{	
			...
			if ( offset >= NEWLZF_MIN_OFFSET && offset < dictionarySize )
			{
				...
				// off24 zone
				// getmatchlen_mml8, but ptr_matchend is too close
				S32 len = getmatchlen_mml4(ptr,vs_ptr,ptr_matchend);
	...
	RR_ASSERT( bestml == 0 || newLZF_IsAllowedNormalMatch(bestml,bestoff,minLenTab) );
	...
	const OodleKrakenChunkDeadlines *)
{
	...
	*pJ = LAGRANGE_COST_INVALID;
	...
	int minLenTab[32];
	newLZF_InitMinLenTab(minLenTab, mml, selkie ? OFF24_MML_SELKIE : OFF24_MML_MERMAID, pOptions);
	...
	for(const U8 * ptr = start_ptr; ;)
	{
		...
		for(;;)
		{
			...
			if ( newLZF_get_match_heuristic(&chosen,ctmf,step,lastoffset,ptr,ptr_matchend,mml,minLenTab,dictionarySize,literals_start) )
				break;
		...
		if ( t_do_lazy_parse )
		{
			...
			while(ptr+1<parse_end_ptr)
			{
				...
				match lazy;
				if (
					newLZF_get_match_heuristic(&lazy,ctmf,1,lastoffset,ptr+1,ptr_matchend,mml,minLenTab,dictionarySize,literals_start)
				...
				else if ( t_do_lazy_parse >= 2 ) // lazy2 check
				{
					...
					if ( newLZF_get_match_heuristic(&lazy,ctmf,1,lastoffset,ptr+2,ptr_matchend,mml,minLenTab,dictionarySize,literals_start)
	...
	const OodleKrakenChunkDeadlines *)
{
	...
	*pJ = LAGRANGE_COST_INVALID;
	...
	int minLenTab[32];
	newLZF_InitMinLenTab(minLenTab, mml, selkie_comp ? OFF24_MML_SELKIE : OFF24_MML_MERMAID, pOptions);
	...
	for(int twice=0;twice<2;twice++)
	{
		...
		HashType * hash_table = ctmf->m_hash_table;
				...
				for(;;)
				{
					...
					if ( match32le == ptr32le )
					{
						...
						if ( (U32)(match_offs - NEWLZF_MIN_OFFSET) < dictionary_span )
						{
							...
							if (newLZF_IsAllowedNormalMatch(ml,(int)match_offs,minLenTab))
								break;
						}
	...
	for(int twice=0;twice<2;twice++)
	{
		...
		if ( chunk_pos == dump_pos && twice == 0 )
		{
			if ( FILE *f = fopen("dump.dat", "wb") )
			{
				...
				// history
				fwrite(window_base, 1, dump_pos, f);
				...
				rrprintf("initoffs=%u nesco=%u nexc=%u npak=%u nlit=%u nofs=%u\n",
...
#define COST_PENALTY_PACKET			(2*COST_PENALTY_SCALE)  // penalty per packet; fewer packets = less mode switching
...
***/
static RADINLINE match newLZF_get_match_heuristic(
	const newLZF_MatchParseRecord & matches, 
	S32 lastoffset, const U8 * ptr, const U8 * ptr_matchend, 
	int lrl, int mml, const int minLenTab[32],
	...
	)
{
	...
	for(int m=0;m<NEWLZF_MATCH_NUM_PAIRS;m++)
	{
		...
		{
			//offset = NEWLZF_MIN_OFFSET;
			// go up in multiples of offset :
			int stride = offset;
			do { offset += stride; } while ( offset < NEWLZF_MIN_OFFSET );
...
static RADINLINE S32 cost_literals(const U8 * ptr,int lrl,S32 lo,const S32 * codecost_literal, U32 subliteralmask)
{
	RR_ASSERT( lo >= NEWLZF_MIN_OFFSET );
	S32 ret = 0;
	for(int i=0;i<lrl;i++)
	{
		int sub_literal = (U8)(ptr[i] - (ptr[i - lo]&subliteralmask));
		ret += codecost_literal[sub_literal];
	}
	return ret;
}

static RADINLINE S32 cost_7_literals(const U8 * ptr,S32 lo,const S32 * codecost_literal, U32 subliteralmask)
{
	RR_ASSERT( lo >= NEWLZF_MIN_OFFSET );
	S32 ret = 0;
	RR_UNROLL_I_7(0, int sub_literal = (U8)(ptr[i] - (ptr[i - lo]&subliteralmask)); ret += codecost_literal[sub_literal]; );
	return ret;
}

static RADINLINE S32 cost_lo_match(int lrl_for_packet,int ml,const S32 * codecost_packet)
{
	...
	S32 cost = codecost_packet[packet];
		
	return cost;
}

// cost_offset not included
static RADINLINE S32 cost_normal_match(int lrl_for_packet,int ml,const S32 * codecost_packet)
{
	RR_ASSERT( ml >= 1 && ml <= 15 );
	RR_ASSERT( lrl_for_packet >= 0 && lrl_for_packet <= 7 );
...
static RADINLINE S32 cost_long_lrl(int lrl,const S32 * codecost_packet,int * plrl_for_packet)
{
	RR_ASSERT( lrl > 7 );
	...
	{
		S32 lrl_continue_cost = codecost_packet[128 + 7];
		S32 cost = 0;
	...
	int & parse_chunk_end)
{
	...
	//	but it's small
	if ( cost < arrivals[pos+ml].cost )
	{
		arrivals[pos+ml].cost = cost;
		arrivals[pos+ml].ml = ml;
		arrivals[pos+ml].lrl = lrl;
		arrivals[pos+ml].offset = offset;
		arrivals[pos+ml].is_twostep = 0;
...
static RADINLINE S32 try_lo_arrival(newlzf_optimal_arrival * arrivals,
	S32 pos,S32 base_cost,S32 lrl,S32 lrl_for_packet,S32 ml,S32 offset,const S32 * codecost_packet,
	int & parse_chunk_end)
{
	// cost it
	S32 cost = base_cost + cost_lo_match(lrl_for_packet,ml,codecost_packet);				
...
static RADINLINE S32 try_match_arrival(newlzf_optimal_arrival * arrivals,
	...
	int & parse_chunk_end)
{
	S32 cost = base_cost + cost_normal_match(lrl_for_packet,ml,codecost_packet);
...
{			
	...
	int minLenTab[32];
	newLZF_InitMinLenTab(minLenTab, mml, off24mml, vtable->pOptions);
	...
	for(int pos=literals_start;pos<parse_end_pos;)
	{
		...
		match chosen = newLZF_get_match_heuristic(matches[pos],lastoffset,ptr,ptr_matchend,pos-literals_start,mml,minLenTab,dictionaryBase,dictionarySize);
		...
		while(pos+1<parse_end_pos)
		{
			...
			lazy = newLZF_get_match_heuristic(matches[pos+1],lastoffset,ptr+1,ptr_matchend,pos+1-literals_start,mml,minLenTab,dictionaryBase,dictionarySize);
			
			if ( newLZF_LazyMatchDelta(lazy.ml,lazy.off,lazy.IsLO(),chosen.ml,chosen.off,chosen.IsLO()) > 0 )
			{
				// insert a literal :
				ptr++;
				pos++;
			...
			else // if ( t_do_lazy_parse >= 2 ) // lazy2 check
			{
				...
				lazy = newLZF_get_match_heuristic(matches[pos+2],lastoffset,ptr+2,ptr_matchend,pos+2-literals_start,mml,minLenTab,dictionaryBase,dictionarySize);
				
				// IsLazy2Better : check newLZ_LazyMatchDelta > 3 :
				if ( newLZF_LazyMatchDelta(lazy.ml,lazy.off,lazy.IsLO(),chosen.ml,chosen.off,chosen.IsLO()) > NEWLZF_LAZYBETTER_LAZY2 )
				{
					// insert a literal :
					ptr+=2;
					pos+=2;
...
//	if so, then fill *plrl with the distance to the match start ( <= 7)
static RADINLINE bool get_lrl_to_rep(const U8 * ptr, SINTa offset, S32 * plrl)
{
...
static RADFORCEINLINE void mermaid_try_twostep(
	...
	const S32 * codecost_literal,U32 subliteralmask,
	const S32 * codecost_packet,
	int & parse_chunk_end)
{
	S32 twostep_lrl;
	if ( ! get_lrl_to_rep(ptr+ml,offset,&twostep_lrl) )
		return;
		
	if ( pos+ml+twostep_lrl >= parse_end_pos )
		return;
		
	RR_ASSERT( twostep_lrl > 0 && twostep_lrl <= 7 );
	
	const U8 * twostep_rep_ptr = ptr+ml+twostep_lrl;
	
	S32 twostep_ml = getmatchlen_mml1(twostep_rep_ptr,twostep_rep_ptr-offset,ptr_matchend);
...
*/
	...
	if ( twostep_ml > 0 )
	{
		...
		cost += cost_literals(ptr+ml,twostep_lrl,offset,codecost_literal,subliteralmask);
		cost += cost_lo_match(twostep_lrl,twostep_ml,codecost_packet);
		
		int twostep_arrival_pos = pos+ml+twostep_lrl+twostep_ml;
...
static void newLZF_passinfo_to_codecost(
	...
	S32 codecost_offsetL[256],
	S32 codecost_offsetH[256])
{
	const int cost_entropy_threshold = (75 * COST_ONE_BIT)/10; // @@ <- tweaky
	// <- this really should be based on lambda

	histo_to_codecost(passinfo.literal_histo,codecost_literal,256,COST_PENALTY_HUFF,cost_entropy_threshold);
	histo_to_codecost(passinfo.packet_histo,codecost_packet,256,COST_PENALTY_PACKET,cost_entropy_threshold);
	...
	***********/
	...
	histo_to_codecost(passinfo.offsetL_histo,codecost_offsetL,256,0,cost_entropy_threshold);
	histo_to_codecost(passinfo.offsetH_histo,codecost_offsetH,256,0,cost_entropy_threshold);
	...
	if ( HL > 7.5 )
	{
		for LOOP(i,256) codecost_offsetL[i] = COST_ONE_BYTE;
	}
	...
	if ( HH > 7.5 )
	{
		for LOOP(i,256) codecost_offsetH[i] = COST_ONE_BYTE;
	}
...
// this doesn't seem to make a very big difference on Mermaid, not like Kraken
// book1 :    768,771 ->   292,304 =  3.042 bpb =  2.630 to 1 
// book1 :    768,771 ->   292,513 =  3.044 bpb =  2.628 to 1 
// dickens : 10,192,446 -> 3,330,793 =  2.614 bpb =  3.060 to 1 
// dickens : 10,192,446 -> 3,333,350 =  2.616 bpb =  3.058 to 1 
// -> overall on GTS & Silesia there are a few files it helps on
//	  it doesn't seem to ever hurt (encode speed is fine) so turn it on
// -> the reason why it neither hurts speed nor helps compression is
//	  because I only do it for len < 15
#define MERMAID_TRY_ALL_LEN_REDUCTIONS	1
//#define MERMAID_TRY_ALL_LEN_REDUCTIONS	0
	...
	)
{
	...
	newLZF_passinfo_to_codecost(passinfo,codecost_literal,codecost_packet,codecost_offsetL,codecost_offsetH);
	...
	int minLenTab[32];
	newLZF_InitMinLenTab(minLenTab, mml, off24mml, pOptions);

	U32 subliteralmask = (U32)-1;
	if ( greedy_chunktype == NEWLZ_LITERALS_TYPE_RAW )
		subliteralmask = 0;
	...
	for(;;)
	{
			...
			pos++)
		{
			...
			if ( (pos - cheap_preceding_arrival_pos) > 0 )
			{
				...
				U8 sub_literal = (U8)(ptr[-1] - (ptr[-1-lo]&subliteralmask));
				cheap_preceding_arrival_literal_cost += codecost_literal[sub_literal];
			
				// @@ N^2 assert can make debug very slow :
				NEWLZF_ASSERT_HEAVY( cost_literals(ptr - (pos - cheap_preceding_arrival_pos),(pos - cheap_preceding_arrival_pos),arrivals[cheap_preceding_arrival_pos].offset,codecost_literal,subliteralmask) == cheap_preceding_arrival_literal_cost );
				...
				if ( arrivals[pos].cost != RR_S32_MAX )
				{
					...
					if ( arrivals[pos].ml > 1 )
					{
						...
						if ( lrl > 7 )
							cost_lrl = cost_long_lrl(lrl,codecost_packet,&lrl_for_packet);
			...
			if ( arrivals[pos].cost != RR_S32_MAX )
			{
				if ( pos+7 < parse_end_pos )
				{
					...
					S32 base_cost = arrivals[pos].cost;
					...
					//  this was pretty big in Selkie, almost irrelevant here
					if ( ptr[6] == ptr[6-(SINTa)lo] )
					{
						base_cost += cost_literals(ptr,6,lo,codecost_literal,subliteralmask);
						S32 cost = base_cost + codecost_packet[NEWLZF_PACKET_LO_FLAG + 6 + (1<<3)];
						try_arrival(arrivals,pos+6,cost,6,1,lo,parse_chunk_end);
					}
					//else // this else is optional
					{
						// tiny win from trying 7-lrl even when 6+1 is possible
						// with the else :
						// lzt99 : 24,700,820 ->10,309,684 =  3.339 bpb =  2.396 to 1
						// without :
						// lzt99 : 24,700,820 ->10,308,787 =  3.339 bpb =  2.396 to 1 

						base_cost += cost_7_literals(ptr,lo,codecost_literal,subliteralmask);
						S32 cost = base_cost + codecost_packet[NEWLZF_PACKET_LO_FLAG + 7];
						...
						if ( ptr[7] == ptr[7-(SINTa)lo] )
						{	
							cost = base_cost + codecost_packet[NEWLZF_PACKET_LO_FLAG + 7 + (1<<3)];
			...
			UnpackedMatchPair cur_matches[NEWLZF_MATCH_NUM_PAIRS+1];
			...
			for(int m=0;m<NEWLZF_MATCH_NUM_PAIRS;m++)
			{
				...
				if ( ptr+ml > ptr_matchend )
				{
					ml = rrPtrDiff32(ptr_matchend - ptr);
				}
				...
				if ( ! newLZF_IsAllowedNormalMatch(ml,offset,minLenTab) )
					continue;
			...
			if ( 1 )
			{
				...
				// this is in the optimal parse; could do a 3 here
				// but I don't want to encourage MIN_OFFSET matches too much @@??
				ml = getmatchlen_mml4_one32(ptr32,ptr,ptr-offset,ptr_matchend);
			...
			int max_lrl = RR_MIN((pos-parse_chunk_start),7);
			...
			if ( any_escapes )
			{
				...
				{			
					...
					S32 lrl_base_cost = cost_literals(ptr-lrl,lrl,lo,codecost_literal,subliteralmask);
					
					// send an LRL with no match packet :
					lrl_base_cost += codecost_packet[128 + lrl];
				...
				if ( longestml >= NEWLZF_ML_EXCESS )
				{
					...
					RR_ASSERT( escape_ml >= 0 );
					
					// cost here is actually irrelevant because it's a forced skip

					// cost of offset not included
					//S32 cost = base_cost + codecost_packet[packet];											
				...
				else
				{
					...
					for(int m=0;m<cur_matches_count;m++)
					{
						...
						RR_ASSERT( ml < NEWLZF_ML_EXCESS );
						if ( offset <= 0xFFFF ) // only excesses here
							continue;
						...
						mermaid_try_twostep(arrivals,pos,parse_end_pos,ml,lrl,offset,cost,
							ptr,ptr_matchend,codecost_literal,subliteralmask,codecost_packet,parse_chunk_end);
			...
			for(int lrl=0;lrl<=max_lrl;lrl++)		
			{
				...
				/*
				// don't check lo match unless lo changed,
				//	or we found a lower base cost
				// -> no compression penalty if you do both these checks
				//	-> but also no speed gain
				//	-> it's not a cache miss cuz LO is the same, just repeating the same check
				if ( lo != prev_lo || base_cost < lowest_base_cost )
				{
				prev_lo = lo;
				*/
				{
				...
				if ( loml > 0 )
				{
					...
					if ( loml > 15 )
					{
						...
						// make packets for ml 15 steps and add them to cost
						
						// first packet gets the lrl :
						int packet = lrl_for_packet + (15<<3) + NEWLZF_PACKET_LO_FLAG;
						cost += codecost_packet[packet];
						remainingml -= 15;						
						
						// try_arrival after one packet step
						RR_ASSERT( (loml - remainingml) == 15 );
						try_arrival(arrivals,pos,cost,lrl,15,lo,parse_chunk_end);	
						
						while( remainingml > 1 ) // don't go down to remainingml == 0 if last packet has only 1
						{
							int curml= RR_MIN(remainingml,15);
							packet = (curml<<3) + NEWLZF_PACKET_LO_FLAG;
							cost += codecost_packet[packet];
							remainingml -= curml;
							
							// could try_arrival after each packet step
							// -> don't seem to help at all
							//try_arrival(arrivals,pos,cost,lrl,loml - remainingml,lo,parse_chunk_end);	
						}

						RR_ASSERT( remainingml == 0 || remainingml == 1 );
						try_arrival(arrivals,pos,cost,lrl,loml - remainingml,lo,parse_chunk_end);	
		
						//try_lo_arrival(arrivals,pos,base_cost,lrl,lrl_for_packet,15,lo,codecost_packet,parse_chunk_end);
					
						#if 0
						// the fact that 14 is tried here is a mistake, but it helps :
						// Mermaid :
						// with trying 14 :	
						// lzt99 : 24,700,820 ->10,785,659 =  3.493 bpb =  2.290 to 1 
						// without  trying 14 :
						// lzt99 : 24,700,820 ->10,786,072 =  3.493 bpb =  2.290 to 1 
						try_lo_arrival(arrivals,pos,base_cost,lrl,lrl_for_packet,14,lo,codecost_packet,parse_chunk_end);		
						#endif		
					}
					else
					{				
						S32 cost = try_lo_arrival(arrivals,pos,base_cost,lrl,lrl_for_packet,loml,lo,codecost_packet,parse_chunk_end);
											
						mermaid_try_twostep(arrivals,pos,parse_end_pos,loml,lrl,lo,cost,
							ptr,ptr_matchend,codecost_literal,subliteralmask,codecost_packet,parse_chunk_end);
						
						#if MERMAID_TRY_ALL_LEN_REDUCTIONS
						
						for(int tryml=2;tryml<loml;tryml++)
						{
							try_lo_arrival(arrivals,pos,base_cost,lrl,lrl_for_packet,tryml,lo,codecost_packet,parse_chunk_end);
						}
						...
						if ( loml > 2 )
						{
							try_lo_arrival(arrivals,pos,base_cost,lrl,lrl_for_packet,loml-1,lo,codecost_packet,parse_chunk_end);
						}
				...
				// cost matches at all LRL's where the base_cost decreases :
				//  (this doesn't massively reduce the amount of work
				//	 because base_cost should generally be going down as you step backwards)
				...
				for(int m=0;m<cur_matches_count;m++)
				{
					...
					RR_ASSERT( newLZF_IsAllowedNormalMatch(ml,offset,minLenTab) );
					...
					cost += codecost_offsetL[offset&0xFF] + codecost_offsetH[offset>>8];
					
					// try multi-packet steps for longer ml ?
					//	-> yes this is important
					if ( ml >= 15 )
					{
						S32 match_cost = try_match_arrival(arrivals,pos,cost,lrl,lrl_for_packet,15,offset,codecost_packet,parse_chunk_end);
						...
						for(;;)
						{
							...
							match_cost += cost_lo_match(lrl_for_packet,next_ml,codecost_packet);	
					...
					{
						S32 match_cost = try_match_arrival(arrivals,pos,cost,lrl,lrl_for_packet,ml,offset,codecost_packet,parse_chunk_end);
						...
						{
							try_match_arrival(arrivals,pos,cost,lrl,lrl_for_packet,tryml,offset,codecost_packet,parse_chunk_end);
						}
						...
						if ( ml > mml )
						{
							try_match_arrival(arrivals,pos,cost,lrl,lrl_for_packet,ml-1,offset,codecost_packet,parse_chunk_end);
						
							if ( ml > mml+1 )
							{
								try_match_arrival(arrivals,pos,cost,lrl,lrl_for_packet,ml-2,offset,codecost_packet,parse_chunk_end);
			...
			RR_ASSERT( lowest_base_cost != RR_S32_MAX );		
		...
		{			
			...
			for(int pos=final_arrival_pos;pos<=last_arrival_pos;pos++)
			{
				...
				cost += cost_literals(ptr,(chunk_len - pos),lo,codecost_literal,subliteralmask);
		...
		newLZF_passinfo_to_codecost(
			passinfo,
			codecost_literal,codecost_packet,codecost_offsetL,codecost_offsetH);
	...
	)
{
	...
	*/
	...
	for(int pos=start_pos;;pos++)
	{
		...
		//if ( longestml >= NEWLZF_ML_EXCESS ) || any large offsets
		if ( any_escapes )
		{
			...
			for(int lrl=1;lrl<=max_lrl;lrl++)
			{
				if ( lrl_base_cost[lrl] == RR_S32_MAX )
					continue;
				
				S32 base_cost = lrl_base_cost[lrl] + codecost_packet[128 + lrl] + COST_PENALTY_ZEROML_PACKET;

				if ( base_cost < escape_cheapest_base_cost )
				{
					escape_cheapest_base_cost = base_cost;
					escape_cheapest_lrl = lrl;
				}
			}
			/*/
...
//						pOptions,17,21,18,24);
	...
	int hash_len = 4;
	//*
	//if ( ! is_64k ) // for 64k? -> yes newlzf likes this even at 32k chunks
	if ( raw_len > 16384 && raw != NULL ) // @@ not well tweaked; at some small size, don't bother
	{
		// definitely bad on HyperFast3
