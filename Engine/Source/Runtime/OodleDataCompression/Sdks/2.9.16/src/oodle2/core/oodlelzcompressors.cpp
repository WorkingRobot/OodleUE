...
// pass rawLen <= 0 to support any size
...
OOFUNC1 S32 OOFUNC2 OodleLZDecoder_MemorySizeNeeded(OodleLZ_Compressor compressor,SINTa rawLen)
{
	...
	{
		ooLogError("OodleLZDecoder_MemorySizeNeeded: legacy compressors not supported!\n");
	...
	S32 memSize,S32 decSize)
{
	...
	lzhD->decLen = RR_MAX(rawLen, 0); // clamp negative sizes to 0
    lzhD->decPos = 0;
...
OOFUNC1 rrbool OOFUNC2 OodleLZDecoder_Reset(OodleLZDecoder * lzhD, SINTa decPos,SINTa decLen RADDEFAULTX(0))
{
	if ( decLen < 0 )
	{
		ooLogError("May not reset to negative decLen");
		return false;
	}
...
static void UsageWarning_WriteCombiningCheck(const void *ptr,SINTa len)
{
	...
	static const int NUM_RUNS = 5; // number of consecutive slow runs we require to complain. NOTE: update usage warning below
	...
	{
		run_times[i] = oodle_x64_wc_probe(ptr);
		if ( run_times[i] < SLOW_RUN_THRESHOLD )
			return;
	}
	...
	ooLogUsageWarning2("Destination buffer appears to be in uncached " \
		...
		"decompress to graphics memory quickly?\" in the Oodle FAQ. (Probe times %lld,%lld,%lld,%lld,%lld)\n",
...
// Called if a provided input stream is for a legay codec and we don't
// have support for that format plugged in through the legacy VTable.
//
// There are a few possible causes for this error, try to produce as
// helpful an error message as possible.
static void LogLegacyFormatError(const U8 * compBuf, SINTa compBufferSize)
{
	if ( compBufferSize > 0 && compBuf[0] == 0 )
	{
		ooLogError("OodleLZ_Decompress: first byte is zero; this is likely not valid compressed data!\n");
		return;
	}

	// Re-read the header to see if it's an old version to hopefully give a more useful
	// error message.
	if ( compBufferSize >= OODLELZ_BLOCK_HEADER_BYTES_MAX )
	{
		LZBlockHeader header;
		if ( LZBlockHeader_Get(&header,compBuf) )
		{
			// v4 is the first version that supports Kraken, our first "new" codec.
			// Anything pre-v4 has to be legacy and has good odds of not being a valid
			// Oodle stream at all.
			if ( header.version < 4 )
			{
				ooLogError("OodleLZ_Decompress: data is either corrupted, not a valid Oodle stream, or was compressed with an unsupported legacy compressor!\n");
				return;
			}
		}
	}
                                        ...
                                        )
{
	...
	if ( decoder->decLen <= 0 )
	{
		ooLogError("Must call OodleLZDecoder_Reset to set concrete decLen before initiating decode.\n");
		return false;
	}

	if ( decBufferSize < decoder->decLen )
	{
		ooLogError("Decoder buffer size insufficient! (Is %zd, needs to be at least %lld.)\n", decBufferSize, (long long) decoder->decLen);
	}
    ...
    S64 chunkPos = decoder->decPos & (OODLELZ_BLOCK_LEN-1);
    ...
    if ( chunkPos == 0 && decoder->gotHeaderPos != decoder->decPos )
    {
		...
		// NOTE(fg): 2022-Jan-12 legacy/unsupported format check moved to before fuzz
		// safety check since you want that one to be the message you see on invalid/malformed
		// streams.
    ...
    {
		ooLogError("initial block in stream is not RESET\n");
		return false;
    }
        ...
        else // normal LZ
        {
			...
			*/
			...
			else if ( decodeType == RAD_LZ_DECODE_LZB16 )
			{
				...
				gotCodeCompLen = LZB_DecodeOneQuantum(decPtr,decPtrEnd,compPtr,compPtrEnd,qh,decBuf,(SINTr)decBufferSize,dictionaryBase);
			}
			else if ( is_legacy )
			{
				...
				gotCodeCompLen = g_OodleLZLegacyVTable.fp_legacy_OodleLZ_DecodeOneQ(decodeType,legacy_OodleLZDecoder_Get(decoder),
					...
					false,// sliding window support removed
					pos_since_reset,
					checkBuf);
			}
			else
											...
											OodleLZ_Decode_ThreadPhase threadPhase)
{
	...
	if ( compBufferSize <= 0 )
	{
		ooLogError("OodleLZ_Decompress: invalid compressed size (<=0)\n");
	...
	// NOTE: GetAllChunksCompressor returns OodleLZ_Compressor_Count for mixed newlz/non-newlz data
	...
	if ( OodleLZ_Compressor_IsLegacy(compressor) && ! OodleLZLegacyVTable_IsInstalled() )
	{
		// Trying to decode random non-Oodle data is likely to end up here since it's going to
		// read as an old-format header; the old formats had basically no reserved/magic header
		// bits so any random byte value will be decoded as flagging an old compression alg.
		//
		// We have a helper function to look at the data and try and produce a helpful
		// diagnostic in that case for typical scenarios.
		LogLegacyFormatError(compBuf, compBufferSize);
		return OODLELZ_FAILED;		
	}
    ...
    SINTa scratchSize RADDEFAULT(0) )
{
	...
	if ( rawLen <= 0 )
	{
		ooLogError("OodleLZ_Compress: rawLen invalid! (<=0)\n");
		return OODLELZ_FAILED;
	}

	if ( !rawBufV )
	{
		ooLogError("OodleLZ_Compressor: rawBuf is null!\n");
		return OODLELZ_FAILED;
	}

	if ( !compBufV )
	{
		ooLogError("OodleLZ_Compressor: compBuf is null!\n");
		return OODLELZ_FAILED;
	}
...
OOFUNC1 SINTa OOFUNC2 OodleLZ_GetCompressScratchMemBoundEx(
	OodleLZ_Compressor compressor,
	OodleLZ_CompressionLevel level,
...
{
	// Only NewLZ codecs have an estimate
	if ( ! OodleLZ_Compressor_IsNewLZFamily(compressor) )
	{
		return OODLELZ_SCRATCH_MEM_NO_BOUND;
	}
	...
	// If a worst-case estimate exists, always use it
	SINTa worst_case_estimate = OodleLZ_GetCompressScratchMemBound(compressor, level, rawLen, pOptions);
	if ( worst_case_estimate != OODLELZ_SCRATCH_MEM_NO_BOUND )
	{
		return worst_case_estimate;
	}

	// We don't have a worst-case estimate; if that's what was requested, bail.
	if ( boundType == OodleLZ_CompressScratchMemBoundType_WorstCase )
	{
		return OODLELZ_SCRATCH_MEM_NO_BOUND;
	}

	// Okay, we're asked to estimate typical memory use
	...
	// For single chunks at Optimal1, we do have a rough estimate.
	if ( level >= OodleLZ_CompressionLevel_Optimal1 && rawLen <= OODLELZ_BLOCK_LEN )
	{
		// Experimentally, 32MB seems to generally work fine in this case.
		return 32 * 1024 * 1024;

		// Can track typical memory use for the suffix trees at >=Optimal2 etc. here, but it's
		// not clear that having an estimate here for blocks larger than the standard block size
		// is even particularly interesting.
	}
