...
Jan 20, 2026 (FG): remove everything but "Fast" level encode, we keep minimal LZB16 support
    in Oodle for old Kraken stream backwards compat but there's just no reason to lug
	multiple LZB encoders around in every Oodle release
...
#define LZB_AddExcessBW(cp,val)	do { UINTr b = *cp++; \
	if ( b < 192 ) val += b; \
												...
												rrbool isSlidingWindow)
{
	...
	RR_ASSERT_ALWAYS( ! isSlidingWindow );
	RR_ASSERT_ALWAYS( ! fh->IsSlidingWindow() );
	fh->m_windowSize = 0;
	fh->m_windowMask = -1;
...
{	
	...
	fp_Encode_Fast * enc = Encode_LZB_OneChunk_Fast_NoSlide_Sub;
...
/**/
	...
	// Can skip offset chunk past initial 64k; we used to do this but as of this
	// writing (for Jan 20, 2026, Oodle 2.9.16 dev cycle) we no longer care about
	// decode perf for large-chunk LZB since the only remaining use cases of LZB
	// are tiny-chunk LZB16 for legacy Oodle 2 releases that would sometimes
	// emit them for tiny blocks instead of Kraken.
...
{	
	...
	else
	{
		...
		S32 tableBits = GetLZMatchTableBits(rawLen,level,pOptions,12,17,14,19);
