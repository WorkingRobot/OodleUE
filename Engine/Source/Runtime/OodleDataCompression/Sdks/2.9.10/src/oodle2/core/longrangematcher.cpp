					...
					SINTa * pMatchOffset, SINTa matchOffsetLimit)
{
	...
	SINTa ml_must_be_above = LRM_MIN_MATCH_LEN - 1;
	while( it->hash == hash )
	{
		//if ( it->firstfour == firstfour )
		{
			...
			SINTa ml;
			if ( havematch_better(ptr,vs,ptrend,ml_must_be_above,&ml) )
			{
				RR_ASSERT( ml >= best_ml );
				RR_ASSERT( memcmp(ptr,vs,ml) == 0 );

				ml_must_be_above = ml - 1; // -1 since we allow ties
