...
IncrementalMatchFinder * SuffixTrie_CreateMatchFinder(const U8 * ubuf,SINTa sizeA,
		...
		void * jobifyUserPtr,
		rrArenaAllocator *scratch_arena)
{
	...
	SuffixTrie2::SuffixTrie2MatchFinder* mf;
	if (scratch_arena)
	{
		mf = (SuffixTrie2::SuffixTrie2MatchFinder*)scratch_arena->Alloc(sizeof(*mf));
	}
	else
	{
		mf = OODLE_MALLOC_ONE(SuffixTrie2::SuffixTrie2MatchFinder);
	}
	return construct(mf, ubuf, size, startRecordingPos, lrms, jobify, jobifyUserPtr, size <= SuffixTrie1_MaxSize ? 2 : 3, scratch_arena);
}
