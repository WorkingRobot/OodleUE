...
void rrColor_SRGBToLinear(const rrColor32BGRA* src, rrColor4F* dst, int count)
{
	for (int i = 0; i < count; i++)
	{
		dst[i].r = srgb_byte_to_linear_unit_float(src[i].u.r);
...
void rrColor_LinearToSRGB_Reference(const rrColor4F* src, rrColor32BGRA* dst, int count)
{
	for (int i = 0; i < count; i++)
	{
...
void rrColor_LinearToSRGB(const rrColor4F* src, rrColor32BGRA* dst, int count)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		const VecF32x4 InClamped = vmin(vmax(InRGBA, MinValFlt), AlmostOne);
		...
		const Vec128_U8 Packed8 = Packed16.to_uint8_sat();
