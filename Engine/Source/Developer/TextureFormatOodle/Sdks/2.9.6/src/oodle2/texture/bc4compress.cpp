...
// the entire time.
static RADFORCEINLINE Vec128 calc_bc4_pixel_error(const Vec128 & pixels, const Vec128 & pal_entry, const Vec128 & index_bias)
{
	// Calculate |pixels-pal_entry|
	Vec128 diff = _mm_sub_epi16(pixels, pal_entry);
	Vec128 absdiff = _mm_abs_epi16(diff);
	...
	Vec128 clamped_absdiff = _mm_min_epi16(absdiff, _mm_set1_epi16((1 << 13) - 1));

	// Now shift left by 3 and put the index bit in the low 3 bits (also biasing everything by -0x8000 so the values
	// make sense as signed values)
	Vec128 result = _mm_add_epi16(_mm_slli_epi16(clamped_absdiff, 3), index_bias);
	return result;
}

static RADFORCEINLINE Vec128 initial_err_dot_product(const Vec128 & input)
{
	// Un-bias the errors and shift them down
	Vec128 errs = _mm_srli_epi16(_mm_sub_epi16(input, _mm_set1_epi16(-0x8000)), 3); // absolute diff for best index

	// Now compute sum of squared errors and sum between adjacent pairs
	return _mm_madd_epi16(errs, errs);
}
...
static void BC4_FindIndices(const BC4SourceData & values, typename Tblock::Endpoint e0, typename Tblock::Endpoint e1, U32 * pError, BC4Indices * pInds)
{
	...
	Vec128 v_palette = load128u(palette);
	Vec128 v_pixels0 = load128u(&values.values[0]);
	Vec128 v_pixels1 = load128u(&values.values[8]);

	Vec128 v_besterr0 = _mm_set1_epi16(0x7fff); // INT16_MAX (neutral element for min)
	Vec128 v_besterr1 = _mm_set1_epi16(0x7fff);
	Vec128 v_index_bias = _mm_set1_epi16(-0x8000);
	Vec128 v_pal_shuffle = _mm_set1_epi16(0x0100); // 0,1 repeating

	// Loop over palette entries and keep track of the lowest error for each pixel
	for (int index = 0; index < 8; index++)
	{
		// Broadcast palette entry
		Vec128 v_pal_value = _mm_shuffle_epi8(v_palette, v_pal_shuffle); // broadcast current palette entry

		// Update "best possible error" estimates for all 16 pixels
		v_besterr0 = _mm_min_epi16(v_besterr0, calc_bc4_pixel_error(v_pixels0, v_pal_value, v_index_bias));
		v_besterr1 = _mm_min_epi16(v_besterr1, calc_bc4_pixel_error(v_pixels1, v_pal_value, v_index_bias));

		v_pal_shuffle = _mm_add_epi8(v_pal_shuffle, _mm_set1_epi8(2)); // advance to next palette entry
		v_index_bias = _mm_add_epi16(v_index_bias, _mm_set1_epi16(1)); // increment current index
	}

	// Extract the best indices
	Vec128 v_index_mask = _mm_set1_epi16(7);
	Vec128 v_inds0 = _mm_and_si128(v_besterr0, v_index_mask);
	Vec128 v_inds1 = _mm_and_si128(v_besterr1, v_index_mask);
	Vec128 v_inds = _mm_packs_epi16(v_inds0, v_inds1);
	store128u(pInds->vals, v_inds);

	// Extract the sum of squared errors
	Vec128 v_ssd0 = initial_err_dot_product(v_besterr0);
	Vec128 v_ssd1 = initial_err_dot_product(v_besterr1);

	// And do the final reduction
	Vec128 error_sum = _mm_add_epi32(v_ssd0, v_ssd1);

	*pError = reduce_add_u32(error_sum);
}
