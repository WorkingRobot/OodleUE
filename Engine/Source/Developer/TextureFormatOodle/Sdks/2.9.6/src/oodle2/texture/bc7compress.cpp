...
static void scalar_calc_subset_pca3(float mean[4], float pca[4], const BC7SubsetInput *in)
{
	...
	if_unlikely (len_sq == 0.0f)
	{
		...
		// The only way for this to happen is for all colors to be == mean exactly,
		// which is a degenerate case we should have caught in 7BC7Prep_init().
		RR_ASSERT( len_sq > 0.0f );
...
static void sse4_calc_subset_pca3(float mean[4], float pca[4], const BC7SubsetInput *in)
{
	...
	if_unlikely (_mm_ucomieq_ss(len_sq, VecF32x4::zero()))
	{
		...
		for (int i = 0; i < in->num_pixels; ++i)
		{
			...
			if (_mm_ucomigt_ss(len2, len_sq)) // if (len2 > len_sq)
			{
				local_pca = d;
				len_sq = len2;
			}
		...
		// The only way for this to happen is for all colors to be == mean exactly,
		// which is a degenerate case we should have caught in 7BC7Prep_init().
		RR_ASSERT( len_sq.scalar_x() > 0.0f );
