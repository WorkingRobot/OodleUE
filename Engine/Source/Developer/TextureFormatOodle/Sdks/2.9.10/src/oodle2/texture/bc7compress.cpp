...
static void sse4_neon_calc_subset_pca3(float mean[4], float pca[4], const BC7SubsetInput *in)
{
	...
	// Note(djg): len_sq is not 0.0, so there's no need to check for / 0.0.
	VecF32x4 final_pca = local_pca / len_sq.sqrt();
...
#if defined(DO_BUILD_SSE4) || defined(DO_BUILD_NEON64)
		VecF32x4 sdev_scale { sdev };
		if (has_ib2)
			sdev_scale = sdev_scale.insert_scalar<3>(sqrtf(sum_sqr_a * in->rcp_num_pixels));

		VecF32x4 vscaled_pca = sdev_scale * vpca;
#else
		float scaled_pca[4];
		float sdev_a = sdev;
		if (has_ib2)
