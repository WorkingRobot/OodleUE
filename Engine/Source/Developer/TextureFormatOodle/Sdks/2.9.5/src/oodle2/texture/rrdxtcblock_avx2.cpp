...
U32 DXT1_FindErrors_AVX2(const rrColorBlock4x4 & block, const rrColor32BGRA palette[4])
{
	...
	// Process pairs of rows
	Vec256 error_sum256;
	error_sum256 = FindErrorsInner(load256u(block.colors + 0), pal_br16, pal_ga16);
	error_sum256 = _mm256_add_epi32(error_sum256, FindErrorsInner(load256u(block.colors + 8), pal_br16, pal_ga16));
...
{
	const Vec256 * pix_br = (const Vec256 *)&ctx->pixels.unpacked[0];
	const Vec256 * pix_ga = (const Vec256 *)&ctx->pixels.unpacked[32];
...
U32 DXT1_FindBestPalette_AVX2(const DXT1_FindErrorsContext * ctx, const rrColor32BGRA palettes[], int count)
{
	...
	pix_br[0] = load256u(ctx->pixels.unpacked +  0);
	pix_br[1] = load256u(ctx->pixels.unpacked + 16);
	pix_ga[0] = load256u(ctx->pixels.unpacked + 32);
	pix_ga[1] = load256u(ctx->pixels.unpacked + 48);
