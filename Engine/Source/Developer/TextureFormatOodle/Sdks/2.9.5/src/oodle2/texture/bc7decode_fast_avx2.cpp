...
static RADFORCEINLINE void avx2_interpolate_one_subset_twoind(U8 * out_rgba, U32 crot_idxmode, const __m256i & lerpf8_256, const Vec128 &endpoints16_in)
{
...
#undef IDXS
	...
	__m256i lerpf16_shuf0 = load256u(lerpf16_shuffles[crot_idxmode]);
	...
	__m256i lerpf8_0 = _mm256_permute2x128_si256(lerpf8_256, lerpf8_256, 0x00); // broadcast low half
	__m256i lerpf8_1 = _mm256_permute2x128_si256(lerpf8_256, lerpf8_256, 0x11); // broadcast high half

	__m256i lerpf16_0 = _mm256_shuffle_epi8(lerpf8_0, lerpf16_shuf0);
	__m256i lerpf16_1 = _mm256_shuffle_epi8(lerpf8_0, lerpf16_shuf1);
	__m256i lerpf16_2 = _mm256_shuffle_epi8(lerpf8_1, lerpf16_shuf0);
	__m256i lerpf16_3 = _mm256_shuffle_epi8(lerpf8_1, lerpf16_shuf1);

	// Interpolate via rounding_shift_right(neg_factor * (lo - hi), 15) + lo
	__m256i interp16_0 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, diff16), lo16);
	__m256i interp16_1 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, diff16), lo16);
	__m256i interp16_2 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, diff16), lo16);
	__m256i interp16_3 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, diff16), lo16);

	// Pack down to 8 bits and store
	__m256i interp8_0 = _mm256_packus_epi16(interp16_0, interp16_1);
	__m256i interp8_1 = _mm256_packus_epi16(interp16_2, interp16_3);
...
#undef FOUR
	...
	const __m256i subset_mask = _mm256_set1_epi32(partition->subset_mask);

	const __m256i subset_shift_0 = _mm256_setr_epi32(31,31, 29,29, 23,23, 21,21);
	const __m256i subset_shift_1 = _mm256_setr_epi32(27,27, 25,25, 19,19, 17,17);
	...
	__m256 in_subset1_0 = _mm256_castsi256_ps(_mm256_sllv_epi32(subset_mask, subset_shift_0));
	__m256 in_subset1_1 = _mm256_castsi256_ps(_mm256_sllv_epi32(subset_mask, subset_shift_1));
	__m256 in_subset1_2 = _mm256_castsi256_ps(_mm256_sllv_epi32(subset_mask, subset_shift_2));
	__m256 in_subset1_3 = _mm256_castsi256_ps(_mm256_sllv_epi32(subset_mask, subset_shift_3));

	// Using these masks, select the "lo" and "diff" values to use
	__m256i lo16_0 = _mm256_castps_si256(_mm256_blendv_ps(lo16_subset0, lo16_subset1, in_subset1_0));
	__m256i lo16_1 = _mm256_castps_si256(_mm256_blendv_ps(lo16_subset0, lo16_subset1, in_subset1_1));
	__m256i lo16_2 = _mm256_castps_si256(_mm256_blendv_ps(lo16_subset0, lo16_subset1, in_subset1_2));
	__m256i lo16_3 = _mm256_castps_si256(_mm256_blendv_ps(lo16_subset0, lo16_subset1, in_subset1_3));

	__m256i diff16_0 = _mm256_castps_si256(_mm256_blendv_ps(diff16_subset0, diff16_subset1, in_subset1_0));
	__m256i diff16_1 = _mm256_castps_si256(_mm256_blendv_ps(diff16_subset0, diff16_subset1, in_subset1_1));
	__m256i diff16_2 = _mm256_castps_si256(_mm256_blendv_ps(diff16_subset0, diff16_subset1, in_subset1_2));
	__m256i diff16_3 = _mm256_castps_si256(_mm256_blendv_ps(diff16_subset0, diff16_subset1, in_subset1_3));

	// Grab the lerp factors
	__m256i lerpf8_0 = _mm256_permute2x128_si256(lerpf8_256, lerpf8_256, 0x00); // broadcast low half
	__m256i lerpf8_1 = _mm256_permute2x128_si256(lerpf8_256, lerpf8_256, 0x11); // broadcast high half

	__m256i lerpf16_0 = _mm256_shuffle_epi8(lerpf8_0, load256u(lerpf16_shuffles +  0));
	__m256i lerpf16_1 = _mm256_shuffle_epi8(lerpf8_0, load256u(lerpf16_shuffles + 32));
	__m256i lerpf16_2 = _mm256_shuffle_epi8(lerpf8_1, load256u(lerpf16_shuffles + 64));
	__m256i lerpf16_3 = _mm256_shuffle_epi8(lerpf8_1, load256u(lerpf16_shuffles + 96));

	// Interpolate via rounding_shift_right(neg_factor * (lo - hi), 15) + lo
	__m256i interp16_0 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, diff16_0), lo16_0);
	__m256i interp16_1 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, diff16_1), lo16_1);
	__m256i interp16_2 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, diff16_2), lo16_2);
	__m256i interp16_3 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, diff16_3), lo16_3);
...
void avx2_decode_mode4(U8 * out_rgba, const U8 * block_bits)
{
	...
	// And interpolate!
	avx2_interpolate_one_subset_twoind(out_rgba, block_bits[0] >> 5, lerpf8_256, endpoints16);
}
...
void avx2_decode_mode5(U8 * out_rgba, const U8 * block_bits)
{
	...
	avx2_interpolate_one_subset_twoind(out_rgba, block_bits[0] >> 6, lerpf8_256, endpoints16);
}
...
#undef FOUR
	...
	__m256i lerpf16_0 = _mm256_shuffle_epi8(lerpf256, load256a(lerpf16_shuffles +  0));
	__m256i lerpf16_1 = _mm256_shuffle_epi8(lerpf256, load256a(lerpf16_shuffles + 32));
	__m256i lerpf16_2 = _mm256_shuffle_epi8(lerpf256, load256a(lerpf16_shuffles + 64));
	__m256i lerpf16_3 = _mm256_shuffle_epi8(lerpf256, load256a(lerpf16_shuffles + 96));
...
#undef FOUR
	...
	// Broadcast
	__m256i lerpf8_256 = _mm256_inserti128_si256(_mm256_castsi128_si256(lerpf8), lerpf8, 1);

	// Replicate the lerp factors 4x each and expand to 16 bits by shifting left by 8
	__m256i lerpf16_0 = _mm256_shuffle_epi8(lerpf8_256, load256u(lerpf16_shuffles));
	__m256i lerpf16_1 = _mm256_shuffle_epi8(lerpf8_256, load256u(lerpf16_shuffles + 32));
	__m256i lerpf16_2 = _mm256_shuffle_epi8(lerpf8_256, load256u(lerpf16_shuffles + 64));
	__m256i lerpf16_3 = _mm256_shuffle_epi8(lerpf8_256, load256u(lerpf16_shuffles + 96));

	// Interpolate via rounding_shift_right(neg_factor * (lo - hi), 15) + lo
	// also computes diff to original block values which are subtracted from base
	__m256i interp16_0 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_0, load256u(b->diff +  0)), load256u(b->base +  0));
	__m256i interp16_1 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_1, load256u(b->diff + 16)), load256u(b->base + 16));
	__m256i interp16_2 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_2, load256u(b->diff + 32)), load256u(b->base + 32));
	__m256i interp16_3 = _mm256_add_epi16(_mm256_mulhrs_epi16(lerpf16_3, load256u(b->diff + 48)), load256u(b->base + 48));

	// First half of dot product: R*R + G*G, B*B + A*A
	// the rest we do as part of the reduction
	__m256i err_0 = _mm256_madd_epi16(interp16_0, interp16_0);
	__m256i err_1 = _mm256_madd_epi16(interp16_1, interp16_1);
	__m256i err_2 = _mm256_madd_epi16(interp16_2, interp16_2);
	__m256i err_3 = _mm256_madd_epi16(interp16_3, interp16_3);

	// Finish the reduction
	__m256i err_01 = _mm256_add_epi32(err_0, err_1);
	__m256i err_23 = _mm256_add_epi32(err_2, err_3);
	__m256i total_err = _mm256_add_epi32(err_01, err_23);

	__m128i lo_total = _mm256_castsi256_si128(total_err);
	__m128i hi_total = _mm256_extracti128_si256(total_err, 1);
	__m128i sum_total = _mm_add_epi32(lo_total, hi_total);
...
#undef INDS
	...
	const __m256i lerpf8_lo = _mm256_broadcastsi128_si256(load128u(lerpf_buf + 0));
	const __m256i lerpf8_hi = _mm256_broadcastsi128_si256(load128u(lerpf_buf + 16));
	const __m256i lshuf0 = load256u(lerpf16_shuffles[index_swap] + 0);
	const __m256i lshuf1 = load256u(lerpf16_shuffles[index_swap] + 32);

	__m256i lerpf16_0 = _mm256_shuffle_epi8(lerpf8_lo, lshuf0);
	__m256i lerpf16_1 = _mm256_shuffle_epi8(lerpf8_lo, lshuf1);
	__m256i lerpf16_2 = _mm256_shuffle_epi8(lerpf8_hi, lshuf0);
	__m256i lerpf16_3 = _mm256_shuffle_epi8(lerpf8_hi, lshuf1);
