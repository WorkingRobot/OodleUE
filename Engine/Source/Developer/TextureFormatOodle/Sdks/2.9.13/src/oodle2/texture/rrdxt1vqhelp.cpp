...
{			
	...
	for (SINTa epi = 0; epi < endpoints.sizea(); )
	{
		U32 cur = endpoints[epi];

		SINTa epj = epi + 1;
		while (epj < endpoints.sizea() && cur == endpoints[epj])
			++epj;

		counting.emplace_back(dword_and_count { cur, U32(epj - epi) });
		epi = epj;
	}
}		
...
{			
	...
	for (SINTa epi = 0; epi < endpoints.sizea(); )
	{
		...
		counting.emplace_back(bc7bits_and_count { cur, U32(epj - epi) });
		epi = epj;
	}
}
...
static S32 AnyIndexD_batch_lookup_SSE4(S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count)
{
	int i = 0;
	Vec128_S32 min_so_far { 0x7fffffff };
	...
	for (; i < (count & ~3); i += 4)
	{
		...
		min_so_far = min_so_far.min(Vec128_S32(v_ssd0));

		store128u(out_d + i, v_ssd0);
	}

	S32 min_d = reduce_min(min_so_far);

	// tail
	for (; i < count; ++i)
	{
		S32 d = AnyIndexD_lookup(aid, in_indices[i]);
		out_d[i] = d;
		min_d = RR_MIN(min_d, d);
	}
...
S32 AnyIndexD_batch_lookup(CpuDispatchFlags dispatch, S32 out_d[], const AnyIndexD * aid, const U32 in_indices[], int count)
{
	...
	if ( dispatch.AVX512() )
	{
		return internal::AnyIndexD_batch_lookup_AVX512(out_d, aid, in_indices, count);
	}
#endif
	...
	if ( dispatch.AVX2() )
	{
		return internal::AnyIndexD_batch_lookup_AVX2(out_d, aid, in_indices, count);
	}
#endif

	return AnyIndexD_batch_lookup_SSE4(out_d, aid, in_indices, count);
}
...
{
	// Do the filter first and the compaction second in a separate loop.
	//
	// The rejection rates here are high enough in typical use cases (typically 1-5% of items survive) that
	// it's not beneficial to the index filtering at the same time as the filtering loop: the decision here is
	// one between
	//   c1 * count
	// for the single pass and
	//   c2 * count + c3 * filtered_count
	// for the two-pass approach, where c2 < c1 (the cost of handling the indices in the main filtering loop is
	// nonzero).
	//
	// If filtered_count is small enough relative to count, and c3 small enough, then the two-pass solution is
	// preferable to single-pass.
...
S32 AnyIndexD_batch_lookup(CpuDispatchFlags /*dispatch*/, S32 out_d[], const AnyIndexD * in_aid, const U32 in_indices[], int count)
{
	S32 min_d = 0x7fffffff;

#ifdef DO_BUILD_NEON64
	// Copy AID values to on-stack var, which proves to the compiler
	// that they can't alias out_d and only need to be loaded once
	// (outside the loop.)
	AnyIndexD local_aid = *in_aid;
	const AnyIndexD * aid = &local_aid;
#else
	const AnyIndexD * aid = in_aid;
#endif
	for (int i = 0; i < count; ++i)
	{
		S32 d = AnyIndexD_lookup(aid, in_indices[i]);
...
static Vec128_U32 neon_index_diff4(const Vec128_U8 ref_inds, const U8 * ptr)
{
	// sum across 4 index diffs (half of the index SAD reduction)
	Vec128_U8 diff = abs_diff(ref_inds, Vec128_U8::loadu(ptr));
	Vec128_U16 diff16 = vpaddlq_u8(diff);
	return vpaddlq_u16(diff16);
}

#endif

int filter_by_index_sad(CpuDispatchFlags /*dispatch*/, int positions[], int count32, const U8 * index_batch, const U8 ref_inds[16], int sad_thresh)
{
	// copy to locals so compiler knows it doesn't alias positions
...
#if defined(DO_BUILD_NEON64)
	Vec128_U8 v_ref_inds = Vec128_U8::loadu(ref_inds);
	Vec128_S32 v_sad_thresh { sad_thresh };
	Vec128_S32 v_base_id { 0 };

	for (; i < (count & ~3); i += 4)
	{
		// Initial diffs across 4-lane groups
		Vec128_U32 presad0 = neon_index_diff4(v_ref_inds, index_batch + i*16 + 0*16);
		Vec128_U32 presad1 = neon_index_diff4(v_ref_inds, index_batch + i*16 + 1*16);
		Vec128_U32 presad2 = neon_index_diff4(v_ref_inds, index_batch + i*16 + 2*16);
		Vec128_U32 presad3 = neon_index_diff4(v_ref_inds, index_batch + i*16 + 3*16);

		// Finish summation
		Vec128_U32 presad01 = vpaddq_u32(presad0, presad1);
		Vec128_U32 presad23 = vpaddq_u32(presad2, presad3);
		Vec128_S32 sads = Vec128_U32(vpaddq_u32(presad01, presad23)).s32();

		// Compare against threshold
		Vec128_S32 below = v_sad_thresh.cmp_gt(sads);

		// Store lanes that match
		out_count += store_active_lane_ids(positions + out_count, v_base_id, below.u32());
		v_base_id += Vec128_S32(4);
	}
#endif
...
int filter_by_index_sad_with_inds(CpuDispatchFlags dispatch, int remaining_positions[], U32 out_indices[], int count, const U8 * index_batch, const U32 * in_indices, const U8 ref_inds[16], int threshold)
{
	...
	for (SINTa i = 0; i < (SINTa)filtered_count; i++)
		out_indices[i] = in_indices[remaining_positions[i]];
...
void batch_find_best_errors(
	...
	const rrColorBlock4x4 & colors)
{
#if defined(DO_BUILD_ARM_DOTPROD)
	if (dispatch.ARM_DotProd())
	{
		internal::batch_find_best_errors_ARM_DP(dest_ssd, vqendpoints, inds, count32, colors);
		return;
	}
#endif
...
S32 bc4rd_single_block_change_distortion_explicit(const S16 *src_pixels, const S16 palette[8], const U8 * index8_scaled_ptr)
{
	...
	Vec128_U8 index8_scaled = Vec128_U8::loadu(index8_scaled_ptr);
	...
	Vec128_S16 result_0 = pal.u8().shuf(index16_0).s16();
	Vec128_S16 result_1 = pal.u8().shuf(index16_1).s16();
	...
	Vec128_S32 sum = vmull_low_s16(diff01_16, diff01_16);
	sum = vmlal_high_s16(sum, diff01_16, diff01_16);
	sum = vmlal_low_s16 (sum, diff23_16, diff23_16);
	sum = vmlal_high_s16(sum, diff23_16, diff23_16);

	// Horizontal reduction
	return reduce_add(sum);

#else // DO_BUILD_NEON64

	S32 ssd = 0;
	for (SINTa i = 0; i < 16; i++)
	{
		S32 decoded = palette[index8_scaled_ptr[i] >> 1];
		S32 diff = decoded - src_pixels[i];
		ssd += diff * diff;
	}
...
static RADFORCEINLINE VecF32x4 batch_VQD_weight_calc(const Vec128_U8 & pixel, const Vec128_U8 & pal_vec, const VecF32x4 & weight)
{
	// Compute the absolute differences in 8 bits
	Vec128_U8 diff = abs_diff(pixel, pal_vec);

	// squares of differences in 4 lanes, each is R+G+B+A
	Vec128_U32 sum = Vec128_U32::sqr_sum(diff);

	// convert to float and multiply with weight
	return VecF32x4::from_int32(sum) * weight;
}

static RADFORCEINLINE VecF32x4 batch_VQD_shuffle_one(const VecF32x4 vec, const Vec128_U8 & shuffle_inds)
{
	return vec.u8().shuf(shuffle_inds).f32();
}

static RADFORCEINLINE void batch_VQD_sum_along_col(VecF32x4 & out_sum0, VecF32x4 & out_sum1, const VecF32x4 errors[][2], const Vec128_U32 & inds_shl2)
{
	const Vec128_U8 c_byte_inds = Vec128_U32(0x03020100).u8(); // select all 4 bytes of the DWord we need

	// Indices are pre-shfited by 2, but still need to mask the 2 index bits we need
	const Vec128_U8 inds_masked = inds_shl2.u8() & Vec128_U8(3 << 2);

	// Now, for our 4 summands, form the final index, and sum down the column
	Vec128_U8 shuf_index;

	shuf_index = inds_masked.shuf(Vec128_U8 { 0,0,0,0, 4,4,4,4, 8,8,8,8, 12,12,12,12 }) | c_byte_inds;
	out_sum0 = batch_VQD_shuffle_one(errors[0][0], shuf_index);
	out_sum1 = batch_VQD_shuffle_one(errors[0][1], shuf_index);

	shuf_index = inds_masked.shuf(Vec128_U8 { 1,1,1,1, 5,5,5,5, 9,9,9,9, 13,13,13,13 }) | c_byte_inds;
	out_sum0 += batch_VQD_shuffle_one(errors[4][0], shuf_index);
	out_sum1 += batch_VQD_shuffle_one(errors[4][1], shuf_index);

	shuf_index = inds_masked.shuf(Vec128_U8 { 2,2,2,2, 6,6,6,6, 10,10,10,10, 14,14,14,14 }) | c_byte_inds;
	out_sum0 += batch_VQD_shuffle_one(errors[8][0], shuf_index);
	out_sum1 += batch_VQD_shuffle_one(errors[8][1], shuf_index);

	shuf_index = inds_masked.shuf(Vec128_U8 { 3,3,3,3, 7,7,7,7, 11,11,11,11, 15,15,15,15 }) | c_byte_inds;
	...
	U32 bbox_min_d, bool require_3c)
{
	...
	if ( dispatch.ARM_DotProd() )
	{
		return internal::filter_endpoint_color_bbox_ARM_DP(
			dest_inds,
			vqendpoints, inds, count32,
			bbox,
			bbox_min_d, require_3c);
	...
	return internal::filter_endpoint_color_bbox_NEON_core<internal::NEONKernelTraits>(
		dest_inds,
		vqendpoints, inds, count32,
		bbox,
		bbox_min_d, require_3c);
	...
	const SingleFloatBlock4x4 & activity)
{
	...
	const Vec128_U8 pal0 = Vec128_U8::loadu(first_palette);
	const Vec128_U8 pal1 = Vec128_U8::loadu(second_palette);
	...
	for (i = 0; i < 16; ++i)
	{
		...
		Vec128_U8 pixel = Vec128_U32(colors.colors[i].dw).u8();
	...
	for (i = 0; i < count; i += 4)
	{
		...
		else
		{
			...
			index_vec = Vec128_U32::loadu_dup32(&vqindices[inds[i + 0]].dw);
			if (i + 1 < count) index_vec.load_lane<1>(&vqindices[inds[i + 1]].dw);
			if (i + 2 < count) index_vec.load_lane<2>(&vqindices[inds[i + 2]].dw);
		}
		...
		batch_VQD_sum_along_col(sum0p0, sum0p1, weighted_errs + 0, index_vec.shl<2>());
		batch_VQD_sum_along_col(sum1p0, sum1p1, weighted_errs + 1, index_vec);
		batch_VQD_sum_along_col(sum2p0, sum2p1, weighted_errs + 2, index_vec.srl<2>());
		batch_VQD_sum_along_col(sum3p0, sum3p1, weighted_errs + 3, index_vec.srl<4>());
...
#else // DO_BUILD_NEON64
		float32x4x2_t sum0p01 = { sum0p0, sum0p1 };
		vst2q_f32(dest_ptr, sum0p01);
#endif
	}
...
S32 compute_sad_bound_for_ssd_threshold(S32 delta_ssd_per_pixel_per_sad[16][8], S32 base_ssd, S32 ssd_limit, S32 * out_ssd_limit)
{
	RAD_ALIGN(U32, delta_ssd[16][8], 16);
	RAD_ALIGN(U32, initial_ssd[16], 16);

	// clamp ssd_limit so that we can use it as a limit per pixel and leave room for lane IDs
	ssd_limit = RR_MIN(ssd_limit, (1 << 28)-2);

	Vec128_U32 vi = Vec128_U32::zero();
	Vec128_U32 limit(ssd_limit + 1);
	for (int i=0; i < 16; ++i)
	{
		// Clamp error values at limit and insert the pixel ID
		Vec128_U32 a = Vec128_U32::loadu(delta_ssd_per_pixel_per_sad[i] + 0).min(limit).shl<4>() + vi;
		Vec128_U32 b = Vec128_U32::loadu(delta_ssd_per_pixel_per_sad[i] + 4).min(limit).shl<4>() + vi;
		vi += Vec128_U32(1);
		a.storea(delta_ssd[i] + 0);
		b.storea(delta_ssd[i] + 4);
		a.storeu_lo32(&initial_ssd[i]);
	}

	Vec128_U32 cur_ssd0 = Vec128_U32::loada(initial_ssd + 0);
	Vec128_U32 cur_ssd1 = Vec128_U32::loada(initial_ssd + 4);
	Vec128_U32 cur_ssd2 = Vec128_U32::loada(initial_ssd + 8);
	Vec128_U32 cur_ssd3 = Vec128_U32::loada(initial_ssd + 12);
	...
	for (sad=1;;++sad)
	{
		...
		Vec128_U32 ab = cur_ssd0.min(cur_ssd1);
		Vec128_U32 cd = cur_ssd2.min(cur_ssd3);
		Vec128_U32 abcd = ab.min(cd);
		smallest_ssd = reduce_min(abcd);

		Vec128_U32 smallest_ssd_vec(smallest_ssd);
		...
		// work our which lane of cur_ssd to update
		Vec128_U32 update_lane = smallest_ssd_vec & Vec128_U32(15);

		// grab next value delta and shift them in place (faster than indirect addressing)
		Vec128_U32 next_value(delta_ssd[smallest_p][1]);
		Vec128_U32 delta0 = Vec128_U32::loada(&delta_ssd[smallest_p][0]);
		Vec128_U32 delta1 = Vec128_U32::loada(&delta_ssd[smallest_p][4]);

		Vec128_U32 new_delta0 = vsrl_byte<4>(delta0, delta1);
		Vec128_U32 new_delta1 = delta1.srl_byte<4>();
		new_delta0.storea(&delta_ssd[smallest_p][0]);
		new_delta1.storea(&delta_ssd[smallest_p][4]);

		// update the right lane of cur_ssd
		cur_ssd0 = update_lane.cmp_eq(Vec128_U32 {  0, 1, 2, 3 }).select(next_value, cur_ssd0);
		cur_ssd1 = update_lane.cmp_eq(Vec128_U32 {  4, 5, 6, 7 }).select(next_value, cur_ssd1);
		cur_ssd2 = update_lane.cmp_eq(Vec128_U32 {  8, 9,10,11 }).select(next_value, cur_ssd2);
		cur_ssd3 = update_lane.cmp_eq(Vec128_U32 { 12,13,14,15 }).select(next_value, cur_ssd3);
	}
	if ((S32) smallest_ssd > ssd_limit)
	{
		if ( out_ssd_limit ) *out_ssd_limit = ssd_limit;
		return 16*7; // maximum index sad if we consumed all entries (indicated by smallest_ssd being a sentinel)
	}
	if ( out_ssd_limit ) *out_ssd_limit = total_ssd;
	return sad-1;
}

#else // no SSE4 and no NEON64

S32 compute_sad_bound_for_ssd_threshold(S32 ssd_for_index_delta[16][8], S32 base_ssd, S32 ssd_limit, S32 * out_ssd_limit)
{
	// clamp SSD limit so we can use our indexing trick
	// ssd_limit = max limit we target
	// clamp_limit = what we clamp out-of-range values to (guaranteed >ssd_limit so falls out)
	ssd_limit = RR_MIN(ssd_limit, (1 << 28)-2);
	int clamp_limit = ssd_limit + 1;

	// initially, current delta for each pixel is the first
	U32 delta_cur[16];
	int index_delta_cur[16] = { 0 };
	for (int p = 0; p < 16; ++p)
		delta_cur[p] = (RR_MIN(ssd_for_index_delta[p][0], clamp_limit) << 4) | p;
	...
	{
		// find the smallest ssd delta
		smallest_ssd = delta_cur[0];
		for (int p=1; p < 16; ++p)
			smallest_ssd = RR_MIN(smallest_ssd, delta_cur[p]);
		...
		// now pixel smallest_p picks the next-larger index delta
		++index_delta_cur[smallest_p];
		if (index_delta_cur[smallest_p] >= 8)
		{
			delta_cur[smallest_p] = (clamp_limit << 4) | smallest_p; // sentinel
		}
		else
		{
			S32 next = RR_MIN(ssd_for_index_delta[smallest_p][index_delta_cur[smallest_p]], clamp_limit);
			delta_cur[smallest_p] = (next << 4) | smallest_p;
		}
	}
...
const U32 c_active_lane_table8[256] =
{
