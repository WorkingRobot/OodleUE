	...
	const bc7bits & bits_mask)
{
	...
	// We are only changing indices; clear preserve extremes flags for endpoints in here,
	// the endpoints of the blocks we emit are ignored, we only care about the final
	// index bits. We _do_ need to obey the index constraints, though. (See below.)
	flags &= ~BC7ENC_PRESERVE_EXTREMES;
