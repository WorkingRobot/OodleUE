...
static U64 AnyIndexD_add_with_1bt(AnyIndexD * aid,const rrColorBlock4x4 & colors,rrColor32BGRA palette[4])
{
	U64 illegal_index_mask = 0;
	...
	for LOOP(c,16)
	{
		...
		for LOOP(p,4)
		{
			U32 err = rrColor32BGRA_DeltaSqrRGBA_1BT(cur,palette[p]);
			aid->ssd[p][c] += err;

			// 1-bit-transparency errors are hard constraint violations; hitting this even once needs to be
			// a hard out, several other blocks with large error don't get to outvote a single block with
			// a constraint violation. Track such pixels as actually illegal index values.
			if (err > 3*255*255)
			{
				illegal_index_mask |= 1ull << (p*16 + c);
			}
		}
	}

	return illegal_index_mask;
}
...
static U32 AnyIndexD_best_index_pair_D(const AnyIndexD * aid1, const AnyIndexD * aid2)
{
	...
	for (int c=0;c<16;c+=4)
	{
		...
		error_sum += (v0.min(v1)).min(v2.min(v3));
	}
...
static U32 AnyIndexD_add_then_find_indices(AnyIndexD * to, const AnyIndexD & fm, U32 * pD)
{
	...
	*/
	...
	for (int i=0;i<4;++i)
	{
		...
		Vec128_U32 best01 = d0.shl<2>().min(d1.shl<2>() | Vec128_U32(1));
		Vec128_U32 best23 = d2.shl<2>().min(d3.shl<2>() | Vec128_U32(1));
		Vec128_U32 best = best01.min(best23 + Vec128_U32(2));
	...
	const rrDXT1_VQ_Config & config)
{
	...
	*/
	...
	for LOOPINT(epi,num_endpoints)
	{
		for LOOPINT(indi,num_indices)
		{
...
static void update_mtf_window(int new_val,int * lz_window, int & lz_window_size, int max_lz_window_size)
{
	...
	int lz_found_i = -1;
	for LOOPINT(lz_i,lz_window_size)
	{
...
{
	const int nblocks = check_value_cast<int>(blocks->count);
	if ( ! nblocks )
		return true;
	...
	const rrDXTCRD_Options & rdopts)
{
	...
	int nblocks = check_value_cast<int>(to_blocks->count);
	...
	for LOOPINT(i,max_either_window_size)
	...
	for LOOP(vq_iterations,config.num_vq_iterations)
	{
		...
		for LOOPINT(i,endpoint_counting.size32())
		{
		...
		for LOOPINT(i,index_counting.size32())
		{
		...
		// for each block
		for LOOPINT(block_i,blocks.size32())
		{
	...
	rrDXT1PaletteMode pal_mode)
{
	...
	for(int dcvi=dcv_i_start;dcvi<dcv_i_end;dcvi++)
	{
		...
		{
			// we need to be careful about alpha and track illegal index values
			illegal_index_mask |= AnyIndexD_add_with_1bt(&aid,colors,palette);
		}
	}

	// if there are illegal index candidates, fix up the distortion scores so they can't get picked
	if (illegal_index_mask)
	{
		for LOOP(p,4)
		{
			for LOOP(c,16)
			{
				// Clamp legal values at 0x3fff'fffe
				aid.ssd[p][c] = RR_MIN(aid.ssd[p][c], 0x3fff'fffe);
				// Illegal pixels get 0x3fff'ffff
				if (illegal_index_mask & (1ull << (p*16 + c)))
				{
					aid.ssd[p][c] = 0x3fff'ffff;
				}
			}
		}
	}
	...
	rrDXT1PaletteMode pal_mode)
{
	...
	while(dcv_i_start<nblocks)
	{
		...
		else // count >= 2
		{
			...
			// don't bother when the count is huge, we're not gonna find anything and it can cause overflow problems
			
			U32 new_indices = old_indices;
			if ( count < AnyIndexD::MAX_BLOCKS_FOR_S32 &&
				DXT1_FindIndices_MultiBlock(&new_indices,old_indices,blocks,dcv,dcv_i_start,dcv_i_end,pal_mode) )
			{
...
static U32 block_link_to_AnyIndexD(const index_vq_entry * entry, const rrDXT1_VQ_Block * blocks, AnyIndexD * aid, rrDXT1PaletteMode pal_mode )
{
	...
	RR_DURING_ASSERT( U32 link_count = 0 );
...
static RADNOINLINE void actually_shrink_vq_heap(vector<index_vq_heap_entry>& heap, SINTa target_limit)
{
	...
	stdsort(heap.data(), heap.data() + heap.size(), stdgreater_via_less<index_vq_heap_entry>());
	heap.resize(target_limit);
}

// if heap is at maximum size, shrink it to be half that size
static inline void test_shrink_vq_heap(vector<index_vq_heap_entry>& heap, SINTa limit)
{
	...
	const rrDXT1_VQ_Config & config)
{
	...
	indices_out->clear();
	RR_DURING_ASSERT(U32 total_count_out = 0);
	...
	for LOOPINT(loop_is3c,2)
	{
		indices_nc_raw.clear();
		indices_nc_sorted.clear();
		
		for LOOPINT(b,nblocks_total)
		{
		...
		RR_DURING_ASSERT(U32 total_count_in = 0);
		for LOOPVEC(i,indices_nc_sorted)
		{
			...
			RR_DURING_ASSERT(total_count_in += cur_entries[i].count);
		}
		...
		for LOOPINT(i,indices_nc_sorted.size32())
		{
	...
	const SINTa vq_heap_limit = RR_MAX(num_entries,64) * 8;
	...
	for (int mode_is3c = 0; mode_is3c <= 1; ++mode_is3c)
	{
		...
		for (int to_chunk_begin = mode_begin; to_chunk_begin < mode_end; to_chunk_begin += entry_chunk_size)
		{
			...
			for (int fm = mode_begin; fm < mode_end; ++fm)
			{
				...
				// likewise with "from" entries used frequently enough that their AIDs might run into overflows
				if ( indices_are_flat(entries[fm].indices) || entries[fm].count >= AnyIndexD::MAX_BLOCKS_FOR_S32 )
					continue;
				...
				for (int i = 0; ; i++) // yes, no loop condition!
				{
					...
					RR_ASSERT( 0 <= distortion && distortion <= best_distortion );

					// This should never hit, but we've hit crashes here due to internal overflows in the distortion
					// calc, and if our best_distortion ever goes negative really bad stuff can happen.
					if ( distortion < 0 )
						continue;
	...
	for LOOPVEC(entry_i,entries)
	{
		...
		RR_DURING_ASSERT(total_count_out += entries[entry_i].count);
	}
