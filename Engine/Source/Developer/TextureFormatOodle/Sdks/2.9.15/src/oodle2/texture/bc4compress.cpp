...
static RADFORCEINLINE Vec128_U16 calc_bc4_pixel_error(const Vec128_S16 & pixels, const Vec128_S16 & pal_entry, const Vec128_U16 & index_bias)
{
	...
	Vec128_U16 clamped = diff.min(Vec128_U16((1 << 13) - 1));
...
static RADFORCEINLINE U32 initial_err_dot_product(const Vec128_U16 & input0, const Vec128_U16 & input1)
{
	...
	Vec128_S32 result = vmull_low_s16(errs0, errs0);
	result = vmlal_high_s16(result, errs0, errs0);
	result = vmlal_low_s16(result, errs1, errs1);
	result = vmlal_high_s16(result, errs1, errs1);
...
static void BC4_FindIndices(const BC4SourceData& values, typename Tblock::Endpoint e0, typename Tblock::Endpoint e1, U32* pError, BC4Indices* pInds)
{
	...
	for (int index = 0; index < 8; index++)
	{
		...
		v_besterr0 = v_besterr0.min(calc_bc4_pixel_error(v_pixels0, v_pal_value, v_index_bias));
		v_besterr1 = v_besterr1.min(calc_bc4_pixel_error(v_pixels1, v_pal_value, v_index_bias));
	...
	Vec128_U8 v_inds = v_inds0.to_u8_sat(v_inds1);
...
static void BC5_Deinterleave_U16(U16 * RADRESTRICT destR, U16 * RADRESTRICT destG, const U16 * srcRG)
{
	for (int i = 0; i < 16; i++)
	{
		destR[i] = srcRG[i*2 + 0];
...
static void BC4_ConvertSourcePixels_U16(BC4SourceData * pBlock, const U16 * source16)
{
...
static void BC4_ConvertSourcePixels_S16(BC4SourceData * pBlock, const S16 * source16)
{
...
void BC4_ReadSourceFromBlock(BC4SourceData * pBlock, const void * src, BC4SourceFormat format)
{
	...
	case BC4SourceFormat_U16:
		BC4_ConvertSourcePixels_U16(pBlock, reinterpret_cast<const U16 *>(src));
		break;

	case BC4SourceFormat_S16:
		BC4_ConvertSourcePixels_S16(pBlock, reinterpret_cast<const S16 *>(src));
		break;
...
void BC5_ReadSourceFromBlock(BC4SourceData * pBlockR, BC4SourceData * pBlockG, const void * block, BC4SourceFormat format)
{
	...
	case BC4SourceFormat_U16:
		BC5_Deinterleave_U16(source16_r, source16_g, reinterpret_cast<const U16*>(block));
		BC4_ConvertSourcePixels_U16(pBlockR, source16_r);
		BC4_ConvertSourcePixels_U16(pBlockG, source16_g);
		break;

	case BC4SourceFormat_S16:
		BC5_Deinterleave_U16(source16_r, source16_g, reinterpret_cast<const U16*>(block));
		BC4_ConvertSourcePixels_S16(pBlockR, reinterpret_cast<const S16 *>(source16_r));
		BC4_ConvertSourcePixels_S16(pBlockG, reinterpret_cast<const S16 *>(source16_g));
		break;
...
void BC4_GreedyOptimizeBlock2(Tblock * pBlock, U32 * pError, const BC4SourceData & values, rrDXTCOptions options)
{
	...
	U32 try_options = 0;
	if ( (pBlock->e1 == Tblock::minimum_q || pBlock->e0 <= pBlock->e1) && (options & rrDXTCOptions_BC3457_PreserveExtremes) )
		try_options |= TryOptions_NeedMin;
	if ( (pBlock->e0 == Tblock::maximum_q || pBlock->e0 <= pBlock->e1) && (options & rrDXTCOptions_BC3457_PreserveExtremes) )
